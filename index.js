...
.then(async () => {
    ...
    // ====================================================================
    // 🔧 TEMP MIGRATION — شغّالة مرة وحدة فقط، احذف هذا البلوك كامل بعد التشغيل
    // يبدّل قدرة "👻 خطوة ما بين الأبعاد" بقدرة "🔱 إرادة الأبدية"
    // لشخصية Cayena Hill عند اللاعب 89602194993244@lid — بشرط تكون
    // الشخصية فعلاً رتبة أوميقا (evolutionLevel === 7)، وإلا ما يسوي شي.
    // ====================================================================
    try {

        const TEMP_TARGET_USER_ID = '89602194993244@lid'
        const TEMP_TARGET_CHAR_NAME = 'Cayena Hill'

        const TEMP_OLD_ABILITY_NAME = '👻 خطوة ما بين الأبعاد'
        const TEMP_NEW_ABILITY = {
            name: "🔱 إرادة الأبدية",
            type: "attack",
            value: 50,
            description: "+50% هجوم — أقوى ضربة ممكنة",
            chance: 4
        }

        const tempPlayer = await Player.findOne({ userId: TEMP_TARGET_USER_ID })

        if (!tempPlayer) {
            console.log('🔧 [TEMP MIGRATION] ❌ اللاعب غير موجود:', TEMP_TARGET_USER_ID)
        } else {

            const tempChar = (tempPlayer.characters || []).find(
                c => c.name === TEMP_TARGET_CHAR_NAME && c.evolutionLevel === 7
            )

            if (!tempChar) {
                console.log('🔧 [TEMP MIGRATION] ❌ ما لقيت شخصية أوميقا بهذا الاسم عند اللاعب — تأكد من الاسم أو رتبة التطوير.')
            } else if (!Array.isArray(tempChar.urAbilities) || !tempChar.urAbilities.length) {
                console.log('🔧 [TEMP MIGRATION] ❌ ما فيه قدرات أوميقا مسجلة لهذي الشخصية')
            } else {

                const tempIdx = tempChar.urAbilities.findIndex(
                    a => a && a.name === TEMP_OLD_ABILITY_NAME
                )

                if (tempIdx === -1) {
                    console.log('🔧 [TEMP MIGRATION] ❌ ما لقيت قدرة "خطوة ما بين الأبعاد" — القدرات الحالية:',
                        tempChar.urAbilities.map(a => a.name))
                } else {

                    console.log(`🔧 [TEMP MIGRATION] 🔁 استبدال قدرة رقم ${tempIdx + 1}: "${tempChar.urAbilities[tempIdx].name}" ← "${TEMP_NEW_ABILITY.name}"`)

                    tempChar.urAbilities[tempIdx] = { ...TEMP_NEW_ABILITY }

                    tempPlayer.markModified('characters')
                    await tempPlayer.save()

                    console.log('🔧 [TEMP MIGRATION] ✅ تم الاستبدال والحفظ بنجاح — احذف هذا البلوك الآن')
                }
            }
        }

    } catch (tempErr) {
        console.log('🔧 [TEMP MIGRATION] ❌ خطأ:', tempErr)
    }
    // ====================================================================
    // 🔧 END TEMP MIGRATION
    // ====================================================================
