...
const commandExplanations = {
    ...
    'الشخصيات': {
        ...
        '.عرض': 'يعرض تفاصيل شخصية معينة تملكها برقمها بقائمة .شخصياتي. مثال: .عرض رقم_الشخصية',
...
╰━━━━━━━━━━━━━━╯`
    ...
    await sendCharacterCardFrame(
        sock,
        msg.key.remoteJid,
        applyCustomImage(mainCharacter, mainCharacter),
        profileText,
        { mentions: [userId] }
    )
...
💰 ${omegaCost.toLocaleString()}`
    ...
    return sendCharacterCardFrame(
        sock,
        msg.key.remoteJid,
        applyCustomImage(char, char),
        characterFrameCaption(char.name, [
...
const currentRankLabel =
    ...
    // ⚠️ .عرض ما عاد يبعث كابشن نصي إطلاقًا (كل الرتب: SSS وفوق،
    // واسطوري وتحت) — البطاقة نفسها (renderSingleCharacterCard) فيها
    // كل المعلومات (الاسم/الرتبة/القوة/الأنمي) فما نحتاج نكررها نص.
    // نبقي captionFallback بس عشان الحالة النادرة اللي تفشل فيها حتى
    // الصورة الخام (sendCharacterCardFrame يرجع لإرسال نص لو كل شي فشل).
