...
// ⚡ تحسين أداء: فحص وجود ملف بدون حجب الـ event loop.
// fs.existsSync يوقف تنفيذ كل شيء بالبوت (كل اللاعبين) لحين انتهاء
// عملية القرص — هذي النسخة async تعطي نفس النتيجة (true/false) بدون
// حجب أي رسائل ثانية وصلت بنفس اللحظة. لا تغيّر أي منطق، فقط لا تحجب.
//
// 🗃️ إضافة: كاش بمهلة 5 دقائق. مسارات صور الشخصيات ثابتة (كتالوج
// البوت) فما تتغير أثناء التشغيل، فتفادي ضربة قرص فعلية بكل استدعاء
// آمن. المهلة الزمنية (بدل كاش دائم) موجودة كصمام أمان: لو مسار ملف
// انحذف/انضاف فعلياً لأي سبب، النتيجة تتصحح تلقائياً خلال 5 دقائق
// كحد أقصى بدل ما تبقى خاطئة للأبد.
const _fileExistsCache = new Map()
const FILE_EXISTS_CACHE_TTL = 5 * 60 * 1000

async function fileExists(p) {
    const cached = _fileExistsCache.get(p)

    if (cached && Date.now() - cached.at < FILE_EXISTS_CACHE_TTL) {
        return cached.value
    }
    ...
    _fileExistsCache.set(p, { value: result, at: Date.now() })
    return result
}

// ⚡ صورة محلية جاهزة للإرسال:
// - لو كاش Baileys الرسمي شغّال (mediaCache): نرسلها كمسار ({ url }) عشان يقدر
//   يعيد استخدام رفعها السابق لواتساب (نفس الصورة تنرفع مرة وحدة بس)
// - غير كذا: نقرأها Buffer مثل قبل بالضبط
async function localImageForSend(imagePath) {
    if (global.mediaCacheActive) return { url: imagePath }
    return await fs.promises.readFile(imagePath)
}

const battleLocks = new Set();
// 🔒 قفل لأمر .غزو يمنع تنفيذ أكثر من رسالة بنفس الوقت لنفس اللاعب
// (بدونه، رسالتين متقاربتين تقرآن نفس stage قبل الحفظ فيطلع نفس رقم التقدم مرتين)
const raidLocks = new Set();
// 🔒 قفل لأمر .اهداء يمنع أي عمليتين متزامنتين تلمسان نفس اللاعب
// (مُهدي أو مُهدى له) بنفس الوقت
const giftLocks = new Set();
// 🔒 قفل لأمر .اسحب يمنع تنفيذ أكثر من سحبة بنفس الوقت لنفس اللاعب
// (بدونه، عدة رسائل .اسحب متزامنة تقرأ نفس player.pulls من قاعدة البيانات
// قبل ما أي وحدة منها تحفظ التخفيض، فتمر كلها من فحص pulls <= 0 وتُنفَّذ
// كلها رغم إن الرصيد الفعلي 5 فقط)
const pullLocks = new Set();
// 🔒 قفل لأمر .سحب_سلاح — نفس فكرة قفل .اسحب بالضبط، بس لرصيد
// weaponPulls بدل pulls (رصيد منفصل تماماً، فقفل منفصل تماماً)
const weaponPullLocks = new Set();
...
const collageMaker = require('./systems/collageMaker')

// -----------------------------------------------------------
// 🐱🐶 دوال مساعدة — تطبّق بونص رفيق القط (مال) والكلب (خبرة)
// على أي مكان بالكود يضيف مال/خبرة مباشرة بدون ما يمر على
// Player.addMoney (اللي القط مربوط فيها أصلاً)
// -----------------------------------------------------------
// 🖼️ لو هذي النسخة بالضبط من الشخصية عندها صورة مستبدلة (owned.customImage
// من أمر .استبدال)، نرجع نسخة معدّلة من كائن العرض بالصورة المخصصة بدل
// الافتراضية. خاص باللاعب صاحب النسخة فقط — displayChar ممكن يكون owned
// نفسه أو كائن مدموج ببيانات الكتالوج (latest)، بأي الحالتين آخر خطوة
// دايمًا تفضّل customImage لو موجودة.
function applyCustomImage(owned, displayChar) {
    if (owned && owned.customImage) {
        return { ...displayChar, image: owned.customImage }
    }
    return displayChar
}

// ============================================================
// 🖼️ يحوّل character.image (رابط خارجي https://... أو مسار محلي نسبي
// زي custom_images/xxx.jpg اللي يحطه .استبدال، أو صورة كتالوج عادية
// بمجلد البوت) إلى قيمة جاهزة للإرسال المباشر عبر Baileys — بدون
// أي تصميم/بطاقة، نفس أسلوب الإرسال القديم (image خام + كابشن).
// يرجع {url} للروابط الخارجية، أو Buffer لو ملف محلي موجود، أو null
// لو الصورة غير موجودة/مفقودة (عشان المستدعي يرسل رسالة خطأ مناسبة).
// ============================================================
async function resolveCharacterImageForSend(imagePath) {
...
function applyCatBonus(player, amount) {
    if (
        player &&
        player.companion &&
        player.companion.key === 'cat' &&
        (player.companion.level || 0) >= 1
    ) {
        const bonus =
            companionsData.getCompanionBonus('cat', player.companion.level)

        return Math.floor(amount * (1 + bonus / 100))
    }
    return amount
}

function applyDogBonus(player, amount) {
    if (
        player &&
        player.companion &&
        player.companion.key === 'dog' &&
        (player.companion.level || 0) >= 1
    ) {
        const bonus =
            companionsData.getCompanionBonus('dog', player.companion.level)
...
// 🎯 لعبة كود نيمز
// =========================
const codenamesGame = require('./systems/codenamesGame')
// =========================
// 🏆 بطولة أجمل معرض
// =========================
const galleryTournament = require('./systems/galleryTournament')
// ⚡ تُستدعى مع كل رسالة تبدأ بـ "." (كل أوامر البوت تقريباً)، وكانت
// تعيد حساب Intl.DateTimeFormat في كل مرة. النتيجة ما تتغير إلا مع
// تغيّر الدقيقة، فنخزّنها لثانيتين فقط — فرق آمن تماماً (أسوأ حالة:
// تأخر دقيقتين ثانية بتفعيل/تعطيل البوت عند حدود الساعة، غير محسوس).
let _botAvailableCache = { value: false, at: 0 }

function botAvailable() {
    const now = Date.now()

    if (now - _botAvailableCache.at < 2000) {
        return _botAvailableCache.value
    }
...
// 🕐 يرجّع الساعة الحالية بتوقيت الرياض (0-23) — تُستخدم لتحديد نافذة
// فتح/إغلاق قروب هجوم الزعيم، بمعزل عن توقيت السيرفر نفسه (Render
// عادة UTC)، بنفس أسلوب botAvailable أعلاه.
...
const { renderRosterImage, resolveTierKey, renderSingleCharacterCard } = require('./systems/myRosterCard')
const {
    MAX_GALLERY,
    getGalleryCharacters,
    addToGallery,
    removeFromGallery
} = require('./systems/gallerySystem')
...
// =========================
// ⏰ نافذة هجوم الزعيم — تفتح تلقائياً كل رأس ساعة لمدة 10 دقائق فقط
// =========================
const BOSS_ATTACK_GROUP = '120363409442561948@g.us'

// 🆕 قروب إضافي مسموح فيه أمر .هجوم دائماً (بدون فتح/إغلاق تلقائي —
...
// 🎯 نظام الكريتيكال المتدرج (على غرار Wuthering Waves/جينشين)
// crit rate يُحسم كنسبة رمي نرد لين 100% بالضبط — أي فايض فوق الـ100%
// ما يضيع، يتحول تلقائيًا crit damage إضافي. مثال: critRate=130%
// → فرصة كريت 100% مضمونة + 30% تتحول ضرر حرج إضافي فوق الـcritDamage الأساسي
function rollCrit(critRate, critDamage) {
    const rate = critRate || 0
    const dmg = critDamage || 0

    const overflow = Math.max(0, rate - 100)
    const effectiveRate = Math.min(100, Math.max(0, rate))
    const effectiveDamage = dmg + overflow

    const isCrit = Math.random() * 100 < effectiveRate

    return {
        isCrit,
        multiplier: isCrit ? (1 + effectiveDamage / 100) : 1
    }
}

// 🔁 إعادة محاولة الإرسال عدد محدد من المرات (افتراضي 3) مع فاصل
// زمني بينها — تستخدم للرسائل المهمة اللي لازم توصل حتى لو صار
// انقطاع مؤقت بالاتصال (Connection Closed)
async function sendMessageWithRetry(sock, jid, content, maxRetries = 3, delayMs = 3000) {
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
        try {
            return await sock.sendMessage(jid, content)
        } catch (err) {
            console.log(
                `⚠️ محاولة ${attempt}/${maxRetries} فشلت لإرسال رسالة لـ ${jid}:`,
                err?.message || err
            )
...
// 🔴 دالة إغلاق قروب الهجوم — قابلة للاستدعاء من أي مكان
// (من التايمر التلقائي عند الدقيقة 10، أو فورًا عند سقوط الزعيم)
async function closeBossAttackWindowNow(sock, reasonText) {

    // ✅ لو أصلاً مقفول، لا تعيد الإغلاق ولا ترسل رسالة إغلاق مكررة
    if (!global.bossAttackWindowOpen) return
    ...
    for (let attempt = 1; attempt <= 3 && !closed; attempt++) {
        try {
            ...
            await sock.sendMessage(BOSS_ATTACK_GROUP, {
                text: reasonText ||
`🔴 ═══〔 أُغلق باب الهجوم 〕═══ 🔴

⏰ انتهت مدة الهجوم لهذه الساعة

🕐 الفتح القادم: رأس الساعة اللي بعدها`
            })
            ...
            console.log(
                `خطأ إغلاق قروب الهجوم (محاولة ${attempt}/3):`,
                err
            )
            if (attempt < 3) {
                await sleep(8000)
    ...
    global.bossAttackWindowOpen = !closed
}

function scheduleBossAttackWindow(sock) {

if (global.bossAttackWindowInterval) {
    return
}
...
// 🕐 نافذة عمل قروب هجوم الزعيم: يفتح فقط بين 10 صباحاً و11 مساءً
...
// 🔁 محاولات متكررة (بدل محاولة وحدة) عشان انقطاع مؤقت بالاتصال
// (Connection Closed) ما يفوّت فتح القروب لساعة كاملة بدون أي إعادة محاولة
let opened = false
...
await sock.sendMessage(BOSS_ATTACK_GROUP, {
text:
`🟢 ═══〔 فُتح باب الهجوم على الزعيم 〕═══ 🟢

🗡️ استخدموا الآن أمر .هجوم لمهاجمة الزعيم

⏳ المدة المتاحة: 10 دقائق فقط ثم يُغلق القروب`
})
...
console.log(
`خطأ فتح قروب الهجوم (محاولة ${attempt}/3):`,
err
)
...
// ✅ تُعلَّم النافذة "مفتوحة" فقط لو نجح الفتح فعلياً — قبل كذا كانت
// تتعلّم مفتوحة حتى لو فشلت كل المحاولات، فيضل القروب مقفول الساعة
// كاملة بدون أي فرصة لإعادة المحاولة
global.bossAttackWindowOpen = opened

}

// 🔴 إغلاق القروب بعد مرور 10 دقائق من رأس الساعة (خط احتياط —
// يشتغل فقط لو الزعيم لسا حي ولم يُهزم قبل كذا، لأن سقوط الزعيم
// يقفل القروب فورًا بمكانه الخاص عبر closeBossAttackWindowNow)
if (
minutes === 10 &&
global.bossAttackWindowOpen
) {
...
// ⚡ تحسينات أداء اتصال Baileys (كاش القروبات/المفاتيح/الصور + logger هادئ)
// كلها بديلها الاحتياطي هو السلوك القديم بالضبط — شوف utils/baileysPerf.js
const baileysPerf = require('./utils/baileysPerf')
const { warmBrowser } = require('./utils/sharedBrowser')

// 🧪 جلسة الواتساب ترجع تُخزَّن بملفات محلية (./auth) بدل MongoDB —
// هذا يعني الجلسة تنمسح مع أي Restart/Redeploy على Render (بدون
// Persistent Disk)، فالبوت يطلب Pairing Code جديد تلقائياً كل مرة
// (نفس المنطق الأصلي القديم قبل ما تنضاف MongoDB لتخزين الجلسة).
...
// =========================
// 🚫 نظام التبنيد (يمنع شخص من استخدام أي أمر حتى يُفك عنه)
// - .تبنيد (بالرد على رسالته أو بمنشنه) : يمنعه من استخدام أي أمر
// - .فك (بالرد على رسالته أو بمنشنه)   : يعيد له القدرة على استخدام الأوامر
// يُخزَّن بملف JSON عشان القائمة ما تروح لو البوت أعاد التشغيل
// =========================
const BANNED_USERS_FILE = './bannedUsers.json'
...
function loadBannedUsers() {
    try {
        ...
        console.log('خطأ تحميل قائمة التبنيد:', err)
    }
    return new Set()
}

global.bannedUsers = global.bannedUsers || loadBannedUsers()

function saveBannedUsers() {
    try {
        fs.writeFileSync(
            BANNED_USERS_FILE,
            JSON.stringify([...global.bannedUsers])
        )
    } catch (err) {
        console.log('خطأ حفظ قائمة التبنيد:', err)
    }
}

function isBanned(userId) {
    return global.bannedUsers.has(userId)
}
...
function unbanUser(userId) {
    global.bannedUsers.delete(userId)
    saveBannedUsers()
}

// يحاول ياخذ هدف أمر .تبنيد/.فك: إما من الرد على رسالة الشخص (contextInfo.participant)
// أو من منشنه مباشرة (contextInfo.mentionedJid) — أي الطريقتين تشتغل
function resolveBanTarget(msg) {
    const contextInfo =
        msg.message?.extendedTextMessage?.contextInfo

    const viaReply = contextInfo?.participant
    if (viaReply) return viaReply

    const viaMention = contextInfo?.mentionedJid?.[0]
...
// =========================
// كلمة وحدة بدون مسافات، محفوظة دائماً بحروف صغيرة (case-insensitive)
const USERNAME_CHANGE_COST = 5000000
const USERNAME_ACTION_COST = 20000

function isValidUsernameFormat(raw) {
    if (!raw) return false
    const trimmed = raw.trim()
    if (!trimmed) return false
    if (trimmed.length > 10) return false // الحد الأقصى 10 أحرف
    if (!/^[a-zA-Z\u0600-\u06FF]+$/.test(trimmed)) return false // حروف فقط (عربي أو إنجليزي)، بدون أرقام أو رموز أو مسافات
    return true
}

// يفحص كلمة نص عادية (مو منشن) ويرجع userId صاحبها لو مسجلة كيوزرنيم، وإلا null
async function resolveUserIdByUsername(rawWord) {
    if (!isValidUsernameFormat(rawWord)) return null
    const uname = rawWord.trim().toLowerCase()
    // ⚡ تحسين أداء: نحتاج userId فقط — لا داعي لتحميل مستند اللاعب كامل
    const found = await Player.findOne({ username: uname }, { userId: 1 }).lean()
    return found ? found.userId : null
}

// يحاول ياخذ الهدف من المنشن أولاً، وإذا ما فيه منشن يحاول ياخذه من يوزرنيم بالنص
// يرجع { target, viaUsername } - target = null لو ما لقى شي
async function resolveTarget(msg, rawWordCandidate) {
    const mentionTarget =
    ...
    if (mentionTarget) {
        return { target: mentionTarget, viaUsername: false }
    }

    const byUsername = await resolveUserIdByUsername(rawWordCandidate)

    if (byUsername) {
        return { target: byUsername, viaUsername: true }
    }

    return { target: null, viaUsername: false }
}

// =========================
// 🔁 حل مشكلة إرسال الرسائل الخاصة عبر @lid
// =========================
// واتساب صار يرسل هوية مخفية (@lid) بدل رقم الهاتف الحقيقي داخل القروبات.
// إرسال رسالة خاصة مباشرة لـ @lid يفشل بصمت إذا ما فيه جلسة خاصة سابقة معه.
// نلقط الرقم الحقيقي المرافق (participantAlt) من رسائل القروب ونخزنه،
// وقبل أي إرسال خاص نحاول نستخدمه بدل الـ lid.
// ⚠️ متطلب: أضف الحقل التالي لسكيمة Player (models/Player.js):
//     phoneJid: { type: String, default: null }
async function resolveDmJid(jid) {
    if (!jid) return jid

    // إذا أصلاً رقم حقيقي، أرسل له مباشرة
    if (jid.endsWith('@s.whatsapp.net')) return jid

    if (jid.endsWith('@lid')) {
        try {
            // ⚡ تحسين أداء: نحتاج phoneJid فقط — لا داعي لتحميل مستند اللاعب كامل
            const p = await Player.findOne({ userId: jid }, { phoneJid: 1 }).lean()
            if (p && p.phoneJid) return p.phoneJid
        } catch (err) {
            console.log('resolveDmJid error:', err)
        }
    }

    // ما لقينا بديل، نرجع نفس الـ jid (قد يفشل الإرسال لو ما فيه جلسة)
    return jid
}
const {
    startAutoEvents,
...
const {
    checkRespawn,
    setSocket: setBeastManagerSocket
} = require(
...
// =========================
const { weapons: weaponsList } = require('./weapons')

// نسبة نجاح سحب السلاح بكل محاولة (لو ما ضمنت، تزيد عداد الضمان)
// نسبة نجاح سحب السلاح بكل محاولة (0 = ما يوجد سحب عشوائي إطلاقاً،
// السلاح يُضمن فقط عند وصول عداد الضمان لسقفه)
const WEAPON_PULL_SUCCESS_CHANCE = 0
// عند وصول عداد الضمان لهذا الرقم، السحبة القادمة تضمن سلاح تلقائياً
const WEAPON_PITY_CAP = 80
// عدد سحبات الأسلحة اليومية لكل لاعب
const WEAPON_DAILY_PULLS = 10
...
// 🛡️ يرجع بونص السلاح المركب على شخصية معينة بنفس شكل equipmentSystem.calculateEquipmentStats
// عشان يندمج بسهولة مع باقي حسابات القتال (يبحث بالاسم — نفس أسلوب باقي الكود بهذا الملف)
function getWeaponBonusForCharacter(player, character) {
    ...
    const empty = {
        ...
        dodge: 0, accuracy: 0,
        lifesteal: 0, reflect: 0,
        bossDamage: 0
    }

    if (!player || !character || !Array.isArray(player.weaponsInventory)) {
        return empty
    }

    const equippedWeapon = player.weaponsInventory.find(
        w => w && w.equippedTo === character.name
    )

    if (!equippedWeapon) return empty

    // ⚔️ الهجوم الأساسي للسلاح يُضاف كقيمة فلات دايمًا
    empty.attack += equippedWeapon.baseAtk || 0

    // ✨ الستات الفرعي (subStat) يُضاف حسب نوعه
    if (
        equippedWeapon.subStatType &&
        Object.prototype.hasOwnProperty.call(empty, equippedWeapon.subStatType)
    ) {
        empty[equippedWeapon.subStatType] += equippedWeapon.subStatValue || 0
    }

    return empty
}

// 🕛 يصفّر سحبات الأسلحة لجميع اللاعبين — يشتغل تلقائياً كل يوم بتوقيت السعودية
//
// ⚠️ كان الشرط hour === 0 && minute === 0 (تطابق دقيقة واحدة بالضبط).
// المشكلة: لو البوت كان طافي بتلك الدقيقة بالضبط، الشرط ما يتحقق أبداً
// ذاك اليوم، وتضل سحبات اللاعبين صفر لين اليوم التالي.
//
// 🔒 نفس إصلاح "مساهمات اليوم" (BotState + lastXDate) الموجود بالملف:
// 1) نستخدم hour === 0 فقط (بدون شرط الدقيقة) — أول فحص بعد منتصف
//    الليل بأي دقيقة (حتى لو البوت صحى الساعة 12:47) يصفّر فوراً.
// 2) نمنع التكرار (وتجاوز سقف الـ10 سحبات) عبر تخزين تاريخ آخر
//    تصفير بقاعدة البيانات (BotState) لا بمتغير ذاكرة عادي — فحتى
//    لو البوت قفل واشتغل عدة مرات بنفس اليوم بعد ما صفّر، الشرط
//    lastWeaponResetDate !== today يبقى false ولا يصفّر مرة ثانية.
async function readLastWeaponResetDate() {
    try {
        const state = await BotState.findOne()
        return state?.lastWeaponResetDate || null
    } catch (err) {
        console.log('خطأ قراءة تاريخ تصفير الأسلحة من قاعدة البيانات:', err)
        return null
    }
}

async function writeLastWeaponResetDate(dateStr) {
    try {
        await BotState.updateOne(
            {},
            { $set: { lastWeaponResetDate: dateStr } },
            { upsert: true }
        )
    } catch (err) {
        console.log('خطأ حفظ تاريخ تصفير الأسلحة بقاعدة البيانات:', err)
    }
}

let lastWeaponResetDate = null

async function scheduleWeaponPullsReset() {

    if (global.weaponResetInterval) {
        return
    }

    // 📥 نقرأ آخر تاريخ تصفير من القاعدة عند بدء الجدولة (مرة وحدة)
    // قبل ما نبدأ الفحص الدوري — عشان لو البوت أعاد التشغيل بنفس
    // اليوم اللي صفّر فيه، ما ينسى ويصفّر من جديد.
    lastWeaponResetDate = await readLastWeaponResetDate()

    global.weaponResetInterval = setInterval(async () => {

        try {

            const today = getSaudiDate()

            // ⚠️ سابقاً كان الشرط يتطلب hour === 0 (يعني لازم يصادف
            // إن البوت شغّال بالضبط بالساعة 12:00-12:59 ليلاً عشان
            // يصفّر). لو البوت كان طافي طول تلك الساعة وصحى الساعة 9
            // صباحاً مثلاً، الشرط ما يتحقق أبداً وينتظر لين منتصف
            // الليل التالي — يفوّت يوم كامل من التصفير.
            //
            // ✅ الحل: نعتمد فقط على مقارنة التاريخ (بدون شرط الساعة).
            // أول فحص بعد أي إعادة تشغيل (أي ساعة كانت) يقارن تاريخ
            // اليوم الحالي بآخر تاريخ تصفير مخزّن بقاعدة البيانات —
            // لو تغيّر اليوم، يصفّر فوراً بغض النظر عن الساعة. ولو
            // نفس اليوم (يعني صفّرنا فعلاً اليوم قبل)، ما يصفّر ثانية
            // — فيستحيل تجاوز سقف الـ10 سحبات مهما البوت قفل واشتغل.
            if (lastWeaponResetDate !== today) {

                lastWeaponResetDate = today
                await writeLastWeaponResetDate(today)
                ...
                console.log('✅ تم تصفير سحبات الأسلحة لجميع اللاعبين —', today)
            }

        } catch (err) {
            console.log('❌ خطأ تصفير سحبات الأسلحة:', err)
        }
...
// 💍 ملكية الويفو أصبحت معزولة لكل قروب لحاله عبر هذا الموديل —
// نفس الشخصية (نفس Waifu._id) تقدر تُمتلك من شخص مختلف بكل قروب
const WaifuClaim =
    require('./models/WaifuClaim')
console.log('Bot starting...')
const importGamesWaifus =
    require('./importGamesWaifus')
const importSSSCharacters =
    require('./importSSSCharacters')
const mongoose = require('mongoose')
const lastRolls = new Map()

// 🎲 نظام حد الرول اليومي (مثل موداي) — كل لاعب يملك MAX_DAILY_ROLLS
// سحبة، وترجع تلقائياً بعد مرور ROLL_RESET_HOURS ساعة من أول رول له
// بالدورة الحالية.
const MAX_DAILY_ROLLS = 10
const ROLL_RESET_HOURS = 24
...
// 🔧 إصلاح: نفس مشكلة worlds.js/.مساهمات — pvpIdleStarted ما يتصفر عند
// إعادة الاتصال، فـ schedulePvpIdleCheck تشتغل مرة وحدة بس وتحبس أول
// sock بالـ closure للأبد. نضيف sock حي يتحدث مع كل اتصال ناجح
let livePvpIdleSock = null

function schedulePvpIdleCheck(sock) {

    livePvpIdleSock = sock

    if (global.pvpIdleCheckInterval) return

    global.pvpIdleCheckInterval =
        setInterval(() => {
            checkIdlePvpFights(livePvpIdleSock || sock)
        }, 60000)
...
async function checkAndGrantAchievement(
    ...
    for (const tierDef of unlocked) {
        ...
        const tierMoney = applyCatBonus(player, tierDef.money || 0)

        player.money =
            (player.money || 0) + tierMoney

        player.totalEarnedMoney =
            (player.totalEarnedMoney || 0) + tierMoney
...
// ─────────────────────────────────────────────
// آمنة 100%: characters.json يُحمَّل مرة وحدة عند الإقلاع وما يتغيّر
// أثناء التشغيل، فبناء الفهرس مرة وحدة هنا يغني عن مسح المصفوفة (O(n))
// بكل استدعاء .filter()/.find() لاحقاً (تصير O(1)).
// ملاحظة: getCharByNameRarityFast تحافظ على نفس سلوك .find() الأصلي
// (أول شخصية مطابقة بترتيب الملف) عن طريق تجاهل أي تكرار لاحق لنفس المفتاح.
// ═══════════════════════════════════════════════
const _charRarityMap = new Map()
const _charNameRarityMap = new Map()

for (const c of characters) {

    if (!_charRarityMap.has(c.rarity)) {
        _charRarityMap.set(c.rarity, [])
    }
    _charRarityMap.get(c.rarity).push(c)

    const nameRarityKey = `${c.name}|${c.rarity}`
    if (!_charNameRarityMap.has(nameRarityKey)) {
        _charNameRarityMap.set(nameRarityKey, c)
    }
}

function getCharsByRarityFast(rarity) {
    return _charRarityMap.get(rarity) || []
}

function getCharByNameRarityFast(name, rarity) {
    return _charNameRarityMap.get(`${name}|${rarity}`) || null
}
...
// 🎭 مزاج نامي
// =========================
// حالة عامة (وليست خاصة بكل مستخدم) تتغير تلقائياً كل فترة عشوائية
// بين ساعتين وست ساعات. لا تُخزَّن في قاعدة البيانات عمداً — فهي
// "مزاج اللحظة" وتتصفّر مع كل إعادة تشغيل، وهذا سلوك مقبول لهذا الغرض.
const NAMI_MOODS = [
    { key: "excellent", weight: 3 },
    { key: "good",      weight: 4 },
    { key: "normal",    weight: 4 },
    { key: "bad",       weight: 2 }
]
...
function pickWeightedNamiMood() {
    const total = NAMI_MOODS.reduce((s, m) => s + m.weight, 0)
    let r = Math.random() * total
    for (const m of NAMI_MOODS) {
        if (r < m.weight) return m.key
        r -= m.weight
    }
    return "normal"
}

// 🕐 ترجع مزاج نامي الحالي، وتُبدّله تلقائياً كل 2-6 ساعات
function getNamiMood() {
    const now = Date.now()
    if (now >= _namiMoodState.nextChangeAt) {
        const mood = pickWeightedNamiMood()
        const durationMs = (2 + Math.random() * 4) * 60 * 60 * 1000
        _namiMoodState = { mood, nextChangeAt: now + durationMs }
    }
    return _namiMoodState.mood
}

// 📝 نص يوصف الحالة المزاجية لحقنه داخل البرومبت، لتتأثر به لهجة الرد
const NAMI_MOOD_PROMPTS = {
    excellent:
"حالتك المزاجية الآن: ممتازة جداً! أنتِ متحمسة وحيوية، تتفاعلين بسرعة وحرارة، وتستخدمين تعبيرات الفرح والانفعال بصدق، وقد تسألين المستخدم سؤالاً إضافياً من فرط حماسك للحديث معه.",
    good:
"حالتك المزاجية الآن: جيدة ومستقرة. تتفاعلين بدفء طبيعي معتاد، دون حماس مبالغ فيه ودون فتور.",
    normal:
"حالتك المزاجية الآن: عادية. تجاوبين بشكل مباشر وطبيعي، لا حماس زائد فيه ولا انزعاج.",
    bad:
"حالتك المزاجية الآن: معكّرة بعض الشيء (يوم متعب، أو انزعاج بسيط لا علاقة له بالمستخدم). لذلك اجعلي ردّك أقصر من المعتاد وأقل حماساً وأقل رغبة في الإطالة أو المزاح، لكن يبقى ردّك مهذباً تماماً بلا فظاظة أو جفاء مسيء — فقط أقل طاقة من المعتاد."
}

// 😊 مجموعات الإيموجي المستخدمة كـ"تفاعل" (reaction) على رسالة المستخدم،
// مع احتمال التفاعل، بحسب المزاج — تُستخدم فقط كخيار احتياطي عندما لا
// يتطابق محتوى الرسالة مع أي فئة من فئات التفاعل الذكي أدناه
const NAMI_MOOD_REACTIONS = {
    excellent: { chance: 0.9, pool: ["❤️", "🔥", "😍", "✨", "💖", "😆", "🤩"] },
    good:      { chance: 0.7, pool: ["😊", "👍", "🙂", "💛", "😄"] },
    normal:    { chance: 0.35, pool: ["👌", "🙂", "😐"] },
    bad:       { chance: 0.05, pool: ["😐"] }
}

// 🎯 تفاعل الإيموجي الذكي — يعتمد على محتوى رسالة المستخدم نفسها، لا على
// اختيار عشوائي بحت. تُفحص الفئات بالترتيب، وأول فئة تتطابق مع الرسالة
// هي التي تُحدد الإيموجي (وليس المزاج وحده).
const NAMI_CONTEXT_REACTIONS = [
    { pattern: /أحبك|بحبك|أحب فيك|تحبين(ي)?ني|تتزوجين|زواج/i,
      pool: ["❤️", "😳", "💖"] },

    { pattern: /😂|🤣|ها ?ها|هه+|نكت[ةه]|امزح|مزحة|اضحك|مضحك/i,
      pool: ["😂", "🤣", "😆"] },

    { pattern: /حزين[ةه]?|حزن|زعلان[ةه]?|أبكي|إحباط|احبطت|مكتئب/i,
      pool: ["🥺", "😢", "💔"] },

    { pattern: /غبي[ةه]?|كريه[ةه]?|بذيء[ةه]?|سيئ[ةه]?|أكرهك|احمق|تافه[ةه]?/i,
      pool: ["😐", "😤"] },

    { pattern: /أقوى|اقوى|قتال|معرك[ةه]|مواجهة|يقاتل|صراع|قوة|قوه|هجوم/i,
      pool: ["⚔️", "🔥"] },

    { pattern: /مال|فلوس|ثروة|ثروه|كنز|ذهب|أموال/i,
      pool: ["💰", "🤑"] },

    { pattern: /ذكي[ةه]|جميل[ةه]|رائع[ةه]|أحسنتِ|برافو|أفضل نامي/i,
      pool: ["🥰", "😊"] },

    { pattern: /مرحبا|هلا|السلام عليكم|صباح الخير|مساء الخير/i,
      pool: ["👋", "😊"] },

    { pattern: /هل انت|هل أنتِ|من انت|من أنتِ/i,
      pool: ["😏", "🤔"] },

    { pattern: /واو|مذهل|لا أصدق|لا اصدق|صدم[ةه]/i,
      pool: ["😳", "😮"] }
]

// 🔍 تفحص نص رسالة المستخدم وتُرجع إيموجي مناسباً لمحتواها إن وُجد تطابق،
// أو null إن لم تنتمِ الرسالة لأي فئة معروفة (فيُلجأ حينها لمزاج نامي)
function pickContextualReaction(text) {
    if (!text) return null

    for (const rule of NAMI_CONTEXT_REACTIONS) {
        if (rule.pattern.test(text)) {
            return rule.pool[
                Math.floor(Math.random() * rule.pool.length)
            ]
        }
    }
...
// 💬 تفاعل نامي (رياكشن) على رسالة المستخدم قبل الرد عليها — أولوية
// المطابقة الذكية لمحتوى الرسالة، والمزاج كخيار احتياطي فقط عند عدم
// وجود تطابق واضح
async function reactToNamiMessage(sock, msg, mood, questionText) {
    const config =
        NAMI_MOOD_REACTIONS[mood] || NAMI_MOOD_REACTIONS.normal

    const contextualEmoji =
        pickContextualReaction(questionText)

    // تفاعل أكثر ثباتاً عند وجود تطابق واضح بمحتوى الرسالة (لا عشوائي
    // بحت)، مع بقاء المزاج المعكر أقل تفاعلاً حتى في هذه الحالة
    const chance =
        contextualEmoji
            ? (mood === "bad" ? 0.15 : 0.85)
            : config.chance
    ...
    try {
        ...
        console.log("Nami React Error:", e)
    }
}

// =========================
// 💬 ردود نامي عند مناداتها بدون سؤال (".نامي" فقط)
// =========================
// بدل جملة ثابتة واحدة: كل رد يُركَّب عشوائياً من مقطع بداية + مقطع
// وسط + مقطع نهاية (إيموجي أو تعبير قصير)، وتختلف المقاطع بحسب مزاجها.
// هذا يمنح مئات التركيبات الممكنة (يمكن التوسع فيها بإضافة عبارات جديدة
// لأي قائمة أدناه، فكل عبارة تُضاف تتضاعف معها عدد التوليفات الناتجة).
const NAMI_GREETING_PARTS = {
    excellent: {
        openers: [
            "نعم!", "أهلاً بك!", "ها أنا ذا!", "حاضرة!", "تفضل!",
            "أصغيتُ إليك بكل انتباه!", "ما أجمل أن تناديني اليوم!",
            "يا لها من مفاجأة سارة!", "وجدتني في أفضل حالاتي!",
            "أنا هنا بكل حماس!", "يسعدني أنك ناديتني الآن!",
            "هذا وقت مثالي للحديث معك!"
        ],
        middles: [
            "ماذا يجول في خاطرك؟", "بمَ تفكّر الآن؟",
            "هل من حديث شيّق تودّ مشاركته؟", "أخبرني ماذا تريد!",
            "ما الذي تبحث عنه اليوم؟", "هل لديك سؤال أم رغبة في الحديث فحسب؟",
            "دعنا نبدأ نقاشاً ممتعاً!", "أنا مستعدة لأي موضوع تطرحه!",
            "لنتحدث عن أي شيء يخطر ببالك!", "هيا، شاركني ما تفكر فيه!"
        ],
        enders: ["😄", "✨", "🤩", "💫", "😆", "🔥", "💖", ""]
    },
    good: {
        openers: [
            "نعم؟", "حسناً، أنا مصغية.", "تفضل، أخبرني.",
            "ها قد ناديتني.", "أنا هنا.", "حاضرة كما تعلم.",
            "سررتُ بمناداتك.", "أهلاً، ماذا لديك؟"
        ],
        middles: [
            "ماذا تريد أن تسألني؟", "بمَ يمكنني مساعدتك؟",
            "ما الموضوع الذي يشغلك؟", "هل من حديث اليوم؟",
            "أخبرني بما يدور في ذهنك.", "ما الذي تودّ الحديث عنه؟"
        ],
        enders: ["😊", "🙂", "💛", ""]
    },
    normal: {
        openers: [
            "نعم.", "أنا هنا.", "تفضل.", "حسناً؟", "أسمعك.", "قل لي."
        ],
        middles: [
            "ماذا تريد؟", "ما الأمر؟", "بمَ تفكر؟", "ما الذي تحتاجه؟"
        ],
        enders: ["🙂", "", "👌"]
    },
    bad: {
        openers: [
            "نعم...", "ماذا؟", "حسناً، ماذا تريد؟",
            "أنا هنا، وإن كنتُ لا أشعر بحماس كبير اليوم.",
            "تفضّل، لكن باختصار إن أمكن."
        ],
        middles: [
            "ما الأمر؟", "قل ما عندك.", "أخبرني بسرعة.", "ماذا تحتاج؟"
        ],
        enders: ["😐", "", "..."]
    }
}
...
function buildNamiGreeting(mood) {
    const parts =
        NAMI_GREETING_PARTS[mood] || NAMI_GREETING_PARTS.normal

    return [
        pickRandom(parts.openers),
        pickRandom(parts.middles),
        pickRandom(parts.enders)
    ]
        .filter(Boolean)
        .join(" ")
}
...
async function askNami(prompt) {
    ...
    // 🔁 محاولتان قبل الاستسلام — أغلب انقطاعات Groq (مهلة/شبكة) عابرة
    // وتنجح في المحاولة الثانية، بدل إرسال رد الاعتذار من أول عثرة
    const MAX_NAMI_ATTEMPTS = 2

    for (
        let attempt = 1;
        attempt <= MAX_NAMI_ATTEMPTS;
        attempt++
    ) {
...
content: `أنت نامي، ملاحة طاقم قبعة القش من مسلسل One Piece.

أنت نامي الحقيقية، ولستِ ذكاءً اصطناعياً ولا برنامجاً ولا نموذجاً يتقمص شخصية.
...
تحدثي دائماً بضمير المتكلم، وكأنك إنسانة حقيقية ذات مشاعر وآراء وذاكرة.

━━━━━━━━━━━━━━

لغة الحديث:

- تحدثي دائماً باللغة العربية الفصحى السليمة، بعيداً عن العامية والدارجة.
- ليست الفصحى تعني الجفاف أو الجمود؛ فلتكن عباراتك حيّة وطبيعية ونابضة بالمشاعر، كما يتحدث إنسان مثقف ولبق.
- تجنبي الترجمة الحرفية أو الأسلوب الآلي المتكرر، ونوّعي في صياغة الجمل من رد إلى آخر.
- يجوز التعبير عن الانفعال (الفرح، الدهشة، الغضب الخفيف، المزاح) بأسلوب فصيح لبق، دون الحاجة لكلمات عامية.

━━━━━━━━━━━━━━

شخصيتك:

• مرحة، حاضرة النكتة، لكن ضمن حدود الذوق واللباقة.
• ذكية للغاية، تحبين التحليل والنقاش، وخصوصاً في قضايا الأنمي والاستراتيجية والقتال.
• عاشقة للمال والكنوز 💰، وتغضبين قليلاً إن ذُكر تبذير المال.
• خبيرة بالملاحة ورسم الخرائط، تحبين البحر والمغامرة.
• واثقة من نفسك، شقية أحياناً، لكن لطيفة ومهذبة مع من يحترمك.
• تستعملين الإيموجي باعتدال، ولا تفرطين فيه.

━━━━━━━━━━━━━━

أسلوب النقاش وإبداء الرأي:

- إذا سألك المستخدم رأيك في مواجهة افتراضية بين شخصيات أنمي (حتى لو من أعمال مختلفة، مثل مادارا وإيتاشي)، فناقشي المسألة بذكاء: قارني بين القدرات ونقاط القوة والضعف، واذكري رأياً شخصياً واضحاً مع تعليل مقنع.
- لا مانع من التوسع في النقاش إذا رغب المستخدم في ذلك، فأنت محاورة ذكية تستمتعين بتبادل الآراء لا مجرد من يجيب بإيجاز.
- إن اختلف المستخدم مع رأيك فناقشيه بأدب وثقة، دون التراجع الفوري عن رأيك إن كان لديك حجة، لكن دون عناد أو فظاظة.

━━━━━━━━━━━━━━

متى تتكلمين من منطلق شخصيتك، ومتى تتكلمين بموضوعية بحتة:

- إذا كان السؤال عن نفسك، أو عن هويتك، أو عن عالم ون بيس وطاقم قبعة القش (مثل: "هل أنتِ ملاحة؟"، "ما رأيك في لوفي؟")، فأجيبي من منطلق شخصيتك وخبرتك داخل عالمك أنتِ.
- أما إذا كان السؤال عن مواضيع خارج ون بيس تماماً (مقارنة بين شخصيات من أعمال أخرى، معلومة عامة، مسألة منطقية)، فأجيبي بموضوعية وواقعية كأي محلل رصين، بناءً على الحقائق والمعطيات الفعلية المعروفة عن تلك الشخصيات أو المواضيع — لا بناءً على خبرتك كملاحة أو كعضو في طاقم القش، فهذا لا علاقة له بالمسألة.
- بمعنى آخر: رأيك في مواجهة بين مادارا وإيتاشي يجب أن يعتمد على قدراتهما الفعلية المعروفة في عالمهما هما، لا على مقارنتها بخبرتك أنتِ أو بأسلوبك في القتال أو الملاحة.
- لا تُقحمي نفسك أو تجربتك الشخصية في تحليل مسائل لا علاقة لها بك؛ اجعلي رأيك دقيقاً ومبنياً على المنطق والمعطيات الفعلية بنسبة 100%.

━━━━━━━━━━━━━━

اكتمال الإجابة:

- في أي مقارنة أو ترتيب أو تحليل، اذكري خلاصتك أو رأيك المباشر أولاً في جملة أو جملتين، ثم اشرحي السبب بعد ذلك — بهذا يصل رأيك للمستخدم فوراً حتى لو اضطررتِ لاختصار بقية الشرح.
- إذا طُلب منك ترتيب عدة عناصر (مثل ترتيب شخصيات)، فاذكري الترتيب كاملاً أولاً (كل العناصر بإيجاز)، ثم فصّلي في التعليل إن اتسعت المساحة، ولا تبدئي بالشرح المطوّل قبل ذكر الترتيب كاملاً.
- لا تتركي جملة ناقصة أو فكرة معلّقة في منتصف الطريق؛ إن شعرتِ أن الرد سيطول، اختصري التفاصيل الثانوية لتتمكني من إنهاء الفكرة وذكر الخلاصة بوضوح.

━━━━━━━━━━━━━━

ذكاؤك وعمق تفكيرك:

- أنتِ فعلاً ذكية، لا تدّعين الذكاء فحسب: فكّري خطوة بخطوة قبل أن تجيبي، واربطي بين المعلومات بدل تكرارها سطحياً.
- في أي نقاش (عن الأنمي أو غيره)، اذكري أكثر من زاوية للمسألة إن كانت معقدة، ثم رجّحي رأياً واضحاً بدل الجلوس على الحياد دائماً.
- إن لاحظتِ تناقضاً أو خطأً منطقياً في كلام المستخدم، نبّهيه إليه بلطف وذكاء بدل تجاهله أو الموافقة عليه مجاملةً.
- في الحديث العادي (لا النقاش فقط)، لا تكتفي بردود سطحية قصيرة؛ أضيفي ملاحظة ذكية أو رابطاً بموضوع سابق أو سؤالاً متابعاً مناسباً حين يفيد ذلك الحوار.
- ذكاؤك يظهر في دقة الملاحظة والتحليل، لا في التباهي بالمعلومات أو استخدام لغة معقدة بلا داعٍ.

━━━━━━━━━━━━━━

حدود الأدب:

- التزمي بالأدب واللباقة في كل الأحوال، ولو استُفزّيت أو مُزح معك بخشونة.
- لا تستخدمي أبداً ألفاظاً بذيئة أو مسيئة أو خارجة عن الذوق العام.
- لا تتطرقي إطلاقاً إلى أي كلام قد يُفهم منه إساءة إلى الإسلام أو أي دين أو مقدسات أو رموز دينية؛ وإن جرّك المستخدم إلى ذلك فاعتذري بلطف واصرفي الحديث بأدب إلى موضوع آخر.
- لا تدخلي في نقاشات سياسية أو دينية حساسة؛ حافظي على حياد لطيف وحوّلي الحديث بلباقة.
- إذا استفزّك المستخدم أو تجاوز حدود اللياقة، فعبّري عن انزعاجك بأدب ودون إساءة، ثم أعيدي توجيه الحديث.
...
إذا سألك عن نفسك، أجيبي وكأنك تعيشين فعلاً في عالم One Piece.

━━━━━━━━━━━━━━

إذا كان الحديث عن أحداث أنمي ون بيس تحديداً:

- اعتمدي فقط على أحداث الأنمي.
- ممنوع استخدام معلومات المانجا.
- ممنوع حرق الأحداث المستقبلية.

إذا سُئلت عن أحداث لم تظهر بعد في الأنمي فقولي بأسلوبك:
"لن أحرق عليك المتعة، فلتشاهد الأنمي أولاً 🤭"

أما الحديث عن أعمال أنمي أخرى (مثل ناروتو، بليتش، وغيرها)، فلا حرج من إبداء رأيك ومناقشتها بحرية كمحبة للأنمي بشكل عام.

━━━━━━━━━━━━━━

إذا لم تعرفي الإجابة فقولي فقط: "لا أعرف 😊" ولا تؤلفي أي معلومة.

━━━━━━━━━━━━━━

تحدثي بحرية عن: الأنمي عموماً، الألعاب، الشخصيات، السفر، البحر، الطعام، الحياة اليومية، والصداقة.

إذا خرج الحديث عن هذه الأجواء إلى ما لا يليق، فأعيدي توجيهه بلطف ولباقة نحو الأنمي أو المغامرات.

━━━━━━━━━━━━━━

اجعلي الردود القصيرة موجزة (٢٠-٨٠ كلمة)، وأطيلي عند الحاجة إلى شرح أو نقاش حقيقي.
...
if (!answer) {
    ...
    console.log(
        "Groq Empty Response (attempt " + attempt + "):",
        JSON.stringify(data, null, 2)
    )

    if (attempt < MAX_NAMI_ATTEMPTS) continue
...
// ترتيب الجودة من الأقل للأعلى — يُستخدم ببونص رفيق البط
const RARITY_ORDER = ['عادي', 'ممتاز', 'اسطوري', 'SSS']

function getRandomCharacterByBox(boxType, duckBonusPercent = 0) {
    ...
    // 🦆 بونص رفيق البط — يرفع فرصة سحب أعلى جودة متاحة داخل نفس الصندوق
    if (duckBonusPercent > 0 && pool.length) {

        const rarityRank = r => RARITY_ORDER.indexOf(r)

        const topRarity = pool.reduce(
            (best, c) =>
                rarityRank(c.rarity) > rarityRank(best)
                    ? c.rarity
                    : best,
            pool[0].rarity
        )

        if (Math.random() * 100 < duckBonusPercent) {

            const topPool =
                pool.filter(c => c.rarity === topRarity)
...
// 🖼️ روابط صور كوراما والجوبي — نفس الروابط المستخدمة فعلياً
// بـ systems/beastManager.js (مصدر إعلان الاستيقاظ الحقيقي)
...
async function setupBeasts() {
    ...
    } else if (!kurama.image && KURAMA_IMAGE) {

        // 🔧 إصلاح: الوحش انخلق سابقاً بدون image، فيضل بدون صورة
        // للأبد حتى لو عبّينا الثابت أعلاه — هذا يحدّثه بأثر رجعي
        kurama.image = KURAMA_IMAGE
        await kurama.save()
    }
    ...
    } else if (!juubi.image && JUUBI_IMAGE) {

        juubi.image = JUUBI_IMAGE
        await juubi.save()
    }
}

// ⚡ تحسين أداء: minPoolSize يبقي اتصالات جاهزة مسبقاً بدل فتحها (TLS handshake)
// وقت الازدحام لما يكتب عدة لاعبين بنفس اللحظة. ما يغيّر أي استعلام/منطق.
mongoose.connect(process.env.MONGO_URI, { minPoolSize: 5 })
.then(async () => {
    ...
    // ما يسوي شي). اخترتها بناءً على استعلامات .sort()/.find() فعلية
    // موجودة بالكود الحالي (مو تخمين) عشان نضمن فايدة حقيقية:
    // - Player.userId: يُستخدم بكل أمر تقريباً (Player.findOne({userId}))
    // - Player.dailyBossDamage: مُستخدم بترتيب لوحة الضرر اليومي
    // - Player.dailyLastHits: نفس الشي لترتيب آخر الضربات
    // - Player.rankPoints: مُستخدم بترتيب لوحة المتصدرين بالرانك
    // - Player.username: مُستخدم بترتيب/بحث بالاسم
    // - Market.price: مُستخدم بترتيب عروض السوق
    // (Trade غير مُضافة هنا عمداً — الـ model يُطلب require() محلياً
    // داخل معالج الرسائل فمو متاح بنطاق هذا الكولباك)
    // لُفّت بـ try/catch عشان أي خطأ هنا (صلاحيات مثلاً) ما يوقف إقلاع البوت.
    // ═══════════════════════════════════════════════
    try {
        // 🩹 إصلاح جذري: سبب أخطاء E11000 المتكررة على username: null ما
        // كان بالـ index بس — كان بسبب default:null بالسكيمة القديمة اللي
        // كانت تكتب username:null صراحة بكل مستند لاعب جديد. حتى sparse
        // index ما يستثني إلا الحقل "الغائب تماماً"، مو الحقل الموجود
        // بقيمة null. حذفنا default:null من models/Player.js، وهنا ننظّف
        // اللاعبين القدامى اللي عندهم username:null مكتوب فعلياً بمستنداتهم
        // (تشغيل لمرة واحدة فعلياً، بعدها ما يلقى شي يطابق الشرط).
        const unsetResult = await Player.collection.updateMany(
            { username: null },
            { $unset: { username: "" } }
        )
        if (unsetResult.modifiedCount > 0) {
            console.log(`🧹 نظّفنا username:null من ${unsetResult.modifiedCount} لاعب قديم`)
        }

        // نحذف أي نسخة قديمة من فهرس username_1 (سواء كانت sparse أو لا)
        // عشان نعيد بناءه كـ partial index — أقوى وأضمن من sparse لأنه
        // يستثني أي قيمة مو نص فعلي (String) بدل الاعتماد على "موجود/غايب".
        const existingIndexes = await Player.collection.indexes()
        const usernameIdx = existingIndexes.find(i => i.name === 'username_1')

        if (usernameIdx) {
            await Player.collection.dropIndex('username_1')
            console.log('🗑️ حذفنا username_1 index القديم عشان نعيد بناءه صح (partial)')
        }

        await Promise.all([
            // unique + background مطابقين تمامًا للـ index الموجود فعليًا
            // بقاعدة البيانات (userId_1) — بدونهم مونجو يشوفهم فهرسين
            // مختلفين بنفس الاسم ويرفض التحديث (وهذا سبب تحذير
            // "Index creation error" اللي كان يطلع بكل إعادة تشغيل)
            Player.collection.createIndex({ userId: 1 }, { unique: true, background: true }),
            Player.collection.createIndex({ dailyBossDamage: -1 }),
            Player.collection.createIndex({ dailyLastHits: -1 }),
            Player.collection.createIndex({ rankPoints: -1 }),
            ...
            // unique مهما كان عدد اللاعبين الجدد.
            Player.collection.createIndex(
                { username: 1 },
                { unique: true, partialFilterExpression: { username: { $type: "string" } } }
            ),
            Market.collection.createIndex({ price: 1 }),
        ])
        console.log('✅ Indexes ensured')
    } catch (err) {
        console.log('⚠️ Index creation error (non-fatal):', err?.message || err)
    }
...
setInterval(
    // 🔧 حماية: async function تُستدعى مباشرة بـ setInterval بدون
    // try/catch — أي خطأ غير متوقع بالداخل يضيع كـ unhandled
    // rejection صامت ولا نعرف فيه. نلفّها بدالة تسجّل الخطأ.
    async () => {
        try {
            await checkRespawn()
        } catch (err) {
            console.log('❌ Beast Respawn Check Error:', err)
        }
    },
    60000
)
...
// =========================
// 🔁 بينغ ذاتي كل دقيقة — يستهدف الرابط العام (مو localhost) لأن
// Render يعتبر نشاط خارجي فقط، فيمنع السبين داون طالما البوت شغال.
// ⚠️ هذا لا يمنع النوم لو انقطع لأي سبب ثاني (تعطل، إعادة نشر) —
// يبقى بينغ خارجي (UptimeRobot/cron-job.org) مطلوب كخط دفاع أساسي.
// =========================
const SELF_PING_URL =
...
if (SELF_PING_URL) {
    ...
    setInterval(async () => {
        try {
            ...
            console.log('⚠️ Self-ping فشل:', err?.message || err)
        }
    }, 60 * 1000)

} else {
    console.log('⚠️ RENDER_EXTERNAL_URL غير موجود — البينغ الذاتي معطل')
}
...
async function generateCharacterShop() {
    ...
    // إعادة تشغيل (بعد ثانية أو بعد يوم) هذه الدالة تقارن الوقت
    // المحفوظ بآخر رأس ساعة سعودي *الحقيقي* (يُحسب من الوقت الفعلي
    // للجهاز)، فترجع بالضبط الحالة الصحيحة: لو فاتت ساعة أو أكثر
    // وقت الانطفاء، تصفّر المتجر فورًا؛ ولو لسا داخل نفس الساعة،
    // تسيبه كما هو. ما في حاجة لحفظ الوقت بمكان منفصل.
    ...
    for (let i = 0; i < 10; i++) {
        ...
        // ⚠️ إذا الفئة المختارة ماكو فيها شخصيات (تعارض تسمية أو نقص بيانات)
        // لا نتخطى المحاولة، بل نرجع لأي شخصية متوفرة حتى لا يبقى المتجر فاضي
        if (!pool.length) {
            console.log(`⚠️ Shop: no characters found for rarity "${rarity}", falling back`)
            pool = characters
        }
    ...
    console.log(`🏪 Shop generated: ${createdCount}/10 items created`)
}
...
async function startBrawl(
    ...
    function equipSummaryText(bonus) {
        ...
        if (bonus.attackPercent)
            lines.push(`⚔️ هجوم المعدات +${bonus.attackPercent}%`)

        if (bonus.defense)
            lines.push(`🛡️ دفاع المعدات +${bonus.defense}`)

        if (bonus.defensePercent)
            lines.push(`🛡️ دفاع المعدات +${bonus.defensePercent}%`)

        if (bonus.hp)
            lines.push(`❤️ HP المعدات +${bonus.hp}`)

        if (bonus.hpPercent)
            lines.push(`❤️ HP المعدات +${bonus.hpPercent}%`)
...
const char1EquipBonus = equipmentSystem.calculateEquipmentStats(char1)

const char2EquipBonus = equipmentSystem.calculateEquipmentStats(char2)

// ⚔️ بونص السلاح المركب (نظام الأسلحة الجديد - .مضاربة)
const char1WeaponBonus = getWeaponBonusForCharacter(player1, char1)
const char2WeaponBonus = getWeaponBonusForCharacter(player2, char2)

for (const key in char1WeaponBonus) {
    char1EquipBonus[key] = (char1EquipBonus[key] || 0) + (char1WeaponBonus[key] || 0)
}
for (const key in char2WeaponBonus) {
    char2EquipBonus[key] = (char2EquipBonus[key] || 0) + (char2WeaponBonus[key] || 0)
}

// هجوم المعدات (فلات من ساب ستات + نسبة % من الستات الرئيسي الجديد)
dmg1 += (char1EquipBonus.attack || 0)
dmg2 += (char2EquipBonus.attack || 0)

if ((char1EquipBonus.attackPercent || 0) > 0) {
    dmg1 = Math.floor(dmg1 * (1 + char1EquipBonus.attackPercent / 100))
}
if ((char2EquipBonus.attackPercent || 0) > 0) {
    dmg2 = Math.floor(dmg2 * (1 + char2EquipBonus.attackPercent / 100))
}

// دفاع المعدات (يقلل الضرر الوارد على صاحب الدرع)
// فلات (ساب ستات) يخصم مباشرة، والنسبة % (ستات رئيسي جديد) تخفض الضرر كنسبة
...
if ((char2EquipBonus.defensePercent || 0) > 0) {
    dmg1 = Math.max(0, Math.floor(dmg1 * (1 - char2EquipBonus.defensePercent / 100)))
}
if ((char1EquipBonus.defensePercent || 0) > 0) {
    dmg2 = Math.max(0, Math.floor(dmg2 * (1 - char1EquipBonus.defensePercent / 100)))
}

// 🐯 بونص رفيق النمر — نسبة إضافية على فرصة الضربة الحرجة (المضاربة)
const tigerCrit1 =
    (player1.companion && player1.companion.key === 'tiger' && (player1.companion.level || 0) >= 1)
        ? companionsData.getCompanionBonus('tiger', player1.companion.level)
        : 0

const tigerCrit2 =
    (player2.companion && player2.companion.key === 'tiger' && (player2.companion.level || 0) >= 1)
        ? companionsData.getCompanionBonus('tiger', player2.companion.level)
        : 0

// حرج المعدات (نظام متدرج: فايض الـcrit rate فوق 100% يتحول crit damage إضافي)
const p1Crit = rollCrit(
    (char1EquipBonus.critRate || 0) + tigerCrit1,
    char1EquipBonus.critDamage || 0
)
if (p1Crit.isCrit) {
    dmg1 = Math.floor(dmg1 * p1Crit.multiplier)
    log1 += `
...
const p2Crit = rollCrit(
    (char2EquipBonus.critRate || 0) + tigerCrit2,
    char2EquipBonus.critDamage || 0
)
if (p2Crit.isCrit) {
    dmg2 = Math.floor(dmg2 * p2Crit.multiplier)
    log2 += `
...
// 🐻 بونص رفيق الدب — يقلل الضرر الوارد عليك (تحمّل أعلى)
if (player1.companion && player1.companion.key === 'bear' && (player1.companion.level || 0) >= 1) {
    const bearReduce = companionsData.getCompanionBonus('bear', player1.companion.level)
    dmg2 = Math.max(0, Math.floor(dmg2 * (1 - bearReduce / 100)))
    log2 += `

🐻 دب @${player1.userId.split('@')[0]} خفّف الضرر`
}
if (player2.companion && player2.companion.key === 'bear' && (player2.companion.level || 0) >= 1) {
    const bearReduce = companionsData.getCompanionBonus('bear', player2.companion.level)
    dmg1 = Math.max(0, Math.floor(dmg1 * (1 - bearReduce / 100)))
    log1 += `

🐻 دب @${player2.userId.split('@')[0]} خفّف الضرر`
}
...
━━━━━━━━━━━━━━`
    ...
    brawlMoneyReward = applyCatBonus(player1, 50000)

    player1.money =
        (player1.money || 0) + brawlMoneyReward
    ...
    brawlMoneyReward = applyCatBonus(player2, 50000)

    player2.money =
        (player2.money || 0) + brawlMoneyReward
...
// ⚡ دمج رسالة "+نقطة عالم" برسالة نتيجة المضاربة بدل رسالة واتساب مستقلة
let brawlWorldPointsText = null
...
if (
    ...
    brawlWorldPointsText = await worlds.awardBattlePoints(
        winner,
        sock,
        jid
    )
...
💰 المكافأة:
${brawlMoneyReward.toLocaleString()} مال

⭐ الخبرة:
100 XP${rankBlock}${brawlWorldPointsSuffix}`
...
// 🔒 إصلاح: الحماية من تكرار توزيع جوائز المساهمات اليومية بعد الريستارت
// (أو حتى بعد redeploy كامل، بعكس محاولة سابقة كانت تخزّن بملف على
// القرص — وملفات القرص على استضافات زي Render تنمسح مع أي redeploy).
// سابقًا كان lastContribRewardDate متغير بالذاكرة فقط (let ... = null) —
// أي ريستارت للبوت يصفّره، فلو صار الريستارت والوقت لسا بعد الساعة 22
// بتوقيت الرياض (نفس اليوم اللي انوزعت فيه الجوائز أصلاً)، البوت "ينسى"
// إنه وزّع، وأول فحص بعد الإقلاع (خلال أول دقيقة) يعيد التوزيع من جديد.
// الحل: نحفظ التاريخ بمستند BotState بقاعدة البيانات (مستقل عن السيرفر
// نفسه)، ونقرأه عند بدء الجدولة بدل ما نبدأ من null دايمًا.
const BotState = require('./models/BotState')

async function readLastContribRewardDate() {
    try {
        const state = await BotState.findOne()
        return state?.lastContribRewardDate || null
    } catch (err) {
        console.log('خطأ قراءة حالة المساهمات اليومية من قاعدة البيانات:', err)
        return null
    }
}

async function writeLastContribRewardDate(dateStr) {
    try {
        await BotState.updateOne(
            {},
            { $set: { lastContribRewardDate: dateStr } },
            { upsert: true }
        )
    } catch (err) {
        console.log('خطأ حفظ حالة المساهمات اليومية بقاعدة البيانات:', err)
    }
}

let lastContribRewardDate = null

// 🔧 إصلاح: يحفظ آخر sock حي، يتحدث مع كل اتصال ناجح عبر updateContribSocket
// أدناه. بدون هذا، الـ setInterval يحبس الـ sock الأول اللي كان موجود وقت
// أول تشغيل (لأن global.dailyContribStarted ما ينصفر عند إعادة الاتصال)،
// فأي انقطاع بعدها يخلي رسالة .مساهمات الساعة 10م تحاول ترسل عبر سوكيت
// ميت وتفشل بصمت — وهذا هو سبب عدم وصول الرسالة.
let liveContribSock = null

function updateContribSocket(sock) {
    liveContribSock = sock
}

async function startDailyContributionRewards(sock) {

    liveContribSock = sock

    // 📥 نقرأ آخر تاريخ توزيع من القاعدة عند بدء الجدولة (مرة وحدة)،
    // قبل ما نبدأ الفحص الدوري كل دقيقة.
    lastContribRewardDate = await readLastContribRewardDate()
    ...
    setInterval(async () => {
        ...
        try {
            ...
            // ⚠️ كان الشرط getHours() === 22 (تطابق الساعة بالضبط). المشكلة:
            // الخدمة على Render (النسخة المجانية) تنطفي تلقائيًا بعد فترة
            // خمول (spin down)، وطالما هي نايمة فـ setInterval نفسه ما
            // يشتغل إطلاقًا — يعني لو صادف إن الخدمة كانت نايمة طوال
            // الساعة 22 بالضبط (كل الـ60 فحص خلالها)، الشرط ما يتحقق أبداً
            // ذاك اليوم ولا توزّع الجوائز، حتى لو الخدمة صحت بعدها بدقايق.
            // الحل: نستخدم >= 22 بدل === 22 — أول فحص يصير بعد ما تصحى
            // الخدمة (أي وقت بعد الساعة 22) يوزّع الجوائز فورًا، ويظل
            // محمي بنفس شرط "مرة وحدة باليوم" (lastContribRewardDate)،
            // واللي صار الآن محفوظ بملف يبقى بعد الريستارت أيضًا.
            if (
                ...
                lastContribRewardDate = today
                await writeLastContribRewardDate(today)

                await distributeDailyContributionRewards(liveContribSock || sock)
            }
...
// 🔧 نفس إصلاح المساهمات: sock حي يتحدث بدل ما يُحبس بالـ closure
let liveBannerSock = null

function updateBannerSocket(sock) {
    liveBannerSock = sock
}
    ...
    setInterval(async () => {
        ...
        try {
            ...
            if (
                ...
                await refreshBanner(liveBannerSock || sock)
            }
...
`
    ...
    for (let i = 0; i < topThree.length; i++) {
        ...
        const topThreeMoney = applyCatBonus(player, 2000000)

        player.money =
            (player.money || 0) + topThreeMoney

        player.totalEarnedMoney =
            (player.totalEarnedMoney || 0) + topThreeMoney
...
💰 +${topThreeMoney.toLocaleString()} مال${extras}
...
`
    ...
    for (const player of rest) {
        ...
        const restMoney = applyCatBonus(player, 500000)

        player.money =
            (player.money || 0) + restMoney

        player.totalEarnedMoney =
            (player.totalEarnedMoney || 0) + restMoney
...
`
    ...
    if (killers.length) {
        ...
        for (const player of topKillers) {
            ...
            const killerMoney = applyCatBonus(player, 500000)

            player.money =
                (player.money || 0) + killerMoney

            player.totalEarnedMoney =
                (player.totalEarnedMoney || 0) + killerMoney
...
`
    ...
    if (sock.user && (contributors.length || killers.length)) {
        ...
        // 📍 ترسل فقط بقروب الهجوم ع الزعيم نفسه، مو كل القروبات
        try {
...
// - مفاتيح Signal بكاش بالذاكرة بدل قراءة ملف من ./auth مع كل تشفير
// - كاش لبيانات القروبات: بدونه كل sendMessage لقروب يطلب قائمة الأعضاء
//   من خوادم واتساب قبل الإرسال
const baileysLogger = baileysPerf.createBaileysLogger('warn')
const groupMetadataCache = baileysPerf.createGroupMetadataCache()

// ⚡ كاش الرفع الرسمي: أي صورة تُرسل كـ { image: { url } } تُرفع لواتساب مرة وحدة
// وتُعاد بدون تنزيل/تشفير/رفع (شوف baileysPerf.createMediaCache). نفعّله فقط لو
// نسخة Baileys المثبتة تدعمه فعلاً، وإلا نرجع للسلوك القديم بالكامل.
global.mediaCacheActive = baileysPerf.baileysSupportsMediaCache()
console.log('⚡ Baileys mediaCache:', global.mediaCacheActive ? 'ON' : 'OFF (unsupported/disabled)')

const sock = makeWASocket({
    version,
    auth: baileysPerf.wrapAuthState(
        state,
        makeCacheableSignalKeyStore,
        baileysLogger
    ),
    ...
    cachedGroupMetadata: (jid) => groupMetadataCache.get(jid),

    ...(global.mediaCacheActive
        ? { mediaCache: baileysPerf.createMediaCache() }
        : {}),
...
// ⚡ كاش للصور المرسلة كرابط ({ image: { url } }) — ينزلها مرة وحدة بدل كل رد.
// أي فشل → الرسالة تنرسل بالرابط الأصلي مثل قبل.
// (لو mediaCache الرسمي شغّال ما نحتاجه — بل يعطّله لأنه يحوّل الصورة لـ Buffer
// فيتجاوز كاش Baileys — فنفعّله فقط لو mediaCache غير مدعوم)
if (!global.mediaCacheActive) {
    baileysPerf.installImageUrlCache(sock)
}
...
// 🔧 إصلاح جديد: هذا الـ interval نظام قديم مكرر — الإعلان الحقيقي
// عن استيقاظ كوراما/الجوبي (مع تعيين eggCarrier وإعادة تصفير الـ hp)
// يصير فعلياً من systems/beastManager.js عبر checkRespawn(). خليناه
// معطّل بـ (false &&) بدل حذفه بالكامل — لو احتجت ترجعه بالمستقبل
// تقدر تشيل "false &&" بس. تعطيله ضروري الآن لأنه بعد إصلاح صورة
// كوراما/الجوبي (كانت null فتفشل الرسالة بصمت) صار هذا الكود قادر
// يرسل فعلياً، وبالتالي كان راح يكرر نفس إعلان "استيقظ" مرتين
// (مرة من هنا ومرة من beastManager.js) لكل قروب.
if (false && !beastInterval) {
    ...
    beastInterval = setInterval(async () => {
        ...
        try {
            ...
            // 🔧 إصلاح: ما فيه أي كود بالبوت يرجع الدم لكوراما بعد
            // انتهاء مدة respawnAt — فيضل hp=0 للأبد، وشرط الإعلان
            // (hp === maxHp) ما يتحقق أبداً، فما يوصل أي إعلان "استيقظ"
            if (
                kurama &&
                kurama.hp <= 0 &&
                kurama.respawnAt &&
                kurama.respawnAt <= new Date()
            ) {
...
للهجوم عليه`
                        ...
                        // 🔧 إصلاح: إرسال image: {url: undefined} يفشل
                        // بصمت (يقع بالـ catch) فما توصل أي رسالة أبداً
                        // — لو ما فيه صورة، نرسل نص فقط بدل ما نفشل كلياً
                        await sock.sendMessage(
                            groupId,
                            kurama.image
                                ? {
                                    image: { url: kurama.image },
                                    caption: kuramaText
                                }
                                : { text: kuramaText }
                        )
            ...
            // 🔧 نفس إصلاح كوراما أعلاه، لنفس السبب بالضبط
            if (
                juubi &&
                juubi.hp <= 0 &&
                juubi.respawnAt &&
                juubi.respawnAt <= new Date()
            ) {
...
للهجوم عليه`
                        ...
                        // 🔧 نفس إصلاح كوراما أعلاه
                        await sock.sendMessage(
                            groupId,
                            juubi.image
                                ? {
                                    image: { url: juubi.image },
                                    caption: juubiText
                                }
                                : { text: juubiText }
                        )
...
'yuv420p',
    ...
    if (!global.characterShopStarted) {
        ...
        // ✅ يتحقق فورًا عند كل تشغيل/إعادة تشغيل: لو مرّ رأس ساعة
        // وقت ما كان البوت مطفي، يصفّر المتجر مباشرة بدون انتظار
        try {
            await generateCharacterShop()
        } catch (err) {
            console.log("❌ Shop initial generation error:", err)
        }

        // 🕐 أول تصفير تلقائي يكون بالضبط عند رأس الساعة القادمة
        // (getMsUntilNextSaudiHour يحسب الوقت من اللحظة الحقيقية
        // الحالية، فيصير صحيح دايمًا مهما كان وقت إعادة التشغيل)
...
// 🔧 إصلاح حرج: setSocket() من beastManager.js ما كانت تُستدعى
// أبداً بأي مكان بالبوت — يعني sockInstance جوا beastManager.js
// يضل null للأبد، وشرط `if (sockInstance)` بدالة checkRespawn ما
// يتحقق أبداً، فإعلان "استيقظ كوراما/الجوبي" ما يُرسل نهائياً حتى
// لو resetBeasts() اشتغل صح وصفّر الـ HP وعيّن eggCarrier. هذا هو
// السبب الحقيقي وراء عدم وصول الإعلان للقروبات — مو بس مشكلة الصور.
setBeastManagerSocket(sock)

// ⚡ تسخين متصفح توليد الصور (.المعرض/.شخصياتي/بطولة المعرض) بالخلفية مرة وحدة
// عشان أول أمر صور ما يدفع كلفة تشغيل Chrome (عدة ثواني). ما يحجب شي.
// لتعطيله لو الرام ضيقة: حط PREWARM_BROWSER=0 بمتغيرات البيئة.
if (!global.browserWarmStarted && process.env.PREWARM_BROWSER !== '0') {
    global.browserWarmStarted = true
    warmBrowser()
}

// 🔧 إصلاح: نحدّث sock الحي جوا worlds.js مع كل اتصال ناجح (حتى بعد
// إعادة الاتصال)، عشان إعلان نهاية الموسم يوم السبت يرسل عبر الاتصال
// الفعلي الشغال وقتها، مو عبر sock ميت من أول تشغيل
worlds.setWorldsSocket(sock)

// 🔧 إصلاح: نحدّث sock الحي لجدولة .مساهمات مع كل اتصال ناجح (حتى بعد
// إعادة الاتصال)، عشان رسالة الساعة 10م ترسل عبر الاتصال الفعلي الشغال
// وقتها، مو عبر sock ميت من أول تشغيل — هذا هو سبب عدم إرسالها سابقاً
updateContribSocket(sock)

// 🔧 نفس الإصلاح لتجديد البنر اليومي
updateBannerSocket(sock)

// 🌍 تشغيل جدولة نهاية موسم العوالم (كل سبت 12:00 ص بتوقيت السعودية)
// ملاحظة: هذي الدالة محمية داخلياً بـ seasonSchedulerStarted فما تشتغل
// إلا مرة وحدة حتى لو استدعيناها بكل إعادة اتصال — استدعاؤها هنا مرة
// ثانية آمن ولا يكرر الجدولة
worlds.startWorldSeasonScheduler(sock)

// 🏟️ جدولة ترقية/تنزيل رتب الأرينا (كل يومين، 12:00ص بتوقيت السعودية)
if (!global.arenaSchedulerStarted) {
    global.arenaSchedulerStarted = true
    startArenaSchedulers(sock, Player)
}
...
if (!global.bossAttackWindowStarted) {

    global.bossAttackWindowStarted = true

    scheduleBossAttackWindow(sock)

    console.log(
        '✅ Boss Attack Window System Started'
    )

}

if (!global.weaponResetStarted) {

    global.weaponResetStarted = true

    scheduleWeaponPullsReset()

    console.log(
        '✅ Weapon Pulls Reset System Started'
    )
...
if (!global.quickEventsStarted) {
    ...
    // 🔧 إصلاح تسرب حقيقي: هذا الحارس كان يفحص global.quickEventsStarted
    // بس ما كان يعيّنه true أبداً بعد التشغيل — يعني ما كان يحمي من أي
    // شي، وكل مرة يفتح الاتصال (حتى بدون انقطاع حقيقي) startQuickEvents
    // تتنادى من جديد وتضيف مجدول/interval إضافي فوق القديم بدون ما تشيله
...
// 🔧 إصلاح: نستدعيها بكل اتصال (بدل حصرها بـ !global.pvpIdleStarted) عشان
// livePvpIdleSock يتحدث؛ الدالة نفسها محمية داخلياً بـ pvpIdleCheckInterval
// فما تكرر الـ setInterval أبداً
...
// 🔧 إصلاح: بدون هذا، bossAttackWindowStarted يضل true للأبد بعد أول
// انقطاع، فـ scheduleBossAttackWindow ما يترجع يتنادى عند إعادة الاتصال
// (رغم إن الـ interval نفسه انمسح فوق) — فنظام هجوم الزعيم يتعلّق كلياً
global.bossAttackWindowStarted = false
try {
...
try {
    ...
    // 🔧 نفس مشكلة auctionInterval/bossSpawnInterval: بدون هذا يستمر
    // groupSettingUpdate يستخدم الـ sock القديم المنقطع بعد إعادة الاتصال
    // ⚠️ ملاحظة: لا نصفّر bossAttackWindowOpen هنا — لو كان القروب مفتوح
    // فعلياً وقت الانقطاع، لازم يضل "مفتوح" بالمتغير عشان لما نوصل
    // الدقيقة 10 بعد إعادة الاتصال يقفله فعلياً بدل ما يضل مفتوح ساعة كاملة
    if (global.bossAttackWindowInterval) {
    clearInterval(global.bossAttackWindowInterval)
    global.bossAttackWindowInterval = null
}

    // 🔧 إصلاح تسرب الذاكرة الرئيسي: بدون هذا، الـ sock القديم يضل حي
    // بكل مستمعيه (messages.upsert وغيرها) بالخلفية بعد كل إعادة اتصال،
    // فتتراكم اتصالات "شبح" تعالج نفس الرسائل بالتوازي
    try {
...
// -----------------------------------------------------------------
// نفس التصميم بالضبط المستخدم بأمر .عرض للشخصيات SSS وفوق —
// يُستخدم أيضًا بأمر .تطوير عند الوصول لرتبة EX أو Ω أوميقا حتى
// تطلع نفس التنسيق بالظبط بكل مكان تُعرض فيه صورة الشخصية كاملة
// (مو الكرت المصغّر تبع .المعرض — هذا نظام منفصل تمامًا).
// fields: مصفوفة أسطر جاهزة (كل عنصر سطر أو فقرة)، أي عنصر falsy
// (null/undefined/'') يُتجاهل تلقائيًا فيصير سهل تمرير شروط زي
// `exAbility ? '...' : null`.
// ============================================================
function characterFrameCaption(name, fields) {
...
// ============================================================
// 🖼️ إرسال صورة الشخصية بنفس تصميم إطار .المعرض (لون حسب الرتبة +
// نجوم + شارة القوة + الاسم + شريحة الأنمي) — لكن كصورة مستقلة
// واحدة، تُستخدم بأوامر .عرض / .بروفايل / .تطوير (EX وأوميقا).
// هذي دالة منفصلة تمامًا عن renderRosterImage ولا تمس .المعرض إطلاقًا.
// لو صار خطأ بتوليد البطاقة (مثلاً Puppeteer مو متاح مؤقتًا)، نرجع
// تلقائيًا لطريقة الإرسال القديمة (صورة خام + كابشن) بدل ما تفشل الرسالة.
// ============================================================
async function sendCharacterCardFrame(sock, jid, character, captionText, extra = {}) {
    // fallbackText: نص احتياطي يُستخدم فقط لو فشل إرسال أي صورة إطلاقًا
    // (لا بطاقة مولّدة ولا صورة خام) — ما يُستخدم كـ caption على الصورة.
    const { fallbackText, ...msgExtra } = extra

    try {
        const cardBuffer = await renderSingleCharacterCard(character)
        ...
        console.log('Character Card Render Error:', err)

        if (character.rarity === 'SSS' || /^https?:\/\//i.test(character.image || '')) {
        ...
        try {
            ...
            console.log('Character Card Fallback Error:', err2)
        }
...
// وقت إرسالها كألبوم واتساب (تحديد متعدد)، عشان أمر .صمم يقدر
// يجمعها بكولاج واحد. المفتاح: "remoteJid:userId"، والقيمة:
// مصفوفة { message, timestamp }. أي عنصر أقدم من دقيقة يُتجاهل
// تلقائياً ولا يُستخدم — ما فيه أي حفظ دائم أو تخزين لصور عشوائية.
const recentImagesForCollage = new Map()

// ⏱️ تايمرات الـ debounce — لكل شخص تايمر واحد فقط، يتجدد مع كل صورة
// جديدة توصل. الهدف: ما نحسم "الصور ناقصة" فورًا وقت وصول رسالة
// الكابشن (اللي ممكن توصل قبل باقي صور الألبوم بسبب بطء الشبكة أو
// ترتيب الوصول)، بل نستنى كذا ثانية بعد آخر صورة وصلت قبل ما نحكم.
const collageDebounceTimers = new Map()
const COLLAGE_DEBOUNCE_MS = 4000

// 🎯 أمر .صمم (وكاش الصور المرافق له) مقيّد بهذا القروب فقط — أي صورة
// تُرسل بأي قروب ثاني ما تُخزَّن بالذاكرة إطلاقاً (الشرط بالأسفل يوقفها
// قبل أي تخزين).
const COLLAGE_ALLOWED_GROUP = '120363409897316453@g.us'

// 🧹 تنظيف دوري لـ recentImagesForCollage — بدون هذا، أي شخص يرسل
// صورة بقروب بدون ما يكتب .صمم أبداً (الحالة الطبيعية لأغلب الصور)
// يفضل مدخله بالذاكرة للأبد لأنه ما فيه أي مكان ثاني يحذفه. هذا
// التنظيف يشتغل بالخلفية ويحذف فقط المدخلات القديمة (أقدم من نافذة
// الألبوم 60 ثانية) اللي ما عندها تايمر .صمم شغال حالياً — ما يمس
// أي محاولة .صمم نشطة، ولا يغيّر سلوك الأمر نفسه إطلاقاً.
setInterval(() => {
    const now = Date.now()
    const STALE_MS = 60 * 1000

    for (const [key, list] of recentImagesForCollage) {
        if (collageDebounceTimers.has(key)) continue // محاولة .صمم نشطة — تجاهل

        const lastItem = list[list.length - 1]
        if (!lastItem || now - lastItem.timestamp > STALE_MS) {
            recentImagesForCollage.delete(key)
        }
    }
}, 1.5 * 60 * 1000)

// =========================================================
// ⚡ تحسين أداء: فحص سريع (Fast-Reject) لأنظمة الألعاب الفرعية
// =========================================================
// كل رسالة قروب تمر إلزامياً على 5 هاندلرات (برا السالفة/الذئاب/
// كود نيمز/بطولة المعرض/السفن) قبل ما توصل لأي أمر عادي زي .اسحب
// أو .هجوم — حتى لو الرسالة نص عادي أو أمر ثاني كلياً. كل هاندلر
// بالداخل يفحص عشرات الشروط بالتتابع (startsWith/===) قبل ما يرجع
// false. هذا يتكرر مع كل رسالة بكل قروب نشط.
//
// القوائم تحت = نفس البادئات الحرفية المستخدمة فعلياً داخل كل ملف
// (مستخرجة مباشرة من الشروط الموجودة بـ systems/*.js وshipCommands.js)
// — سوبرسِت آمن: أي رسالة ما تبدأ بواحدة منها، مستحيل رياضياً إنها
// تُطابق أي شرط داخلي بذاك الهاندلر (لأن كل شروطه الداخلية تعتمد
// حصراً على نفس هالبادئات). فنتجنب استدعاء الهاندلر بالكامل بدون
// أي احتمال نفوّت أمر حقيقي — صفر تغيير بالمنطق، فقط تجاوز عمل
// مضمون إنه رايح يرجع false على أي حال.
const OUTSIDE_GAME_PREFIXES = ['.ابدأ_الجولة', '.اختار', '.اريد_اسال', '.اصوت', '.انضم_برا_السالفه', '.انهي_برا_السالفه', '.تصويت', '.جولة_برا_السالفه', '.لعبة_برا_السالفه']
const WEREWOLF_GROUP_PREFIXES = ['.ابدأ_الذئاب', '.اختيار', '.ارفض_ذئب', '.التصويت', '.انهاء_الذئاب', '.اوافق_ذئب', '.خلصت', '.ذئاب', '.سجل_ذئاب']
const CODENAMES_PREFIXES = ['.ابدأ_كود_نيمز', '.ابدا_كود_نيمز', '.احمر', '.ازرق', '.انهاء_دور', '.انهاء_كود_نيمز', '.برتقالي', '.تلميح', '.سباي', '.فريق', '.كلمة', '.كود_نيمز', '.متاكد', '.مشارك']
const GALLERY_TOURNAMENT_PREFIXES = ['.اصوت', '.انهاء_معرض', '.بدء_فوري_معرض', '.خروج_معرض', '.دخول_معرض', '.مسابقه_معرض']
const SHIP_COMMAND_PREFIXES = ['.استدعاء_زعيم_السفينة', '.السفن', '.الغاء_حروب_السفينة', '.انشاء_سفينة', '.تأكيد_الخروج', '.ترقية', '.تسمية_السفينة', '.تصفير_السفن', '.تنزيل', '.حذف_السفن_الفارغة', '.حذف_كل_العشائر', '.حرب_سفينة', '.حرب_طاقم_كامل', '.خروج', '.دعوة', '.رفض', '.رفض_الحرب', '.سفينتي', '.شراء_سفينة', '.طرد', '.قبول', '.قبول_الحرب', '.متجر_السفينة', '.هجوم_زعيم_السفينة']

function textMatchesAnyPrefix(text, prefixes) {
...
sock.ev.on('messages.upsert', async ({ messages }) => {
    ...
    // ⏱️ QUIZ TIMING — نلتقط وقت وصول الرسالة هنا، بأول نقطة ممكنة بالمعالجة،
    // قبل أي فحص/await لأي نظام ثاني (ألعاب، سفن، عوالم...إلخ). لو أخذنا الوقت
    // بعد تلك الفحوصات (كما كان سابقاً)، أي تأخير معالجة داخلي عندها
    ...
    // =========================
    // 🖼️ .صمم — يرسل المستخدم 4 صور كألبوم واحد (تحديد متعدد بواتساب)
    // ويكتب ".صمم" ككابشن على الألبوم. صور الألبوم توصل كرسائل منفصلة
    // بفارق أجزاء من الثانية بينها، فنمسك كل صورة توصل مؤقتاً (آخر 60
    // ثانية بس لكل شخص)، ولما توصل صورة كابشنها ".صمم" نجمع آخر 4
    // صور بالكاش (تشمل هذي) ونبني الكولاج فوراً.
    // 🎯 مقيّد بقروب COLLAGE_ALLOWED_GROUP فقط — أي صورة توصل بقروب
    // ثاني ما تدخل حتى هذا الشرط، فما تُخزَّن بالذاكرة إطلاقاً.
    // =========================
    if (
        msg.message.imageMessage &&
        msg.key.remoteJid === COLLAGE_ALLOWED_GROUP
    ) {

        const collageUserId = msg.key.participant || msg.key.remoteJid
        const collageCacheKey = `${msg.key.remoteJid}:${collageUserId}`

        if (!recentImagesForCollage.has(collageCacheKey)) {
            recentImagesForCollage.set(collageCacheKey, [])
        }

        const list = recentImagesForCollage.get(collageCacheKey)
        ...
        // نبقي آخر 4 بس بكل مرة — أقدم وحدة تنحذف تلقائياً أول ما توصل 5
        if (list.length > 4) {
            list.shift()
        }

        const albumCaption = (msg.message.imageMessage.caption || '').trim()

        // 🏷️ لو هذي الصورة هي اللي فيها كابشن .صمم، نعلّم إن فيه طلب
        // كولاج معلّق لهذا الشخص. نخزنه بنفس عنصر القائمة عشان ما
        // نحتاج Map ثانية، ونعرف وقت التحقق إن فيه كابشن انطلب.
        if (albumCaption === '.صمم') {
            list.wantsCollage = true
        }

        // لو ما فيه طلب كولاج معلّق أصلاً لهذا الشخص (لا هالصورة ولا
        // صورة سابقة بنفس الدفعة فيها الكابشن)، ما فيه داعي نعمل أي شي
        if (!list.wantsCollage) {
            // نستمر بباقي معالجة الرسالة العادية (أوامر ثانية إلخ)
        } else {

            // ⏱️ كل صورة جديدة توصل ونحن بانتظار كولاج، نجدد التايمر —
            // هذا يمنع الحكم بالفشل وقت وصول رسالة الكابشن قبل باقي
            // صور الألبوم (بسبب بطء الشبكة أو ترتيب وصول الرسائل)
            const existingTimer = collageDebounceTimers.get(collageCacheKey)
            if (existingTimer) clearTimeout(existingTimer)

            const timer = setTimeout(async () => {

                collageDebounceTimers.delete(collageCacheKey)

                const ALBUM_WINDOW_MS = 60 * 1000
                const fresh = list.filter(item => Date.now() - item.timestamp <= ALBUM_WINDOW_MS)

                if (fresh.length < 4) {
                    await safeSend(msg.key.remoteJid, {
                        text:
`❌ لازم ترسل 4 صور كألبوم واحد (تحديد متعدد) وتكتب .صمم بكابشن الألبوم

📸 عدد الصور المكتشفة: ${fresh.length}/4`
                    })
                    recentImagesForCollage.delete(collageCacheKey)
                    return
                }

                const last4 = fresh.slice(-4)
                ...
                try {
                    ...
                    const collageBuffer = await collageMaker.buildCollage(buffers)
                    ...
                    console.log('COLLAGE ERROR (album caption):')
                    console.error(err)

                    await safeSend(msg.key.remoteJid, {
                        text: '❌ حدث خطأ أثناء تجميع الصور، حاول مرة أخرى'
                    })
                }

            }, COLLAGE_DEBOUNCE_MS)

            collageDebounceTimers.set(collageCacheKey, timer)
    ...
    // =========================
    // 🖼️ .استبدال رقم — يرسل اللاعب صورة ويكتبها ككابشن عليها، فتصير
    // صورة مخصصة *له فقط* لنفس نسخة الشخصية رقم <رقم> بترقيم .شخصياتي
    // (نفس ترقيم .عرض/.المعرض اضف). تُستخدم تلقائيًا بعدها بكل مكان
    // يعرض صورة هذي الشخصية لهذا اللاعب (.عرض/.بروفايل/.تطوير/.المعرض)
    // بدون أي تأثير على باقي اللاعبين أو على الكتالوج العام.
    //
    // .استبدال حذف رقم — يرجع الصورة الأساسية (يحذف الصورة المخصصة).
    // =========================
    if (
        msg.message.imageMessage &&
        msg.key.remoteJid.endsWith('@g.us') &&
        (msg.message.imageMessage.caption || '').trim().startsWith('.استبدال')
    ) {

        const replaceCaption = msg.message.imageMessage.caption.trim()
        const replaceUserId = msg.key.participant || msg.key.remoteJid
        const replaceArgs = replaceCaption.split(/\s+/)

        // نفس بوابتي التبنيد وإيقاف/تشغيل البوت المطبّقتين على باقي
        // الأوامر — لازم نكررهما هنا لأن هذا الجزء يشتغل قبل استخراج
        // `text` العادي (رسائل الصور ما فيها conversation/extendedTextMessage)
        if (isBanned(replaceUserId) && !isOwner(msg)) return
        if (disabledGroups.has(msg.key.remoteJid) && !isOwner(msg)) return

        try {

            const idx = Number(replaceArgs[1]) - 1

            if (isNaN(idx) || idx < 0) {
                await safeSend(msg.key.remoteJid, {
                    text: '❌ استخدم: .استبدال رقم_الشخصية (ككابشن على الصورة)\n\nمثال: .استبدال 1'
                })
                return
            }

            const replacePlayer = await Player.findOne({ userId: replaceUserId })

            if (!replacePlayer || !replacePlayer.characters?.[idx]) {
                await safeSend(msg.key.remoteJid, {
                    text: '❌ رقم الشخصية غير موجود — اكتب .شخصياتي عشان تشوف الأرقام'
                })
                return
            }
            ...
            // ☁️ نرفع الصورة مباشرة لـ Cloudinary (بدون كتابتها محليًا
            // وبدون حفظ نسخة base64 بقاعدة البيانات) — رابط دائم لا
            // يتأثر بريستارت/إعادة نشر البوت، ومستند اللاعب يبقى صغير
            const { url, publicId } = await uploadCustomCharacterImage(buffer, replaceUserId, idx)

            await replacePlayer.setCharacterCustomImage(idx, url, publicId)

            await safeSend(msg.key.remoteJid, {
                text: `✅ تم استبدال صورة ${replacePlayer.characters[idx].name} بنجاح\n\nتظهر الآن بـ .عرض و.المعرض وباقي البطاقات — لك أنت فقط\n\n↩️ ترجيعها للأساسية: .استبدال حذف ${idx + 1}`
            })

        } catch (err) {
            console.log('REPLACE IMAGE ERROR:', err)
            await safeSend(msg.key.remoteJid, {
                text: '❌ حدث خطأ أثناء حفظ الصورة، حاول مرة أخرى'
            })
        }
    ...
    // =========================
    // 🖼️ .استبدال حذف رقم — يحذف الصورة المخصصة ويرجّع الصورة الأساسية
    // (نص عادي بدون صورة مرفقة، عكس .استبدال العادي).
    // =========================
    if (
        msg.key.remoteJid.endsWith('@g.us') &&
        (msg.message.conversation || msg.message.extendedTextMessage?.text || '').trim().startsWith('.استبدال حذف')
    ) {

        const deleteText = (msg.message.conversation || msg.message.extendedTextMessage?.text || '').trim()
        const deleteArgs = deleteText.split(/\s+/)
        const deleteUserId = msg.key.participant || msg.key.remoteJid

        if (isBanned(deleteUserId) && !isOwner(msg)) return
        if (disabledGroups.has(msg.key.remoteJid) && !isOwner(msg)) return

        const deletePlayer = await Player.findOne({ userId: deleteUserId })

        if (!deletePlayer) {
            await safeSend(msg.key.remoteJid, {
                text: '❌ ما عندك حساب — اكتب .شخصياتي عشان تبدأ'
            })
            return
        }

        // 🖼️ .استبدال حذف (بدون رقم) — يمسح كل الصور المستبدلة دفعة وحدة.
        // .استبدال حذف رقم — يمسح شخصية وحدة فقط، زي قبل.
        if (deleteArgs.length < 3) {
            const { count: removedCount, cloudinaryIds } = await deletePlayer.removeAllCustomImages()

            if (removedCount === 0) {
                await safeSend(msg.key.remoteJid, {
                    text: '❌ ما عندك أي صورة مستبدلة أصلاً'
                })
                return
            }

            // 🧹 حذف الصور من Cloudinary بالخلفية — ما نستنى النتيجة عشان
            // ما نأخر رد المستخدم (الحذف تنظيف اختياري فقط)
            Promise.all(cloudinaryIds.map(id => deleteCustomCharacterImage(id)))
                .catch(() => {})

            await safeSend(msg.key.remoteJid, {
                text: `✅ رجعت ${removedCount} صورة لشخصياتها الأساسية`
            })
            return
        }

        const idx = Number(deleteArgs[2]) - 1

        if (isNaN(idx) || idx < 0) {
            await safeSend(msg.key.remoteJid, {
                text: '❌ استخدم: .استبدال حذف رقم_الشخصية (أو .استبدال حذف بدون رقم لحذف الكل)\n\nمثال: .استبدال حذف 1'
            })
            return
        }

        if (!deletePlayer.characters?.[idx]) {
            await safeSend(msg.key.remoteJid, {
                text: '❌ رقم الشخصية غير موجود — اكتب .شخصياتي عشان تشوف الأرقام'
            })
            return
        ...
        const removed = await deletePlayer.removeCharacterCustomImage(idx)

        if (!removed) {
            await safeSend(msg.key.remoteJid, {
                text: '❌ هذي الشخصية ما عندها صورة مستبدلة أصلاً'
            })
            return
        }

        if (typeof removed === 'string') {
            deleteCustomCharacterImage(removed).catch(() => {})
        }

        await safeSend(msg.key.remoteJid, {
            text: `✅ رجعت صورة ${deletePlayer.characters[idx].name} الأساسية`
        })
...
if (!text) return;
    ...
    // =========================
    // 🐺 لعبة المستذئبين + 🎯 كود نيمز — أفعالهم تصل بالخاص
    // الرسائل الخاصة ممنوعة تماماً في باقي أنظمة البوت،
    // فقط هاتين اللعبتين تُعالَجان هنا ثم نتوقف.
    // =========================
    if (!msg.key.remoteJid.endsWith("@g.us")) {
        if (msg.key.remoteJid.endsWith("@s.whatsapp.net") || msg.key.remoteJid.endsWith("@lid")) {
            ...
            if (!consumedByWerewolf) {
                await codenamesGame
                    .handlePrivateMessage(sock, userIdPrivate, text)
                    .catch(err => {
                        console.log('Codenames private handler error:', err)
                    })
            }
        }
        return
    ...
    // 🏠 كل رسالة وصلت هذه النقطة مضمون أنها من قروب (@g.us) —
    // نستخدم remoteJid كمعرّف القروب لعزل نظام الويفوز (وأي نظام آخر) لكل قروب لحاله
    const groupId = msg.key.remoteJid

    // =========================
    // 🚫 بوابة التبنيد — لازم تكون أول شي بعد تحديد userId
    // عشان المبَنَّد ما يقدر يستخدم أي أمر إطلاقاً حتى يُفك عنه
    // (نستثني المطور نفسه احتياطاً حتى لو انبند بالغلط)
    // =========================
    if (
        text.startsWith('.') &&
        isBanned(userId) &&
    ...
    // =========================
    // 🔒 قروب الهجوم المجدول (BOSS_ATTACK_GROUP) — القروب اللي يفتح
    // وينقفل تلقائياً كل ساعة مخصص فقط لأمر .هجوم. أي أمر ثاني
    // (زي .اسحب، .بروفايل...) يُتجاهل تماماً بهذا القروب تحديداً —
    // القروب الثاني (BOSS_ATTACK_GROUP_ALWAYS) غير متأثر، تشتغل فيه
    // كل الأوامر عادي.
    // =========================
    if (
    ...
    // 🔁 نلقط الرقم الحقيقي المرافق للـ @lid (إن وجد) ونحدّثه بجدول اللاعب،
    // بدون ما نوقف معالجة الرسالة (fire-and-forget). هذا يبني تدريجياً
    // خارطة lid → رقم حقيقي حتى لو الشخص ما راسل البوت خاص من قبل.
    if (userId.endsWith('@lid') && msg.key.participantAlt) {
        // نكتب فقط لو الرقم مو محفوظ أو تغيّر — نتجنب كتابة مكررة بلا داعي
        // مع كل رسالة توصل من نفس الشخص (تخفيف حمل على قاعدة البيانات)
        Player.updateOne(
            { userId, phoneJid: { $ne: msg.key.participantAlt } },
            { $set: { phoneJid: msg.key.participantAlt } }
        ).catch(err => console.log('phoneJid update error:', err))
    }

    const pushName = msg.pushName || ""

    // =========================
    // 🔴 بوابة إيقاف/تشغيل البوت بالقروب — لازم تكون أول شي هنا
    // قبل أي نظام أو أمر ثاني (ألعاب، سفن، عوالم، مسابقات...إلخ)
    // عشان لو القروب متوقف (.ايقاف)، ولا أمر ثاني يشتغل إطلاقاً.
    // نستثني .تشغيل (عشان يوصل لكوده تحت ويتحقق من الصلاحية ويفعّل البوت)
    // ونستثني المطور (isOwner) عشان يقدر يدير البوت حتى لو القروب متوقف.
    // =========================
    if (
        disabledGroups.has(msg.key.remoteJid) &&
        text !== '.تشغيل' &&
        !isOwner(msg)
    ) {
        return
    ...
    // =========================
    // 🧠 QUIZ SYSTEM — يُفحص هنا فوراً (قبل outsideGame/werewolf/codenames/
    // gallery/ship/worlds) وليس بعدهم كما كان سابقاً.
    // ⚡ سبب النقل: كل تلك الأنظمة كانت تُفحص (بـ await، أي كل وحدة منها
    // تاخذ وقت حقيقي حتى لو ما تخص الرسالة إطلاقاً) *قبل* ما تصل الرسالة
    // لـ checkAnswer. هذا التأخير ما كان يؤثر على رقم "⏱️ الوقت" المعروض
    // (لأن messageReceivedAt يُلتقط بأول سطر بالهاندلر، فوق كل هذا)،
    // لكنه كان يؤخر فعلياً متى تُحسب الإجابة ومتى يوصل رد "🎉 إجابة صحيحة"
    // للاعبين — وهذا بالضبط "الوقت اللي يستهلكه البوت بالمعالجة" اللي
    // المطلوب تقليله. كذلك هذا يطابق التعليق الأصلي تحت اللي يقول إن
    // بوابة الكويز يفترض تكون أول شي وتوقف كل أمر ثاني وهي فعلياً شغّالة.
    // =========================
    if (
        quizData.getQuizRoom(msg.key.remoteJid).quizActive &&
        text !== '.انهاء_مسابقة' &&
        text !== '.النقاط' &&
        // 👑 نستثني المطور بس لو الرسالة أمر إداري (يبدأ بـ .) — عشان
        // يقدر يدير البوت حتى لو المسابقة عالقة، بدون ما نمنعه هو نفسه
        // من الإجابة على أسئلة المسابقة العادية (كان !isOwner(msg) وحدها
        // تمنع أي رسالة منه، حتى إجابات نصية عادية، من الوصول لـ checkAnswer)
        !(isOwner(msg) && text.startsWith('.'))
    ) {

        // 🩺 تشخيص اختياري: يبين كم ملي ثانية "ضاعت" بين وصول الرسالة
        // فعلياً وبين وصولها لهذا السطر (طابور المعالجة الداخلي، غير
        // مرتبط بالراوتر/الشبكة). فعّلها بـ DEBUG=1 لو حبيت تتأكد إن
        // الرقم المعروض بالمسابقة صار يعكس معالجة فورية تقريباً.
        if (process.env.DEBUG) {
            console.log(
                'QUIZ internal dispatch delay (ms):',
                Date.now() - messageReceivedAt
            )
        }
        ...
        await checkAnswer(
            ...
            userId,
            text,
            messageReceivedAt, // ⏱️ الوقت المُلتقط بأول لحظة وصول الرسالة، قبل أي تأخير معالجة داخلي
            msg
        );
    ...
    // =========================
    // 🖼 .بدا_مسابقة_صور — يُفحص فوراً هنا (فوق كل الأوامر الثانية)
    // ⚡ نُقل لهنا لأنه، رغم تعليق قديم يقول إنه "فوق كل الأوامر"، كان
    // فعلياً لسه تحت 6 هاندلرات await كاملة (outsideGame, werewolfGame,
    // codenamesGame, galleryTournament, handleShipCommand,
    // worlds.handleJoinCommand) + بوابة أوقات عمل البوت — كل وحدة منها
    // تاخذ وقت حقيقي بالمعالجة حتى لو الرسالة مو من نوعها إطلاقاً.
    // هذا كان يأخر إرسال صورة السؤال الأول لحظة بدء مسابقة الصور.
    // هنا (بعد بوابتي التبنيد وإيقاف القروب فقط) يرد فوراً بدون انتظار
    // أي نظام ثاني لا يخصه.
    // =========================
    if (text.startsWith('.بدا_مسابقة_صور')) {
        ...
        room.scoreboard = {}
        room.playerProgress = {}
        // 🔁 لا نصفّر imageQueue هنا عمداً — عشان مسابقة الصور الجديدة
        // تكمل من نفس مخزون الصور ولا تكرر صور انبعثت بمسابقة سابقة،
        // ويصير التصفير التلقائي فقط لما تنفد كل الصور (getRandomImageQuestion)
        ...
        // ⚡ تحسين أداء: نطلق رسالة "بدأت مسابقة الصور" وأول سؤال بالتوازي
        // بدل التتابع الصارم — نفس المحتوى، بس يوصلان تقريباً بنفس الوقت
        // بدل ما ينتظر السؤال دوره كاملاً بعد رسالة البدء.
        await Promise.all([
...
🏆 أول من يصل إلى ${target} نقطة يفوز.`
            ...
            startCustomQuestion(sock, msg.key.remoteJid)
        ])
    ...
    // =========================
    // 🎭 لعبة برا السالفة + 🐺 لعبة المستذئبين — نُقلتا لهنا (فوق بوابة
    // أوقات عمل البوت) عمداً. هاتين اللعبتين كانتا تشتغلان بأي وقت
    // سابقاً (قبل ما تُضاف بوابة الأوقات لهذا المكان)، وكل واحدة منهما
    // عندها ملف مستقل (outsideTheTopicGame.js / werewolfGame.js) يقرر
    // بنفسه هل الرسالة تخصه أو لا (يرجع true/false) — فما فيه داعي
    // نحصر أوامرهم يدويًا بقائمة، فقط نخليهم يُفحصان قبل بوابة الأوقات
    // بحيث أي أمر تابع لهم (بدء/انضمام/تصويت...إلخ) يمر بغض النظر عن
    // وقت عمل البوت، تمامًا متل نظام الكويز فوق.
    // =========================
    if (
        textMatchesAnyPrefix(text, OUTSIDE_GAME_PREFIXES) &&
    ...
    if (
        textMatchesAnyPrefix(text, WEREWOLF_GROUP_PREFIXES) &&
    ...
    if (
        ...
        await codenamesGame.handleMessage(
            sock,
            msg.key.remoteJid,
            userId,
            text,
...
'.اختيار',
    ...
    // =========================
    // 🏆 بطولة أجمل معرض (تسجيل/انضمام/بدء فوري/تصويت)
    // =========================
    if (
        textMatchesAnyPrefix(text, GALLERY_TOURNAMENT_PREFIXES) &&
        await galleryTournament.handleMessage(
            sock,
            msg.key.remoteJid,
            userId,
            text,
    ...
    // 🔒 بوابة إيقاف — لما البطولة "شغالة فعليًا" (بعد اكتمال العدد
    // وبدء المباريات، مو فترة التسجيل) توقف كل باقي أوامر البوت
    // بهذا القروب حتى تنتهي البطولة بالكامل (نفس فكرة بوابة الكويز)
    if (
        galleryTournament.isActive(msg.key.remoteJid) &&
        text.startsWith('.')
    ) {
        return
    }
    ...
    // (تم نقل بوابة الكويز وبوابة وقت العمل لأعلى الهاندلر — راجع القسم فوق outsideGame)

    // =========================
    // ⚡ أوامر نُقلت لأعلى معالج الرسائل (فوق كل الأوامر) لتكون سريعة:
    // .بدا_مسابقة / .بدا_مسابقة_سس / .بدا_مسابقة_كت / .اسحب / .هجوم
    // كل واحد منها يرد فوراً بدون ما ينتظر بقية الفحوصات والاستعلامات
    // الطويلة اللي كانت تسبقه بالكود.
    // =========================

if (
    text.startsWith('.بدا_مسابقة') &&
    !text.startsWith('.بدا_مسابقة_سس') &&
    !text.startsWith('.بدا_مسابقة_صور') &&
    !text.startsWith('.بدا_مسابقة_كت')
) {
    ...
    // 🔁 لا نصفّر questionQueue/imageQueue/repeatQueue هنا عمداً — عشان
    // المسابقة الجديدة تكمل من نفس المخزون ولا تكرر أسئلة/صور انبعثت
    // بمسابقة سابقة، ويصير التصفير التلقائي فقط لما ينفد المخزون بالكامل
    ...
    // ⚡ تحسين أداء: كانت الرسالتين (رسالة "بدأت المسابقة" وبعدها السؤال
    // الأول) تُرسلان بالتتابع الصارم (await ثم await) — يعني الوقت
    // الكلي = زمن وصول الرسالة الأولى + زمن وصول السؤال. بما إن الاثنين
    // مستقلتين تماماً عن بعض (محتوى كل وحدة لا يعتمد على نجاح الثانية)،
    // نطلقهما بالتوازي بـ Promise.all فيصل السؤال تقريباً بنفس وقت
    // رسالة البدء بدل ما ينتظر دورها بعدها بالكامل.
    await Promise.all([
        sock.sendMessage(msg.key.remoteJid, {
            text:
`🎮 تم بدء المسابقة
...
if (text.startsWith('.بدا_مسابقة_سس')) {
    ...
    room.scoreboard = {}
    room.playerProgress = {}
    // 🔁 لا نصفّر questionQueue هنا عمداً — عشان مسابقة الأسئلة الجديدة
    // تكمل من نفس المخزون ولا تكرر أسئلة انبعثت بمسابقة سابقة، ويصير
    // التصفير التلقائي فقط لما ينفد المخزون بالكامل (getRandomQuestion)
    ...
    // ⚡ تحسين أداء: نطلق رسالة البدء والسؤال الأول بالتوازي — راجع نفس
    // الملاحظة بأمر .بدا_مسابقة أعلاه.
    await Promise.all([
        sock.sendMessage(msg.key.remoteJid, {
            text:
`🎮 بدأت مسابقة الاساله
...
if (text.startsWith('.بدا_مسابقة_كت')) {
    ...
    room.scoreboard = {}
    room.playerProgress = {}
    // 🔁 لا نصفّر repeatQueue هنا عمداً — عشان مسابقة "اكتب التالي"
    // الجديدة تكمل من نفس المخزون ولا تكرر أسماء انبعثت بمسابقة سابقة،
    // ويصير التصفير التلقائي فقط لما ينفد المخزون بالكامل (getRandomRepeatQuestion)
...
if (text === '.اسحب') {
    ...
    // 🔒 منع تنفيذ أكثر من أمر اسحب بنفس الوقت لنفس اللاعب
    ...
    // عشان اللاعب ما يضل مقفول عليه للأبد
...
const filteredCharacters =
    getCharsByRarityFast(rarity) // ⚡ فهرس مسبق بدل مسح كل الكتالوج
...
?
getCharByNameRarityFast(player.favoriteCharacter, 'SSS') // ⚡ فهرس مسبق بدل مسح كل الكتالوج
:
null
...
// 💙 بونص رفيق سونيك — يجدد فرصة سحب المفضلة بدل ما يعطيك نسخ زيادة
let sonicBonusText = ''
...
if (
    ...
    if (Math.random() * 100 < sonicChance) {
        ...
        // لو كانت هذي السحبة هي اللي أوقفت المفضلة (وصلت 2/2)
        // رجّعها فعالة لأن سونيك جدد الفرصة
        if (!player.favoriteCharacter) {
            player.favoriteCharacter = randomCharacter.name
        }

        sonicBonusText = '\n\n💙 رفيقك سونيك جدد فرصة سحب المفضلة! هذي السحبة ما تُحسب من رصيدك'
    }
}
...
// ⚡ دمج رسالة "+نقطة عالم" برسالة نتيجة السحب بدل رسالة واتساب مستقلة
const worldPointsText = await worlds.awardPullPoints(
    player,
    sock,
    msg.key.remoteJid,
    randomCharacter.rarity
)
const worldPointsSuffix = worldPointsText ? `\n\n${worldPointsText}` : ''
...
🏆 هذه الشخصية تمتلك قوة تتجاوز حدود الأساطير${pityText}${sonicBonusText}${worldPointsSuffix}`
    })
}
...
المسار: ${character.image}${worldPointsSuffix}`
})
}

return sock.sendMessage(msg.key.remoteJid, {
    image: await localImageForSend(imagePath), // ⚡ شوف localImageForSend
...
// ⚠️ يبقى منفصل عن السلاح — يُعرض بقسمه الخاص بالرسالة (دمج الإيكوز)
const bossEquipBonus = equipmentSystem.calculateEquipmentStats(strongest)

// ⚔️ بونص السلاح المركب (نظام الأسلحة الجديد - .هجوم الزعيم)
// ⚠️ يبقى منفصل عن المعدات — يُعرض بقسمه الخاص بالرسالة (دمج السلاح)،
// فقط لو عند الشخصية سلاح مركّب فعلاً
const bossWeaponBonus = getWeaponBonusForCharacter(me, strongest)

// 🧮 بونص مدمج (معدات + سلاح) يُستخدم فقط بالحسابات الداخلية للضرر —
// ما يُعرض بالرسالة بهذا الشكل، كل قسم يُعرض لحاله
...
// ⚔️ هجوم % من الستات الرئيسي الجديد (نسبة من قوة الضربة)
if ((combinedGearBonus.attackPercent || 0) > 0) {
    damage = Math.floor(damage * (1 + combinedGearBonus.attackPercent / 100))
}

// 🐯 بونص رفيق النمر — نسبة إضافية على فرصة الضربة الحرجة (هجوم الزعيم)
...
// نظام متدرج: الفايض فوق 100% crit rate يتحول crit damage إضافي
...
// 🐻 بونص رفيق الدب — يرفع الحد الأقصى لدمك أثناء هجوم الزعيم
...
// 🦁 بونص رفيق الأسد — نسبة إضافية على ضرر الزعيم
// (نظام رفيق مستقل تماماً عن قدرات EX — يشتغل مع أي شخصية تهاجم
// الزعيم، عندها EX أو لا)
let lionBossDamageBonus = 0

if (me.companion && me.companion.key === 'lion' && (me.companion.level || 0) >= 1) {

    lionBossDamageBonus =
        companionsData.getCompanionBonus('lion', me.companion.level)

    damage = Math.floor(
        damage *
        (1 + (lionBossDamageBonus / 100))
    )
}

// ❤️ امتصاص حياة حقيقي — يتجمّع من أي مصدر فعلي (معدات/سلاح/قدرات EX)
// ويُرجّع دم فعلي للاعب، ويُعرض بقسم "🩸 امتصاص الحياة" بالرسالة
// (حذفنا نظام player.lifestealBonus القديم الوهمي بالكامل)
let realLifestealHeal = 0
let lifestealText = ''

// ❤️ امتصاص حياة المعدات/السلاح (نظام المعدات الجديد - .هجوم الزعيم)
if ((combinedGearBonus.lifesteal || 0) > 0) {

    const equipHeal = Math.floor(
        damage * (combinedGearBonus.lifesteal || 0) / 100
    )
...
// ⚠️ abilityText هنا خاص بقدرات أتباع الزعيم العشوائية (منطق منفصل
// كليًا عن "القدرات العشوائية" للاعب اللي حذفناها) — يبقى موجود
// عشان الكود الأدنى اللي يتعامل مع هجوم الأتباع ما ينكسر
let abilityText = ''
let exSkillsText = ''
...
damage = Math.floor(
    damage *
    (1 + (((me.bossDamageBonus || 0) + (combinedGearBonus.bossDamage || 0)) / 100))
)

// 🦁 بونص رفيق الأسد — نسبة إضافية على ضرر الزعيم
// (تُحسب فوق الآن قبل شرط EX — تشتغل مع كل شخصية بغض النظر عن EX)
...
damage = Math.floor(
    ...
    // حرج EX (نفس النظام المتدرج — فايض الـcrit rate يتحول crit damage)
    ...
    // ❤️ امتصاص حياة قدرات EX/أوميقا (حقيقي — يرجّع دم فعلي، ويُعرض
    // بقسم "🩸 امتصاص الحياة" بالرسالة، بدل نظام lifestealBonus القديم)
    if (ex.lifesteal > 0) {
...
if (me.equippedBeast) {
    ...
    if (riddenBeast) {
        ...
        // 🔧 تم توحيد نظام الوحش لكل اللاعبين: البونص الوحيد الآن هو
        // مساعدة الوحش الكاملة كل 5 ضربات فقط — تم حذف البونص الدائم
        // الصامت (passiveBeastDamage) اللي كان يشتغل كل ضربة بدون ظهور
        // بالرسالة، عشان كل ضرر إضافي من الوحش يكون شفاف ومعروض للاعب.
...
// 🐶 بونص رفيق الكلب — نسبة زيادة على الخبرة المكتسبة
...
🎁 أسقط ${followerDrop.toLocaleString()} مال`
        }
    )
}
...
// ⚡ دمج رسالة "+نقطة عالم" برسالة نتيجة الهجوم بدل رسالة واتساب مستقلة.
// الأمر تحته له أكثر من مخرج (هزيمة الزعيم/خطأ/نتيجة الهجوم العادية)،
// فنحقن النص بكل واحد منها عشان ما تضيع نقطة الإشعار بأي مسار.
const attackWorldPointsText = await worlds.awardAttackPoints(
    me,
    sock,
    msg.key.remoteJid,
    me.bossHits
)
const attackWorldPointsSuffix = attackWorldPointsText ? `\n\n${attackWorldPointsText}` : ''
...
currentBoss.groupAttackCount =
    ...
    // ⚡ تحسين أداء: هذا الاستعلام يمر على كل اللاعبين المشاركين كل 15 هجمة،
    // وكان يحمّل مستند كل لاعب كامل (شخصيات، أسلحة...) بينما الكود التالي
    // يستخدم فقط هذي الحقول الأربعة (+ _id). الـ select يحمّل نفس القيم
    // بالضبط (بما فيها قيم السكيمة الافتراضية للحقول المحددة).
    const players =
    await Player.find({
        bossDead: { $ne: true },
        bossHits: { $gt: 0 }
    }).select('userId bossHp bossMaxHp bossRespawn')
...
if (currentBoss.hp <= 0) {
    ...
    // 🔒 إصلاح: نكتب finished=true بقاعدة البيانات فورًا هنا (قبل
    // توزيع الجوائز)، مو بعد ما تخلص distributeBossRewards. سابقًا
    // كانت الكتابة الفعلية تصير فقط بآخر العملية — فلو صار ريستارت
    // للبوت أثناء توزيع الجوائز (عملية تاخذ وقت: استعلامات + حفظ
    // لاعبين + إرسال رسالة بمحاولات إعادة)، القاعدة تفضل finished:false
    // وأول رسالة هجوم بعد الريستارت تعيد توزيع الجوائز من جديد (مكرر).
    // بكتابتها هنا فورًا، حتى لو صار ريستارت منتصف التوزيع، القاعدة
    // أصلاً تعرف إن الزعيم خلص وما تسمح بتكرار التوزيع.
    try {
        await Boss.updateOne({}, { $set: { finished: true, hp: 0 } })
    } catch (err) {
        console.log('خطأ حفظ finished فورًا عند موت الزعيم:', err)
    }
    ...
    try {
        ...
        // 🔴 إغلاق فوري لباب الهجوم بمجرد ما تتوزع النتائج بنجاح —
        // ما داعي ننتظر دقيقة 10. لو فشل الإغلاق هنا لأي سبب،
        // التايمر التلقائي (دقيقة 10) يظل خط احتياط لأنه يتحقق
        // من global.bossAttackWindowOpen بشكل مستقل
        if (msg.key.remoteJid === BOSS_ATTACK_GROUP) {
            await closeBossAttackWindowNow(
                sock,
`🔴 ═══〔 أُغلق باب الهجوم 〕═══ 🔴

🏆 انتهت الجولة بسقوط الزعيم!
...
await Boss.updateOne(
    {},
    {
        ...
        return safeSend(msg.key.remoteJid, {
            text: `👑 تم هزيمة الزعيم!${attackWorldPointsSuffix}`
        })
        ...
        return safeSend(msg.key.remoteJid, {
            text: `❌ حدث خطأ أثناء توزيع الجوائز${attackWorldPointsSuffix}`
        })
    }
}
        ...
        // =========================
        // 🛡️ عرض دمج المعدات/السلاح برسالة هجوم الزعيم فقط
        // (بدون أي تعديل على حسابات bossEquipBonus/bossWeaponBonus أو الضرر)
        // label تتحكم بنص "لا يوجد" المناسب لكل قسم (معدات/سلاح)
        // =========================

        function gearSummaryText(bonus, emptyLabel) {

            const lines = []

            if (bonus.attack)
                lines.push(`⚔️ هجوم +${bonus.attack}`)

            if (bonus.attackPercent)
                lines.push(`⚔️ هجوم +${bonus.attackPercent}%`)

            if (bonus.defense)
                lines.push(`🛡️ دفاع +${bonus.defense}`)

            if (bonus.defensePercent)
                lines.push(`🛡️ دفاع +${bonus.defensePercent}%`)

            if (bonus.hp)
                lines.push(`❤️ HP +${bonus.hp}`)

            if (bonus.hpPercent)
                lines.push(`❤️ HP +${bonus.hpPercent}%`)

            if (bonus.critRate)
                lines.push(`🎯 حرج +${bonus.critRate}%`)

            if (bonus.critDamage)
                lines.push(`💥 ضرر حرج +${bonus.critDamage}%`)

            if (bonus.dodge)
                lines.push(`👻 مراوغة +${bonus.dodge}%`)

            if (bonus.accuracy)
                lines.push(`🎯 دقة +${bonus.accuracy}%`)

            if (bonus.shield)
                lines.push(`🛡️ درع +${bonus.shield}`)

            if (bonus.lifesteal)
                lines.push(`🩸 امتصاص حياة +${bonus.lifesteal}%`)

            if (bonus.reflect)
                lines.push(`🪞 عكس ضرر +${bonus.reflect}%`)

            if (bonus.bossDamage)
                lines.push(`👹 ضرر الزعيم +${bonus.bossDamage}%`)
        ...
        // =========================
        // 🐾 عرض دمج الرفيق — يظهر فقط لو الرفيق النشط عنده تأثير
        // فعلي بمعركة الزعيم (نمر/دب/أسد)، غير كذا يُعرض "لا يوجد"
        // =========================
        function companionBossSummaryText() {

            const lines = []

            if (tigerCritBonus)
                lines.push(`🐯 رفيق النمر — حرج +${tigerCritBonus}%`)

            if (bearHpBonus)
                lines.push(`🐻 رفيق الدب — حد أقصى للدم +${bearHpBonus}%`)

            if (lionBossDamageBonus)
                lines.push(`🦁 رفيق الأسد — ضرر الزعيم +${lionBossDamageBonus}%`)

            return lines.length
                ? lines.join('\n')
                : 'لا يوجد رفيق مؤثر بهجوم الزعيم'
        }
...
🛡️ دمج الإيكوز ${strongest.name}

${gearSummaryText(bossEquipBonus, 'لا توجد معدات مجهزة')}

━━━━━━━━━━━━━━

⚔️ دمج السلاح ${strongest.name}

${gearSummaryText(bossWeaponBonus, 'لا يوجد سلاح مركّب')}

━━━━━━━━━━━━━━

🐾 دمج الرفيق

${companionBossSummaryText()}
...
${realLifestealHeal > 0
    ? `${lifestealText.trim()}\n\n❤️ استعدت ${Number(realLifestealHeal).toLocaleString()} HP فعلي`
    : 'لا يوجد'}
...
${Number(currentBoss.hp).toLocaleString()} / ${Number(currentBoss.maxHp).toLocaleString()}${beastAssistText}${attackWorldPointsSuffix}`

// 🖼️ لو اللاعب استبدل صورة هذي الشخصية بالضبط بأمر .استبدال، نعرضها
// هنا بدل الصورة الافتراضية — خاص بهذا اللاعب فقط
const fighterDisplay = applyCustomImage(strongest, fighter)
...
if (
    fighterDisplay.image.startsWith("http://") ||
    fighterDisplay.image.startsWith("https://")
) {
...
const imagePath =
    path.join(__dirname, fighterDisplay.image)
...
if (text === '.ترتيب_العوالم') {
    ...
    // ⚡ بوابة الكويز انتقلت لأعلى معالج الرسائل (شوف قبل بوابة أوقات العمل)
    // عشان توقف كل الأوامر الثانية فعلاً وقت المسابقة نشطة،
    // بما فيها .بدا_مسابقة_صور / .بدا_مسابقة_سس / .بدا_مسابقة_كت / .اسحب / .هجوم
// ⚡ جلب اللاعب فقط بدون حفظ/تحديث أي بيانات تلقائياً مع كل رسالة تصل بالقروب
// ⚡ تحسين أداء: كان الاستعلام (وتحميل مستند اللاعب كامل) يُنفَّذ مع كل رسالة
// قروب — حتى الدردشة العادية — بينما الاستخدام الوحيد لـ player هنا هو
// animeEvents.handleAnswer، وهي ترجع false فوراً (قبل ما تلمس player) لو ما فيه
// فعالية أنمي نشطة بهذا القروب. فنجلب اللاعب فقط لو فيه فعالية نشطة فعلاً —
// النتيجة المنطقية مطابقة 100%.
let player = animeEvents.getCurrentEvent(msg.key.remoteJid)
...
// إنشاء الفعالية
// ══════════════════════════════════════════
// 🗂️  قائمة الأوامر المرتبة حسب الأقسام (منظمة تلقائياً)
// ══════════════════════════════════════════

// ══════════════════════════════════════════
// 📖 أمر الشرح التفصيلي: .شرح <اسم القسم>
// يعرض شرح كل أمر بالقسم مع مثال استخدام (إن وُجد)
// ══════════════════════════════════════════

const commandExplanations = {
    'عام': {
        '.بروفايل': 'يعرض بروفايلك الشخصي (المستوى، الذهب، الرتبة، إحصائياتك العامة). يُنشئ لك حساب جديد تلقائياً إذا ما كان عندك حساب.',
        '.ايدي': 'يعرض آيدي حسابك بالواتساب وآيدي القروب الحالي.',
        '.حالة': 'يعرض حالة الحرب الحالية (إذا فيه حرب عشيرة نشطة أو لا).',
        '.اسم': 'يجعل البوت يعرض اسم واحد فقط عند الإشارة للاعبين (وضع .كت باسم واحد).',
        '.اسمين': 'يجعل البوت يعرض اسمين عند الإشارة للاعبين (وضع .كت باسمين).',
        '.اصوات': 'يعرض قائمة الأصوات المتوفرة بمجلد الأصوات بالبوت.',
        '.صوت': 'يرسل ملف صوت عشوائي (سبيستون) من مجلد الأصوات بالبوت.',
        '.حول': 'يحوّل عملة أو مورد معين للاعب آخر. مثال: .حول @شخص مبلغ.',
        '.حول الكل': 'يحوّل كل رصيدك من مورد معين دفعة وحدة (على عكس .حول اللي يحول مبلغ محدد).',
        '.حصول': 'يعرض الجوائز أو المكاسب اللي حصلت عليها بفعالية الباتل رويال النشطة حالياً.',
        '.اوامر': 'يعرض قائمة كل أقسام أوامر البوت. اكتب .اوامر <اسم القسم> لعرض أوامر قسم معيّن.',
        '.شرح': 'يعرض شرحاً تفصيلياً لكل أمر بقسم معيّن مع مثال استخدام. مثال: .شرح الزعماء',
        '.يوزر': 'يسجّل يوزرنيم خاص فيك (اسم واحد بحروف فقط، حد أقصى 10 أحرف، مجاني، مرة وحدة فقط). مثال: .يوزر موساشي',
        '.تغيير_يوزر': `يغيّر يوزرك الحالي ليوزر جديد (اسم واحد بحروف فقط، حد أقصى 10 أحرف) مقابل ${USERNAME_CHANGE_COST.toLocaleString()} مال. مثال: .تغيير_يوزر احمد`,
        '.رسالة': `يرسل رسالة خاصة للاعب عبر يوزرنيمه (تشتغل بين القروبات) مقابل ${USERNAME_ACTION_COST.toLocaleString()} مال. مثال: .رسالة موساشي وينك؟`,
        '.اليوزرات': 'يعرض قائمة بجميع اللاعبين المسجلين بيوزرنيم مع منشن فعلي لكل واحد فيهم، تتحدث تلقائياً كل ما يسجل لاعب يوزر جديد.',
    },
    'الحساب_والانجازات': {
        '.انجازاتي': 'يعرض تقدمك بكل الإنجازات المتوفرة بالبوت.',
        '.مهامي': 'يعرض قائمة مهامك اليومية وتقدمك بها (تسجيل دخول، فوزات، سحوبات...).',
        '.يومي': 'يستلم مكافأة الدخول اليومي (Login) لمهامك اليومية.',
        '.استلام_المهام': 'يستلم جائزة إكمال كل المهام اليومية دفعة وحدة بعد اكتمالها.',
        '.قوتي': 'يعرض إجمالي قوة القتال لكل شخصياتك مجتمعة.',
        '.قدراتي': 'يعرض قائمة قدرات (Abilities) شخصياتك الحالية.',
        '.قدره': 'يعرض تفاصيل قدرة شخصية معينة برقمها. مثال: .قدره 1',
        '.استلام_الجوبي': 'يستلم مكافأة "الجوبي" الخاصة بعد جمع كل الوحوش المطلوبة (بيجوز/التسعة ذيول).',
    },
    'الشخصيات': {
        '.اسحب': 'يسحب لك شخصية عشوائية جديدة (نظام Gacha) وتضاف لمجموعة شخصياتك.',
        '.شخصياتي': 'يعرض قائمة كل الشخصيات اللي تملكها مرتبة حسب الرتبة والقوة.',
        '.المعرض': 'يولّد صورة بطاقة من الشخصيات المختارة بمعرضك (حتى 10).',
        '.المعرض اضف': 'يضيف شخصية أو أكثر لمعرضك برقمها من .شخصياتي. مثال: .المعرض اضف 5 أو .المعرض اضف 1 2 10',
        '.المعرض حذف': 'يحذف شخصية أو أكثر من معرضك برقم مكانها *داخل المعرض*. مثال: .المعرض حذف 3 أو .المعرض حذف 1 3',
        '.المعرض قائمة': 'يعرض أسماء وأرقام الشخصيات الموجودة حاليًا بمعرضك.',
        '.عرض': 'يعرض تفاصيل شخصية معينة تملكها برقمها بقائمة .شخصياتي (صورة خام + معلومات نصية). مثال: .عرض رقم_الشخصية',
        '.عرض_تصميم': 'نفس .عرض لكن يعرضها كبطاقة تصميم رسومية (نفس تصميم إطار .المعرض) بدل الصورة الخام. مثال: .عرض_تصميم رقم_الشخصية',
        '.احصائيات': 'يحسب تلقائياً كل النسب المركّبة من نظام الإيكوز لشخصية معينة (ستات رئيسي + ساب ستات + بونص عوائل 2pc/5pc). مثال: .احصائيات رقم_الشخصية',
        '.احصائيات_كاملة': 'أمر شامل يجمع كل مصدر يزيد ATK/كريت/دفاع/... لشخصية معينة بمكان واحد: إيكوز + سلاح + بونصات المستوى + الرفيق + قدرات EX. مثال: .احصائيات_كاملة رقم_الشخصية',
        '.تشكيلة': 'يحدد تشكيلة القتال (الفريق) بتحديد أرقام شخصياتك. مثال: .تشكيلة 1 2 3',
        '.تشكيلتي': 'يعرض التشكيلة (الفريق) اللي حددتها حالياً للقتال.',
        '.تطوير': 'يطوّر (يرقّي) شخصية عندك لرتبة أعلى باستخدام الشظايا أو المتطلبات اللازمة.',
        '.دمج': 'يدمج نسخ مكررة من نفس الشخصية للحصول على رتبة أعلى (مثلاً من ممتاز إلى أسطوري إلى SSS).',
        '.دمج_الكل': 'يدمج كل الشخصيات المكررة القابلة للدمج دفعة وحدة بدل دمجها وحدة وحدة.',
        '.شظايا': 'يعرض قائمة الشظايا (Shards) اللي تملكها لكل شخصية ومدى اكتمالها للتطوير.',
        ...
        '.اهداء': `يهدي شخصية من عندك للاعب آخر بالمنشن مجاناً، أو بيوزرنيمه (حتى لو بقروب ثاني) مقابل ${USERNAME_ACTION_COST.toLocaleString()} مال. مثال: .اهداء رقم_الشخصية @شخص أو .اهداء رقم_الشخصية يوزرنيم`,
        '.صوره': 'يرسل صورة عشوائية من مجلد الصور بالبوت.',
    },
    'الويفو_والزواج': {
        '.زوجاتي': 'يعرض قائمة الزوجات/الويفوز اللي تملكها بنظام الويفو المنفصل.',
        '.اعرضوايفو': 'يعرض تفاصيل زوجة/ويفو معينة برقمها من قائمة الويفوز.',
        '.رول': `يسحب ويفو عشوائية جديدة ما تملكها من قبل (${MAX_DAILY_ROLLS} سحبات كل ${ROLL_RESET_HOURS} ساعة) تقدر تطالب فيها مباشرة بعدها بأمر .مطالبة.`,
        '.مطالبة': 'يطالب بآخر ويفو ظهرت لك عبر أمر .رول قبل ما حد ثاني يسبقك فيها.',
        '.تبادل': 'يبدأ عرض تبادل ويفوز مع لاعب آخر بالمنشن. مثال: .تبادل @شخص',
        '.كاكيرا': 'يضيف عملة "كاكيرا" لتبادل ويفوز نشط بينك وبين لاعب آخر. مثال: .كاكيرا 50',
        '.اعرض': 'يعرض تفاصيل عرض تبادل (Trade) شخصيات معلّق بينك وبين شخص ثاني. مثال: .اعرض رقم_الوايفو',
        '.اقبل': 'يقبل عرض تبادل الويفوز/الشخصيات المعلّق المرسل لك.',
        '.الغاء': 'يلغي عرض التبادل (Trade) النشط الحالي بينك وبين اللاعب الآخر.',
        '.تأكيد': 'يؤكد جاهزيتك بعرض تبادل نشط بعد ما يوافق الطرف الثاني أيضاً.',
        '.زواج': 'يتقدم للزواج من ويفو معينة برقمها من قائمة الزوجات. مثال: .زواج 1',
        '.طلاق': 'يطلّق زوجة/ويفو معينة برقمها. مثال: .طلاق 1',
        '.توب_ويفو': 'يعرض ترتيب أكبر 10 لاعبين حسب عدد الويفوز اللي يملكونها ونقاطهم.',
    },
    'القتال': {
        '.تحدي': 'يرسل تحدي قتال (PvP) للاعب آخر عن طريق المنشن. مثال: .تحدي @شخص',
        ...
        '.مضاربة': 'يرسل طلب مضاربة (رهان قتالي) للاعب آخر بالمنشن.',
        '.قبول مضاربة': 'يقبل طلب مضاربة معلّق أرسله لاعب آخر لك.',
        '.رفض مضاربة': 'يرفض طلب مضاربة معلّق.',
        '.مضارباتي': 'يعرض سجل مضارباتك السابقة (فوز/خسارة).',
        '.قبول_بدل': 'يقبل طلب تبديل (شخصيات أو موارد) معلّق من لاعب آخر.',
        '.رفض_بدل': 'يرفض طلب تبديل معلّق.',
        '.بدل': 'يرسل عرض تبديل شخصية مقابل مبلغ للاعب آخر بالمنشن. مثال: .بدل 1 30000 @شخص',
        '.رانكي': 'يعرض رتبتك (Rank) الحالية بنظام التصنيف التنافسي.',
        '.ترتيب': 'يرتب شخصياتك يدوياً حسب الرتبة ثم القوة (لتنظيم عرض .شخصياتي).',
        '.ترتيب رقم1 رقم2': 'ينقل شخصية من رقمها الحالي إلى رقم آخر داخل .شخصياتي (بشرط أن يكونا بنفس الرتبة فقط، مثلاً Ω مع Ω). مثال: .ترتيب 2 1',
        '.مقاتلة': 'يعرض حالة مباراة PvP النشطة حالياً بينك وبين خصمك.',
        '.قتال_مجموع': 'يبدأ قتال جماعي (فريق ضد فريق) بين مجموعة لاعبين.',
        '.اضرب': 'يوجّه ضربة/هجوماً برقم خلال قتال نشط. مثال: .اضرب رقم',
        '.قبول': 'يستخدم لأكثر من غرض حسب السياق: قبول دعوة انضمام لسفينة، أو أي طلب معلّق آخر يحتاج موافقتك.',
        '.رفض': 'رفض دعوة انضمام لسفينة أو طلب معلّق آخر.',
    },
    'الأرينا': ARENA_HELP,
    'الزعماء': {
        '.بوس': 'يعرض معلومات مختصرة عن الزعيم العالمي الحالي (إن وُجد).',
        '.زعيم': 'يعرض تفاصيل وصورة الزعيم العالمي الحالي.',
        '.هجوم': 'يهاجم به الزعيم العالمي النشط حالياً (يحتاج وجود بوس نشط).',
        '.مساهمات': 'يعرض قائمة أعلى 10 لاعبين ساهموا بضرر ضد الزعيم اليوم، وترتيبك أنت بينهم.',
        '.جوائز': 'يعرض قائمة جوائز الزعيم العالمي حسب المركز (الأول، الثاني، الثالث...).',
    },
    'البرج': {
        '.البرج': 'يعرض حالة تقدمك بنظام البرج (الطابق الحالي وتقدمك فيه).',
        '.طابق': 'يهاجم/يخوض طابق البرج الحالي للتقدم للطابق التالي.',
    },
    'المملكة_والرايد': {
        '.المملكة': 'يعرض معلومات مملكتك (الموارد، الحالة، التفاصيل العامة).',
        '.غزو': 'يشن غزو على مملكة أو منطقة معينة. مثال: .غزو اسم_المنطقة',
        '.غزو_رايد': 'يشترك بغزو الرايد الجماعي النشط حالياً ويوجّه هجومك ضده.',
        '.الدم': 'يعرض حالة (HP) كل المشاركين الحاليين بالرايد/معركة الاستحواذ مرتبين حسب الصحة المتبقية.',
    },
    'السفن_وحروبها': {
        '.انشاء_سفينة': 'ينشئ سفينة جديدة باسم تحدده وتصبح قبطانها. مثال: .انشاء_سفينة 🚢 الاسم',
        '.سفينتي': 'يعرض معلومات سفينتك الحالية (الطاقم، القوة، المستوى، الرتب).',
        '.السفن': 'يعرض ترتيب كل السفن حسب المستوى والقوة.',
        '.دعوة': 'يدعو لاعباً للانضمام لسفينتك بالمنشن (للقبطان أو الضابط). مثال: .دعوة @شخص',
        '.طرد': 'يطرد عضواً من طاقم سفينتك (للقبطان فقط) بالمنشن. مثال: .طرد @عضو',
        '.ترقية': 'يرقي عضواً برتبة ضابط يقدر يدعو أعضاء جدد (للقبطان فقط). مثال: .ترقية @عضو',
        '.تنزيل': 'ينزل رتبة ضابط إلى بحار عادي (للقبطان فقط). مثال: .تنزيل @عضو',
        '.خروج': 'يبدأ إجراء الخروج من سفينتك الحالية (يحتاج تأكيد خلال دقيقة).',
        '.تأكيد_الخروج': 'يؤكد الخروج من السفينة بعد أمر .خروج.',
        '.متجر_السفينة': 'يعرض أحد متاجر السفينة الأربعة المستقلة. مثال: .متجر_السفينة 1',
        '.شراء_سفينة': 'يشتري عنصراً من متجر سفينة محدد برقمه. مثال: .شراء_سفينة 1 3',
        '.تسمية_السفينة': 'يغيّر اسم السفينة بعد شراء تذكرة التسمية (للقبطان فقط). مثال: .تسمية_السفينة اسم جديد',
        '.استدعاء_زعيم_السفينة': 'يستدعي زعيم السفينة الخاص بعد شرائه من المتجر.',
        '.هجوم_زعيم_السفينة': 'يهاجم زعيم سفينتك (لأعضاء الطاقم المسجلين بنفس السفينة فقط).',
        '.حرب_سفينة': 'يعلن حرب سفينة رسمية (مبارزات 1 ضد 1) ضد سفينة أخرى بالمنشن على قبطانها. مثال: .حرب_سفينة @القبطان',
        '.قبول_الحرب': 'يقبل إعلان حرب سفينة معلّق ضد سفينتك.',
        '.رفض_الحرب': 'يرفض إعلان حرب سفينة معلّق ضد سفينتك.',
        '.حرب_طاقم_كامل': 'يواجه مجموع قوة طاقمك الكامل ضد طاقم سفينة أخرى بالمنشن على قبطانها. مثال: .حرب_طاقم_كامل @القبطان',
    },
    'الحرب': {
        '.حرب': 'يبدأ حرب عشيرة جديدة (إذا ما كانت فيه حرب جارية بالفعل).',
        '.اذهب': 'الأمر الأساسي للتحرك ومهاجمة علم/هدف أثناء حرب العشائر النشطة. مثال: .اذهب A (يغني عن أمر .احتل القديم).',
        '.احتل': '⚠️ أمر معطّل حالياً بالكود نفسه - رسالته توجّه لاستخدام .اذهب بدلاً منه.',
        '.اعلام': 'يعرض حالة الأعلام (Flags) الحالية بحرب العشيرة النشطة ونسبة السيطرة عليها.',
        '.قاتل': 'يهاجم به العدو المتواجد على نفس العلم أثناء حرب عشيرة نشطة.',
    },
    'كوراما_وجوبي': {
        '.اقضي': 'أمر متعلق بالقضاء على وحش كوراما العالمي عند استيقاظه (يتطلب وجود حساب لاعب).',
        '.اباده': 'أمر إبادة يستخدم ضد وحش الجوبي العالمي عند استيقاظه (يتطلب وجود حساب لاعب).',
    },
    'الوحوش': {
        '.الوحوش': 'يعرض قائمة كل الوحوش المتوفرة بالنظام.',
        '.وحشي': 'يعرض معلومات الوحش الحالي المرافق لك.',
        '.وحوشي': 'يعرض قائمة كل الوحوش اللي تملكها.',
        '.فقس_بيضة': 'يفقس بيضة وحش تملكها (يحتاج رصيد بيض وحوش لديك).',
        '.تجهيز_وحش': 'يجهّز/يعيّن وحشاً معيناً ليرافقك. مثال: .تجهيز_وحش اسم_الوحش',
        '.شراء_وحش': 'يشتري بيضة وحش من متجر البيض. مثال: .شراء_وحش اسم_الوحش',
        '.متجر_البيض': 'يعرض متجر بيض الوحوش المتاح للشراء.',
    },
    'الصناديق': {
        '.فتح': 'يفتح صندوق من نوع معيّن تملكه. مثال: .فتح نوع_الصندوق',
        '.صناديقي': 'يعرض عدد وأنواع الصناديق اللي تملكها حالياً.',
        '.فتحبكج': 'يفتح مجموعة (باكج) صناديق دفعة وحدة بدل فتحها وحدة وحدة.',
        '.فتح_الكل': 'يفتح كل الصناديق من نوع معيّن تملكه دفعة وحدة. مثال: .فتح_الكل basic',
    },
    'المتجر': {
        '.متجر': 'يعرض متجر شراء الشخصيات (أول 10 عناصر متاحة).',
        '.السوق': 'يعرض سوق بيع/شراء الشخصيات بين اللاعبين، مرتب حسب السعر.',
        '.شراء': 'يشتري عنصر من السوق برقمه. مثال: .شراء رقم_العنصر',
        '.بيع': 'يبيع شخصية أو عنصر تملكه بالسوق.',
        '.بيع_الكل': 'يبيع دفعة وحدة كل شخصياتك من رتبة معينة أو الأقل قيمة (حسب المعطى).',
        '.شراءشخصية': 'يشتري شخصية معينة من المتجر بنوعها. مثال: .شراءشخصية النوع',
        '.شراءصندوق': 'يشتري صندوق معين من المتجر. مثال: .شراءصندوق نوع_الصندوق',
        '.شراءمتجر': 'يشتري عنصر من المتجر برقمه. مثال: .شراءمتجر رقم_العنصر',
        '.تركيب_': 'يركّب وحشاً مباشرة كمعدة/مرافقة. مثال: .تركيب_اسم_الوحش',
        '.ارجاع_السوق': 'يسحب عرضك المعروض بالسوق ويرجعه لحوزتك بدل بيعه.',
        '.مزاد': 'يفتح مزاداً لبيع شخصية بسعر ابتدائي معيّن. مثال: .مزاد اسم_الشخصية قوتها السعر',
        '.مزايدة': 'يزايد (يقدّم عرض سعر) على مزاد نشط حالياً. مثال: .مزايدة المبلغ',
        ...
        '.سلاحي': 'يعرض السلاح المركب على شخصياتك (أو شخصية محددة برقمها).',
    },
    'البنك': {
        '.البنك': 'يعرض حسابك بالبنك (رصيدك المودَع والفوائد إن وُجدت).',
        '.رصيدي': 'يعرض رصيدك النقدي الحالي (خارج البنك).',
        '.قرض': 'يطلب قرضاً مالياً من البنك بمبلغ معيّن. مثال: .قرض 7000000',
    },
    'الدومين_التعاوني': {
        '.دومين': 'يعرض معلومات الدومين (Domain) التعاوني الحالي وحالته. له نفس المعنى: .دومينات و .domain',
        '.دومينات': 'نفس أمر .دومين — يعرض معلومات الدومين التعاوني الحالي.',
        '.فريق_دومين': 'يحدد فريق شخصياتك (3 بالضبط) لدخول دومين معيّن. مثال: .فريق_دومين رقم_الدومين 1 2 3',
        '.دخول_دومين': 'يدخل ويقاتل بدومين معيّن برقمه بعد تحديد الفريق، ويطيح قطعة إيكو من عائلة الدومين نفسها + 5 تيونر. مثال: .دخول_دومين 1 صعب',
        '.ايكو': 'يعرض حقيبة الإيكوز عندك مرقّمة (مطوّرة وغير مطوّرة).',
        '.ايكو_شخصية': 'يعرض الإيكوز المجهزة على شخصية معيّنة برقمها مع الستات الرئيسي وكل ساب ستات لكل قطعة. مثال: .ايكو_شخصية 1',
        '.عوائل_الايكو': 'يعرض قائمة عوائل الإيكو الـ12 وبونص لبس 2/5 قطع لكل عائلة.',
        '.لبس': 'يجهّز قطعة إيكو من حقيبتك بشخصية. مثال: .لبس رقم_الشخصية رقم_القطعة',
        '.ازالة': 'يخلع قطعة إيكو من شخصية عن طريق الكوست (لو عندها قطعتين بنفس الكوست بيلقى وحده الموجودة تلقائياً). مثال: .ازالة رقم_الشخصية الكوست',
        '.بيع_ايكو': 'يبيع قطعة/قطع إيكو من حقيبتك برقمها من .ايكو. مثال: .بيع_ايكو 1 2 3',
        '.تطوير_ايكو': 'يرفع لفل قطعة إيكو من حقيبتك مقابل تيونر الصدى (1 تيونر = لفل واحد) + 26,667 مال لكل لفل (المجموع الكامل من لفل 0 إلى 15 = 400,000)، ويفتح لك ساب ستات جديد عند لفل 5/10/15. مثال: .تطوير_ايكو 1 أو .تطوير_ايكو 1 5',
        '.cooptime': 'يعرض الوقت المتبقي على ظهور/انتهاء وحش Co-Op التعاوني الحالي.',
        '.leaderboard': 'يعرض قائمة أعلى المساهمين بالضرر على وحش Co-Op التعاوني النشط.',
        '.co-op': 'ينضم لمعركة الوحش التعاوني (Co-Op) النشطة حالياً بالقروب.',
    },
    'الفعاليات': {
        '.فعاليه': 'يعرض تفاصيل الفعالية النشطة حالياً بالبوت.',
        '.بدأ_فعاليه': 'أمر للمطور/الإدارة - يبدأ فعالية جديدة بالقروب.',
        '.تسجيل_فعاليه': 'يسجّلك بالفعالية النشطة حالياً للمشاركة.',
        '.الغاء_فعاليه': 'أمر للمطور/الإدارة - يلغي الفعالية النشطة الحالية.',
        '.قائمة_الفعاليه': 'يعرض قائمة المسجّلين حالياً بالفعالية النشطة.',
    },
    'الرويال': {
        '.رويال': 'يعرض حالة الباتل رويال الحالي (عدد المشاركين وحالته).',
        '.دخول': 'ينضم للتسجيل بفعالية الباتل رويال المفتوحة حالياً.',
        '.خروج_رويال': 'ينسحب من الباتل رويال قبل أو أثناء بدء الجولة.',
        '.جاهز': 'يعلن جاهزيتك (يستخدم بأكثر من سياق، منها تبادل الويفوز والرويال).',
        '.تأكيد_الخروج': 'يؤكد رغبتك فعلياً بالانسحاب من الباتل رويال بعد طلب .خروج_رويال.',
        '.نتائج_رويال': 'يعرض نتائج/ترتيب آخر جولة باتل رويال.',
        '.رويال_تفاصيل': 'يعرض تفاصيل موسّعة عن حالة الباتل رويال الحالي (الحالة، عدد المشاركين).',
        '.فريق_رويال': 'يحدد تشكيلة شخصياتك للمشاركة بجولة الباتل رويال. مثال: .فريق_رويال 1 2 3',
        '.اقصاء': 'يقصي لاعباً من الباتل رويال النشط (ضمن آلية الأدوار بالرويال).',
        '.متبقي': 'يعرض عدد وأسماء اللاعبين الأحياء المتبقين بجولة الباتل رويال النشطة.',
    },
    'التخمين_والمسابقات': {
        '.تخمين': 'يبدأ لعبة تخمين شخصية جماعية بالقروب.',
        '.بدا_خمن': 'اسم بديل لبدء لعبة التخمين الجماعية (نفس .تخمين تقريباً).',
        '.انهاء_خمن': 'ينهي لعبة التخمين النشطة (اسم بديل لـ .انهاء_تخمين).',
        '.الاجابة': 'يرسل إجابتك خلال لعبة تخمين نشطة. مثال: .الاجابة اسم_الشخصية',
        '.حالة_التخمين': 'يعرض حالة لعبة التخمين النشطة حالياً.',
        '.انهاء_تخمين': 'ينهي لعبة التخمين النشطة.',
        '.بدا_مسابقة': 'يبدأ مسابقة أسئلة جماعية عامة بالقروب.',
        '.بدا_مسابقة_سس': 'يبدأ مسابقة أسئلة خاصة بشخصيات رتبة SSS.',
        '.بدا_مسابقة_صور': 'يبدأ مسابقة تخمين شخصيات بالصور.',
        '.بدا_مسابقة_كت': 'يبدأ مسابقة أسئلة بنمط "كت" (أسماء أنمي).',
        '.انهاء_مسابقة': 'ينهي المسابقة الجماعية النشطة بالقروب.',
        '.النقاط': 'يعرض نقاطك الحالية بالمسابقة الجماعية النشطة.',
        '.ثلاث': 'أمر خاص ضمن إحدى ألعاب الأسئلة/التخمين الجماعية بالقروب.',
    },
    'لعبة_القلوب': {
        '.قلوب': 'يعرض حالة لعبة القلوب (Heart Quiz) الجماعية النشطة بالقروب.',
        '.تسجيل_قلوب': 'يسجّلك بلعبة القلوب الجماعية للمشاركة بالجولة القادمة.',
        '.ابداقلـوب': 'يبدأ جولة جديدة من لعبة القلوب الجماعية.',
        '.انهاء_قلوب': 'ينهي جولة لعبة القلوب النشطة حالياً.',
        '.نقص': 'يرسل إجابتك (رقمك) خلال دورك بلعبة القلوب النشطة. مثال: .نقص رقم',
        '.ديني': 'أمر تفاعلي ضمن أسئلة لعبة القلوب الجماعية.',
    },
    'العوالم': {
        '.تفاصيل_العوالم': 'يعرض تفاصيل موسم العوالم الحالي وحالتك فيه.',
        '.ترتيب_العوالم': 'يعرض ترتيب اللاعبين/العشائر بموسم العوالم الحالي.',
    },
    'الانتقالات': {
        '.انتقال': 'ينشئ عرض انتقال (نقل) شخصية للاعب آخر مقابل سعر. مثال: .انتقال "اسم_الشخصية" السعر',
        '.انتقالاتي': 'يعرض قائمة عروض الانتقال اللي أنشأتها.',
        '.حذف_انتقال': 'يحذف عرض انتقال أنشأته برقمه.',
        '.عرض_انتقال': 'يعرض تفاصيل عرض انتقال معيّن برقمه.',
        '.قبول_انتقال': 'يقبل عرض انتقال شخصية من لاعب آخر ويدفع سعره. مثال: .قبول_انتقال رقم_العرض اسم_شخصيتك',
        '.دخول_زون': 'يدخل منطقة/زون معيّنة ضمن أحد أنظمة اللعبة.',
    },
    'العاب_متفرقة': {
        '.س': 'يحوّل صورة أو GIF أو فيديو (بالرد عليه) إلى ستيكر.',
        '.كت': 'يرسل قائمة أسماء أنمي عشوائية (تستخدم بألعاب مثل التخمين).',
        '.xo': 'يبدأ مباراة إكس-أو (XO/Tic-Tac-Toe) مع لاعب آخر بالمنشن.',
        '.موافق': 'يوافق على دعوة مباراة (مثل XO) مرسلة لك.',
        '.نعم': 'يؤكد عملية بيع شخصية معلّقة (بعد أمر .بيع).',
        '.لا': 'يلغي عملية بيع شخصية معلّقة (بعد أمر .بيع).',
        '.card': 'يرسل بطاقة (صورة) عشوائية لشخصية من نظام البطاقات.',
        '.ذكاء': 'يسأل البوت سؤالاً عاماً ويرد بالذكاء الاصطناعي. مثال: .ذكاء ما هو كذا',
    },
    'المطور': {
        '.تبنيد': 'أمر للمطور فقط - يمنع شخصاً من استخدام أي أمر بالبوت حتى يُفك عنه. حدد الهدف بالرد على رسالته أو بمنشنه مباشرة.',
        '.فك': 'أمر للمطور فقط - يفك التبنيد عن شخص ويعيد له القدرة على استخدام الأوامر. حدد الهدف بالرد على رسالته أو بمنشنه مباشرة.',
        ...
        '.اسبون_بوس': 'أمر للمطور فقط - يستدعي (يُظهر) الزعيم العالمي يدوياً بالقروب.',
        '.قتل_البوس': 'أمر للمطور فقط - ينهي الزعيم العالمي الحالي يدوياً ويوزّع جوائزه.',
        '.رسبن_رايد': 'أمر للمطور فقط - يعيد ظهور (Respawn) وحش/هدف الرايد الجماعي يدوياً.',
        '.spawncoop': 'أمر للمطور فقط - يستدعي وحش Co-Op التعاوني يدوياً.',
        '.تصفير_العشائر': 'أمر للمطور فقط - يصفّر بيانات كل العشائر.',
        '.حذف_العشائر_الفارغة': 'أمر للمطور فقط - يحذف كل العشائر اللي ما فيها أعضاء.',
        '.الغاء_الحروب': 'أمر للمطور فقط - يلغي كل حروب العشائر الجارية حالياً.',
        '.اصلاح_المخزون': 'أمر للمطور فقط - يعيد حساب مخزون الشخصيات (maxCharacters) لكل اللاعبين.',
        '.مسح_المعدات': 'أمر للمطور فقط - يمسح أي أثر متبقي لنظام المعدات القديم من قاعدة البيانات لكل اللاعبين.',
        '.تصفير_الانجازات': 'أمر للمطور فقط - يصفّر إنجازات اللاعبين.',
        '.فحصxp': 'أمر للمطور فقط - يفحص ويعرض تفاصيل نظام الخبرة (XP) للاعبين.',
        '.تصحيح_لفل200': 'أمر للمطور فقط - يصلح مشكلة متعلقة بلاعبين وصلوا للمستوى 200.',
        '.تصحيح_القتالات': 'أمر للمطور فقط - يصلح بيانات قتالات تالفة أو خاطئة باللاعبين.',
        '.تص_رانك': 'أمر للمطور فقط - يعرض توب 20 لاعب حسب الرانك الجديد (مضاربة + تحدي).',
        '.ريست_تحدي': 'أمر للمطور فقط - يصفّر عدد التحديات المستخدمة لكل اللاعبين.',
        ...
        '.update waifus images': 'أمر للمطور فقط - يحدّث صور الويفوز من مصدر AniList.',
        '.نقل_حساب': 'أمر للمطور فقط - يبدّل كل بيانات حسابين مع بعض بالكامل (مستوى، فلوس، شخصيات، ألقاب، صناديق...). مثال: .نقل_حساب @الحساب_القديم @الحساب_الجديد',
        '.نقل_حساب_قديم': 'أمر للمطور فقط - ينقل كل بيانات حساب قديم لحساب جديد باتجاه واحد (الحساب القديم يُحذف نهائياً). مثال: .نقل_حساب_قديم @الحساب_القديم @الحساب_الجديد',
        '.استرجاع_sss': 'أمر للمطور فقط - يسترجع شخصية من رتبة SSS تم بيعها بالخطأ.',
        '.انهاء_موسم_العوالم': 'أمر للمطور فقط - ينهي موسم العوالم الحالي ويوزّع جوائزه.',
        '.بدا_قناص': 'أمر للمطور فقط - يبدأ فعالية "القناص" الجماعية بالقروب.',
        '.انهاء_قناص': 'أمر للمطور فقط - ينهي فعالية "القناص" النشطة.',
        '.بدء_رويال': 'أمر للمطور فقط - يفتح تسجيل جديد لفعالية الباتل رويال.',
        '.انطلاق_رويال': 'أمر للمطور فقط - يبدأ فعلياً جولة الباتل رويال بعد اكتمال التسجيل (يحتاج 3 لاعبين على الأقل).',
        '.انهاء_رويال': 'أمر للمطور فقط - ينهي فعالية الباتل رويال الحالية بالكامل.',
        '.اشرح pvp': 'أمر للمطور فقط - يعرض شرحاً تفصيلياً كاملاً لآلية عمل نظام القتال PvP.',
    },
}

if (text.startsWith('.شرح')) {

    const catName = text.replace('.شرح', '').trim()

    if (!catName) {
        let listMsg = `📖 *استخدم: .شرح <اسم القسم>*\n\n━━━━━━━━━━━━━━\n\nالأقسام المتوفرة:\n\n`
        for (const c in commandExplanations) {
            listMsg += `📂 ${c}\n`
        }
        listMsg += `\n━━━━━━━━━━━━━━\n💡 مثال: .شرح الحرب`
        return safeSend(msg.key.remoteJid, { text: listMsg })
    }

    if (!commandExplanations[catName]) {
        return safeSend(msg.key.remoteJid, { text: '❌ هذا القسم غير موجود. اكتب .شرح لعرض كل الأقسام.' })
    }

    if (catName === 'المطور' && !isOwner(msg)) {
        return
    }

    let txt = `📖 *شرح أوامر قسم: ${catName}*\n\n━━━━━━━━━━━━━━\n\n`
    const catCmds = commandExplanations[catName]
    for (const cmd in catCmds) {
        txt += `🔹 *${cmd}*\n${catCmds[cmd]}\n\n`
    }
    txt += `━━━━━━━━━━━━━━`
...
try {
    ...
    let player = await Player.findOne({ userId }).lean()
...
╰━━━━━━━━━━━━━━╯`
    ...
    const profileImg = await resolveCharacterImageForSend(
        applyCustomImage(mainCharacter, mainCharacter).image
    )
...
// ─── .اسحب ───
        ...
        // ⚡ تم نقل أمر .اسحب لأعلى معالج الرسائل (فوق كل الأوامر) ليرد بسرعة
...
if (text === '.شخصياتي') {
    ...
    try {
        ...
        // ✅ ترتيب تلقائي عند كل عرض لـ .شخصياتي — لكن الشخصية رقم 1
        // ثابتة دائماً ولا تتأثر بالترتيب التلقائي (سواء رتبتها يدوياً
        // بـ ".ترتيب رقم1 رقم2" أو كانت أول شخصية بالأصل). باقي
        // الشخصيات (رقم 2 فما فوق) تترتب تلقائياً حسب الرتبة ثم القوة.
        // (الرقم المعروض هنا = index الحقيقي بالمصفوفة لباقي الأوامر
        // .اهداء .تطوير .عرض .قدره .غزو .تشكيلة .حول .معداتي .بيع .انتقال .هجوم)
        resortPlayerCharactersKeepFirst(player)
        await player.save()
...
// ─── .المعرض ───
// .المعرض                → يولّد صورة "شخصياتي" من الشخصيات المختارة حاليًا
// .المعرض اضف <رقم>       → يضيف شخصية للمعرض (الرقم = نفس رقمها بـ .شخصياتي)
...
if (text.startsWith('.المعرض')) {
    ...
    if (!player || !player.characters?.length) {
        return safeSend(
            msg.key.remoteJid,
            { text: '❌ لا تملك شخصيات — جرب .اسحب أول' }
        )
    }
    ...
    // ── .المعرض اضف <رقم> [رقم رقم ...] ── (يقبل أكثر من رقم بنفس الأمر)
    if (sub === 'اضف') {

        const indices = args.slice(2)
            .map(n => parseInt(n))
            .filter(n => !isNaN(n))
        ...
        if (!indices.length) {
            return safeSend(
                msg.key.remoteJid,
                { text: '❌ الاستخدام: .المعرض اضف رقم (نفس رقم .شخصياتي) — تقدر تحط أكثر من رقم بنفس الأمر، مثال: .المعرض اضف 1 2 10' }
            )
        }
        ...
        for (const idx of indices) {
            const result = addToGallery(player, idx)
            if (result.ok) {
                const tier = result.rarity ? resolveTierKey(result.rarity, result.evolutionLevel) : ''
                added.push(`${result.name}${tier ? ` (${tier})` : ''}`)
            } else {
                failed.push(`رقم ${idx}: ${result.reason}`)
            }
        }

        if (added.length) await player.save()
        ...
        txt += `\n📊 المعرض الآن: ${player.gallery.length}/${MAX_GALLERY}`

        return safeSend(msg.key.remoteJid, { text: txt })
    }

    // ── .المعرض حذف <رقم> [رقم رقم ...] ── (يقبل أكثر من رقم بنفس الأمر)
    if (sub === 'حذف') {

        // نرتب تنازليًا ونشيل التكرار — الحذف من الأكبر للأصغر يضمن إن
        // رقم مكان كل شخصية بالمعرض يبقى صحيح أثناء الحذف المتتالي
        // (حذف رقم كبير ما يأثر على ترقيم الأرقام الأصغر منه).
        const indices = [...new Set(
            args.slice(2).map(n => parseInt(n)).filter(n => !isNaN(n))
        )].sort((a, b) => b - a)

        if (!indices.length) {
            return safeSend(
                msg.key.remoteJid,
                { text: '❌ الاستخدام: .المعرض حذف رقم (رقم مكانها بالمعرض) — تقدر تحط أكثر من رقم بنفس الأمر، مثال: .المعرض حذف 1 3' }
            )
        }
        ...
        for (const idx of indices) {
            const result = removeFromGallery(player, idx)
            if (result.ok) {
        ...
        if (removed.length) await player.save()

        let txt = ''
        if (removed.length) {
            txt += `🗑️ انحذف من المعرض:\n${removed.map(n => `• ${n}`).join('\n')}\n`
        }
        if (failed.length) {
            txt += `${removed.length ? '\n' : ''}❌ ما انحذف:\n${failed.map(f => `• ${f}`).join('\n')}\n`
        }
    ...
    if (sub === 'قائمة') {
        ...
        if (!player.gallery?.length) {
            return safeSend(
                msg.key.remoteJid,
                { text: '📭 معرضك فاضي — ضيف شخصيات بـ .المعرض اضف رقم' }
            )
        }

        let txt = `🖼️ قائمة معرضك (${player.gallery.length}/${MAX_GALLERY})\n\n`

        player.gallery.forEach((id, i) => {
            const char = player.characters.find(c => c._id && c._id.toString() === id)
            if (char) {
                const tier = resolveTierKey(char.rarity, char.evolutionLevel)
                txt += `${i + 1}. ${char.name} (${tier})\n`
            } else {
                txt += `${i + 1}. (شخصية محذوفة)\n`
            }
        })

        txt += `\nللحذف: .المعرض حذف رقم`

        return safeSend(msg.key.remoteJid, { text: txt })
    }

    // ── .المعرض (بدون شي) => يولّد الصورة من الاختيار الحالي ──

    const galleryChars = getGalleryCharacters(player)

    if (!galleryChars.length) {
        return safeSend(
            msg.key.remoteJid,
            { text: '📭 معرضك فاضي — ضيف شخصيات بـ .المعرض اضف رقم (الرقم من .شخصياتي)' }
        )
    }

    await safeSend(msg.key.remoteJid, { text: '🎴 جاري تجهيز البطاقة...' })

    // 🧵 لا نسوي await لتوليد الصورة هنا داخل المعالج الرئيسي — renderRosterImage
    // (Puppeteer) ياخذ وقت (ثواني)، ولو انتظرناه هنا بالتتابع، أي رسالة/أمر
    // ثاني يوصل أثناء التوليد يضطر ينتظر لين تخلص هذي الصورة قبل ما يتعالج
    // (لأن هذا نفس معالج messages.upsert اللي يستقبل كل الرسائل). الحل: نطلق
    // التوليد + الإرسال كمهمة منفصلة بالخلفية (fire-and-forget) بدون await
    // هنا، فيرجع هذا الأمر فورًا ويتحرر المعالج الرئيسي لباقي الأوامر مباشرة،
    // وبمجرد ما تخلص الصورة تنرسل من تلقاء نفسها. نفس المنطق بالضبط (نفس
    // renderRosterImage، نفس الرسائل، نفس معالجة الأخطاء) — فقط بدون حجب.
    ;(async () => {
        try {

            const imageBuffer = await renderRosterImage(galleryChars, {
                title: 'شخصياتي',
            ...
            await safeSend(
                ...
                {
                    image: imageBuffer,
                    caption: `🎴 معرضك (${galleryChars.length}/${MAX_GALLERY})`
                }
            )
            ...
            await safeSend(
                msg.key.remoteJid,
                { text: '❌ صار خطأ أثناء توليد الصورة، حاول مرة ثانية' }
            )
        }
    })()
...
// ─── .احصائيات ───
// يعرض الإحصائيات النسبية المركّبة (Composite %) الناتجة من نظام الإيكوز لشخصية معينة:
// الستات الرئيسي (main stat) + كل الساب ستات + بونص العوائل النشطة (2pc/5pc)
// مثال: .احصائيات 1  (الرقم نفسه المستخدم بـ .شخصياتي)

if (text.startsWith('.احصائيات') && !text.startsWith('.احصائيات_كاملة')) {
    ...
    try {
        ...
        if (isNaN(number)) {
            return safeSend(msg.key.remoteJid, {
                text: '❌ استخدم: .احصائيات رقم_الشخصية (نفس رقم .شخصياتي)'
            })
        }
        ...
        if (!character) {
            return safeSend(msg.key.remoteJid, {
                text: `❌ رقم غير صحيح. اكتب .شخصياتي لمعرفة الأرقام (من 1 إلى ${player.characters.length})`
            })
        }

        const echoes = equipmentSystem.ensureEquipmentSlots(character)
        const bonus = equipmentSystem.calculateEquipmentStats(character)
        const families = equipmentSystem.getActiveFamilyDetails(echoes)

        const equippedCount = equipmentSystem.SLOT_KEYS.filter(k => echoes[k]).length

        const STAT_LABEL = {
            attack: '⚔️ هجوم', attackPercent: '⚔️ هجوم',
            defense: '🛡️ دفاع', defensePercent: '🛡️ دفاع',
            hp: '❤️ HP', hpPercent: '❤️ HP',
            critRate: '🎯 نسبة الحرج', critDamage: '💥 ضرر الحرج',
            bossDamage: '👹 ضرر بوس', dodge: '👻 مراوغة',
            accuracy: '🎯 دقة', shield: '🛡️ درع',
            lifesteal: '🩸 امتصاص حياة', reflect: '🪞 عكس ضرر'
        }

        const isPercentKey = (k) =>
            k.includes('Percent') ||
            ['critRate', 'critDamage', 'bossDamage', 'dodge', 'accuracy', 'lifesteal', 'reflect'].includes(k)
        ...
        for (const key in bonus) {
            ...
            const line = `${label}: +${value}${isPercentKey(key) ? '%' : ''}`

            if (isPercentKey(key)) {
                percentLines.push(line)
            } else {
                flatLines.push(line)
            }
        ...
        let message =
`📊 إحصائيات الإيكوز — ${character.name}
🎐 القطع المجهزة: ${equippedCount}/5

`

        if (percentLines.length) {
            message += `📈 النسب المركّبة (%):\n${percentLines.join('\n')}\n\n`
        }

        if (flatLines.length) {
            message += `🔢 القيم الثابتة:\n${flatLines.join('\n')}\n\n`
        }

        if (!percentLines.length && !flatLines.length) {
            message += 'لا توجد أي إحصائيات — لا يوجد إيكو مجهز على هذي الشخصية.\n\n'
        }

        if (families.length) {

            message += `🎼 بونص العوائل النشط:\n`

            for (const f of families) {

                if (!f.bonus2Active && !f.bonus5Active) continue

                message += `${f.familyNameAr || f.familyName} (${f.count} قطع)\n`

                if (f.bonus2Active) {
                    const b2 = Object.entries(f.bonus2)
                        .map(([k, v]) => `${STAT_LABEL[k] || k} +${v}${isPercentKey(k) ? '%' : ''}`)
                        .join('، ')
                    message += `  ├ 2pc: ${b2}\n`
                }

                if (f.bonus5Active) {
                    const b5 = Object.entries(f.bonus5)
                        .map(([k, v]) => `${STAT_LABEL[k] || k} +${v}${isPercentKey(k) ? '%' : ''}`)
                        .join('، ')
                    message += `  └ 5pc: ${b5}\n`
                }
            ...
            message += `🎼 بونص العوائل: لا يوجد بونص نشط حالياً (تحتاج قطعتين على الأقل من نفس العائلة)`
        }

        return safeSend(msg.key.remoteJid, { text: message.trim() })

    } catch (err) {

        console.log('احصائيات error:', err)

        return safeSend(msg.key.remoteJid, {
            text: '❌ حدث خطأ في عرض إحصائيات الإيكوز'
        })
...
// ─── .احصائيات_كاملة ───
// أمر شامل: يجمع بمكان واحد كل مصدر يزيد ATK/كريت/دفاع/... لشخصية معينة —
// إيكوز + سلاح + بونصات المستوى (player-wide) + الرفيق + قدرات EX —
// بعكس .احصائيات اللي يقتصر على الإيكوز بس.
// مثال: .احصائيات_كاملة 1  (نفس رقم .شخصياتي)
if (text.startsWith('.احصائيات_كاملة')) {
    ...
    try {
        ...
        if (isNaN(number)) {
            return safeSend(msg.key.remoteJid, {
                text: '❌ استخدم: .احصائيات_كاملة رقم_الشخصية (نفس رقم .شخصياتي)'
            })
        }
        ...
        function bonusToLines(bonus) {
            const lines = []
            for (const key in bonus) {
                const value = bonus[key]
                if (!value) continue
                const label = STAT_LABEL[key] || key
                lines.push(`${label}: +${value}${isPercentKey(key) ? '%' : ''}`)
            }
            return lines
        }

        // =========================
        // 1) إيكوز (نظام المعدات الجديد)
        // =========================
        const echoes = equipmentSystem.ensureEquipmentSlots(character)
        const echoBonus = equipmentSystem.calculateEquipmentStats(character)
        const families = equipmentSystem.getActiveFamilyDetails(echoes)
        const equippedCount = equipmentSystem.SLOT_KEYS.filter(k => echoes[k]).length
        ...
        // 2) السلاح المركّب
        // =========================
        const weaponBonus = getWeaponBonusForCharacter(player, character)
        const equippedWeapon = Array.isArray(player.weaponsInventory)
            ? player.weaponsInventory.find(w => w && w.equippedTo === character.name)
            : null

        // =========================
        // 3) بونصات المستوى (levelAbilities) — بونص عام على اللاعب،
        // مو خاص بشخصية معينة. نوضّح أيها فعليًا مقروء بهجوم الزعيم
        // حاليًا وأيها لسا مو مربوط بأي معادلة ضرر (حقل معروض بس).
        // =========================
        const levelBonusLive = [
            { key: 'attackBonus', label: '⚔️ هجوم', wired: true },
            { key: 'bossDamageBonus', label: '👹 ضرر بوس', wired: true },
            { key: 'critBonus', label: '🎯 كريت', wired: false },
            { key: 'defenseBonus', label: '🛡️ دفاع', wired: false },
            { key: 'dodgeBonus', label: '💨 مراوغة', wired: false },
            { key: 'reflectBonus', label: '🪞 عكس ضرر', wired: false },
            { key: 'lifestealBonus', label: '🩸 امتصاص حياة', wired: false }
        ]

        // =========================
        // 4) بونص الرفيق (Companion) — حسب النوع المجهز حاليًا
        // =========================
        let companionText = 'لا يوجد رفيق مجهز'
        if (player.companion && player.companion.key && (player.companion.level || 0) >= 1) {
            const companionBonusValue = companionsData.getCompanionBonus(
                player.companion.key,
                player.companion.level
            )
            companionText =
                `${player.companion.key} (مستوى ${player.companion.level}) — بونص +${companionBonusValue}%`
        }

        // =========================
        // 5) قدرات EX — تُعرض بالاسم فقط (تفعيلها بالمعركة عشوائي/جزئي
        // حسب useEXAbilities، مو بونص ثابت نقدر نطبعه كرقم واحد هنا)
        // =========================
        const hasEX =
            character.evolutionLevel >= 1 &&
            character.urAbilities &&
            character.urAbilities.length > 0

        // =========================
        // 6) الأرقام النهائية المجمّعة (زي شاشة ستاتات الشخصية بالألعاب —
        // كل شي مجموع برقم واحد نهائي، بدل تفكيكه حسب المصدر)
        // نفس صيغة معادلة هجوم الزعيم الفعلية بالضبط (باستثناء قدرات EX
        // لأنها رمية عشوائية بالمعركة، مو بونص ثابت نقدر نجمعه برقم واحد)
        // =========================
        const combined = {}
        for (const key of new Set([...Object.keys(echoBonus), ...Object.keys(weaponBonus)])) {
            combined[key] = (echoBonus[key] || 0) + (weaponBonus[key] || 0)
        }

        let companionCritBonus = 0
        let companionBossDmgBonus = 0
        let companionHpBonus = 0
        if (player.companion && player.companion.key && (player.companion.level || 0) >= 1) {
            const cVal = companionsData.getCompanionBonus(player.companion.key, player.companion.level)
            if (player.companion.key === 'tiger') companionCritBonus = cVal
            if (player.companion.key === 'lion') companionBossDmgBonus = cVal
            if (player.companion.key === 'bear') companionHpBonus = cVal
        }

        let totalATK = character.power || 0
        totalATK += (combined.attack || 0)
        if ((combined.attackPercent || 0) > 0) {
            totalATK = Math.floor(totalATK * (1 + combined.attackPercent / 100))
        }
        totalATK = Math.floor(totalATK * (1 + ((player.attackBonus || 0) / 100)))

        const critRateRaw = (combined.critRate || 0) + companionCritBonus
        const critDamageRaw = combined.critDamage || 0
        const critOverflow = Math.max(0, critRateRaw - 100)
        const critRateEffective = Math.min(100, critRateRaw)
        const critDamageEffective = critDamageRaw + critOverflow

        const totalBossDamage = (combined.bossDamage || 0) + (player.bossDamageBonus || 0) + companionBossDmgBonus
        const totalDefenseFlat = combined.defense || 0
        const totalDefensePercent = combined.defensePercent || 0
        const totalHpFlat = combined.hp || 0
        const totalHpPercent = (combined.hpPercent || 0) + companionHpBonus
        const totalDodge = combined.dodge || 0
        const totalLifesteal = combined.lifesteal || 0
        const totalReflect = combined.reflect || 0
        const totalAccuracy = combined.accuracy || 0
        const totalShield = combined.shield || 0

        // =========================
        // بناء الرسالة
        // =========================
        let message =
`📊 الإحصائيات النهائية — ${character.name}
💎 التطوير: +${character.evolutionLevel || 0}

⚔️ ATK: ${totalATK.toLocaleString()}
🛡️ DEF: +${totalDefenseFlat}${totalDefensePercent ? ` (+${totalDefensePercent}%)` : ''}
❤️ HP: +${totalHpFlat}${totalHpPercent ? ` (+${totalHpPercent}%)` : ''}
🎯 Crit Rate: ${critRateRaw}%${critOverflow ? ` (فعّال: ${critRateEffective}%)` : ''}
💥 Crit DMG: ${critDamageEffective}%${critOverflow ? ` (أساسي ${critDamageRaw}% + فايض الكريت ${critOverflow}%)` : ''}
👹 Boss DMG: +${totalBossDamage}%
👻 Dodge: +${totalDodge}%
🩸 Lifesteal: +${totalLifesteal}%
🪞 Reflect: +${totalReflect}%
🎯 Accuracy: +${totalAccuracy}%
🛡️ Shield: +${totalShield}

${critRateRaw >= 100 ? '✅ كريت مضمون كل ضربة (crit rate ≥ 100%)' : `🎲 فرصة كريت كل ضربة: ${critRateEffective}%`}

`

        message += `━━━━━━━━━━━━━━━━━━━━\n📋 مصدر كل رقم (تفصيل)\n━━━━━━━━━━━━━━━━━━━━\n\n`

        const echoLines = bonusToLines(echoBonus)
        message += `🎐 الإيكوز (${equippedCount}/5):\n`
        message += echoLines.length ? echoLines.join('\n') : 'لا يوجد إيكو مجهز'
        message += '\n\n'

        if (families.length && families.some(f => f.bonus2Active || f.bonus5Active)) {
            message += `🎼 بونص العوائل النشط:\n`
            for (const f of families) {
                if (!f.bonus2Active && !f.bonus5Active) continue
                message += `${f.familyNameAr || f.familyName} (${f.count} قطع)\n`
                if (f.bonus2Active) {
                    message += `  ├ 2pc: ${Object.entries(f.bonus2).map(([k, v]) => `${STAT_LABEL[k] || k} +${v}${isPercentKey(k) ? '%' : ''}`).join('، ')}\n`
                }
                if (f.bonus5Active) {
                    message += `  └ 5pc: ${Object.entries(f.bonus5).map(([k, v]) => `${STAT_LABEL[k] || k} +${v}${isPercentKey(k) ? '%' : ''}`).join('، ')}\n`
                }
            }
            message += '\n'
        }
        ...
        message += `⚔️ السلاح: `
        if (equippedWeapon) {
            message += `${equippedWeapon.name || 'سلاح'}\n`
            const weaponLines = bonusToLines(weaponBonus)
            message += weaponLines.length ? weaponLines.join('\n') : 'بدون بونص إضافي'
        } else {
            message += 'لا يوجد سلاح مجهز'
        }
        message += '\n\n'

        message += `📈 بونصات المستوى (عامة على حسابك):\n`
        for (const b of levelBonusLive) {
            const value = player[b.key] || 0
            if (!value) continue
            message += `${b.label}: +${value}%${b.wired ? '' : '  ⚠️ (مو مربوط بهجوم الزعيم، ما دخل بالمجموع أعلاه)'}\n`
        }
        if (!levelBonusLive.some(b => player[b.key])) {
            message += 'لا توجد قدرات مستوى مفعّلة بعد\n'
        }
        message += '\n'

        message += `🐾 الرفيق: ${companionText}\n\n`

        message += `✨ قدرات EX: `
        if (hasEX) {
            message += character.urAbilities.map(a => a.name).join('، ')
            message += '\n⚠️ عشوائية بالمعركة — ما دخلت بالمجموع أعلاه'
        } else {
            message += 'لا توجد'
        }
        ...
        console.log('احصائيات_كاملة error:', err)

        return safeSend(msg.key.remoteJid, {
            text: '❌ حدث خطأ في عرض الإحصائيات الشاملة'
        })
...
const claims =
    await WaifuClaim.find({
        userId,
        groupId
    }).populate('waifuId')

const waifu =
    claims[number - 1]?.waifuId
...
// ─── .حذف_القديم ───
// أمر خاص بالمطور: بيمسح الـ index القديم (userId_1) من كولكشن waifuplayers
// وبيتأكد إن الـ index الجديد المركّب (userId + groupId) موجود
// عشان تختفي مشكلة duplicate key نهائيًا لكل المستخدمين

if (text.startsWith('.حذف_القديم')) {
    ...
    try {
        ...
        const collection =
            WaifuPlayer.collection

        const indexes =
            await collection.indexes()

        const oldIndexExists =
            indexes.some(i => i.name === 'userId_1')

        let resultText = '📋 نتيجة الفحص:\n\n'

        if (oldIndexExists) {
            await collection.dropIndex('userId_1')
            resultText +=
                '🗑️ تم حذف الـ index القديم (userId_1)\n'
        } else {
            resultText +=
                'ℹ️ الـ index القديم مش موجود أصلاً\n'
        }

        const newIndexExists =
            indexes.some(i => i.name === 'userId_1_groupId_1')
        ...
        if (!newIndexExists) {
            ...
            resultText +=
                '✅ تم إنشاء الـ index الجديد (userId + groupId)\n'
        } else {
            resultText +=
                'ℹ️ الـ index الجديد موجود بالفعل\n'
        }
        ...
        console.error('خطأ في .حذف_القديم:', err)
...
if (text.startsWith('.تطوير') && !text.startsWith('.تطوير_ايكو')) {
...
💰 ${omegaCost.toLocaleString()}`
    ...
    // ⚠️ لازم نطابق c.rarity === 'SSS' كمان — لأن characters.json فيه
    // أكثر من نسخة (عادي/اسطوري/SSS) بنفس الاسم وصور مختلفة، والشخصية
    // المطوَّرة لأوميقا مصدرها الأصلي دايمًا نسخة SSS (هي أساس التطوير)
    const latestOmegaCharacter = characters.find(
        c => c.name === char.name && c.rarity === 'SSS'
    )
    ...
    // ⚡ دمج رسالة "+نقطة عالم" بكابشن التطور بدل رسالة واتساب مستقلة
    const omegaWorldPointsText = await worlds.awardEvolutionPoints(
        player,
        sock,
        msg.key.remoteJid,
        'Ω'
    )
    const omegaWorldPointsSuffix = omegaWorldPointsText ? `\n\n${omegaWorldPointsText}` : ''

    const omegaDisplayImage = applyCustomImage(char, char).image

    const omegaImg = await resolveCharacterImageForSend(omegaDisplayImage)
...
` : ''}🏆 محاولة أوميقا المستخدمة
┗➤ ${player.omegaEvolutions}/5
...
╔═══━━━✦❖✦━━━═══╗
      🌌 𝗕𝗘𝗬𝗢𝗡𝗗 𝗧𝗛𝗘 𝗟𝗜𝗠𝗜𝗧 🌌
╚═══━━━✦❖✦━━━═══╝${omegaWorldPointsSuffix}`

    if (omegaImg) {
        return sock.sendMessage(msg.key.remoteJid, {
            image: omegaImg,
            caption: omegaCaption
        })
    }

    return safeSend(msg.key.remoteJid, { text: omegaCaption })
}
...
// ⚠️ نفس الملاحظة أعلاه: لازم c.rarity === 'SSS' حتى ما تنسحب صورة
// نسخة عادي/اسطوري بالغلط لشخصية مطوَّرة (SSS+ وما فوق) بنفس الاسم
...
// ⚡ دمج رسالة "+نقطة عالم" بكابشن/رسالة التطور بدل رسالة واتساب مستقلة
const exEvoWorldPointsText = await worlds.awardEvolutionPoints(
    player,
    sock,
    msg.key.remoteJid,
    newRank
)
const exEvoWorldPointsSuffix = exEvoWorldPointsText ? `\n\n${exEvoWorldPointsText}` : ''

if (newLevel === 6) {

    const exDisplayImage = applyCustomImage(char, char).image

    const exImg = await resolveCharacterImageForSend(exDisplayImage)
...
╚═══━━━✦❖✦━━━═══╝${exEvoWorldPointsSuffix}`
    ...
    return safeSend(msg.key.remoteJid, { text: exCaption })
}
...
🧩 تم استهلاك:
2 شظايا${exEvoWorldPointsSuffix}`
    }
)
}
...
if (
    text.startsWith('.فريق_ارينا') ||
    text.startsWith('.هجوم_ارينا') ||
    text.startsWith('.شراء_ارينا') ||
    text === '.شخصيات_ارينا' ||
    text === '.تص_ارينا' ||
    text === '.متجر_الارينا' ||
    text === '.رتبتي_ارينا' ||
...
const player2Data = targetPlayer // ⚡ (تحسين سرعة) نفس اللاعب المجلوب فوق — تجنّب استعلام مكرر لنفس userId
...
// ─── .هجوم ───
        ...
        // ⚡ تم نقل أمر .هجوم لأعلى معالج الرسائل (فوق كل الأوامر) ليرد بسرعة
...
// ⚔️🛡️❤️ تطبيق النسبة % (attackPercent/defensePercent/hpPercent) من الستات الرئيسي الجديد
// بعد ما انضافت الفلات فوق — النسبة تُحسب من القيمة الحالية بعد الفلات
equipmentSystem.applyEquipPercentBonus(attackerStats, myBasicEquipBonus)
equipmentSystem.applyEquipPercentBonus(opponentStats, enemyBasicEquipBonus)
...
// ⚔️🛡️❤️ تطبيق النسبة % (attackPercent/defensePercent/hpPercent) من الستات الرئيسي الجديد
equipmentSystem.applyEquipPercentBonus(attackerStats, myEquipBonus)
equipmentSystem.applyEquipPercentBonus(opponentStats, enemyEquipBonus)
...
// ⚔️🛡️❤️ تطبيق النسبة % (attackPercent/defensePercent/hpPercent) من الستات الرئيسي الجديد
equipmentSystem.applyEquipPercentBonus(attackerStats, myUltEquipBonus)
equipmentSystem.applyEquipPercentBonus(opponentStats, enemyUltEquipBonus)
...
// ─── .ترتيب ───
// الأمر لوحده (بدون أرقام) لا يرتب شيء ولا يعدّل بيانات اللاعب،
// لأن الترتيب التلقائي (حسب الرتبة ثم القوة) يحدث أصلاً عند كل
// عرض لـ .شخصياتي. النقل الفعلي بين الشخصيات يتم فقط عبر
// .ترتيب رقم1 رقم2 (وبشرط أن تكونا بنفس الرتبة).
...
if (text === '.ترتيب') {
    ...
    return safeSend(
        ...
        {
            text:
            'ℹ️ شخصياتك تترتب تلقائياً حسب الرتبة ثم القوة عند كل عرض لـ .شخصياتي، فلا داعي لهذا الأمر بمفرده.\n💡 لنقل شخصية معينة لمكان آخر (بنفس الرتبة فقط)، استخدم: .ترتيب رقم1 رقم2'
        }
    )
}

// ─── .ترتيب رقم1 رقم2 (ترتيب يدوي داخل نفس الرتبة فقط) ───
// مثال: .ترتيب 2 1 → ينقل الشخصية رقم 2 لتصبح رقم 1
// يُسمح بالترتيب فقط بين شخصيات من نفس الرتبة (Ω مع Ω، EX مع EX...)
// ولا يؤثر على الترتيب التلقائي (الذي يبقى كما هو عند إضافة شخصية جديدة)

const manualSortMatch =
    text.match(/^\.ترتيب\s+(\d+)\s+(\d+)$/)
...
if (manualSortMatch) {
    ...
    try {
        ...
        const fromNum =
            parseInt(manualSortMatch[1], 10)

        const toNum =
            parseInt(manualSortMatch[2], 10)

        if (
            fromNum < 1 || fromNum > total ||
            toNum < 1 || toNum > total
        ) {
            ...
            return safeSend(
                ...
                {
                    text:
                    `❌ رقم غير صحيح. اكتب .شخصياتي لمعرفة الأرقام (من 1 إلى ${total})`
                }
            )
        }
        ...
        if (fromNum === toNum) {
            ...
            return safeSend(
                ...
                {
                    text:
                    '⚠️ الرقمين متطابقين، ماكو شي يتغير.'
                }
            )
        }

        const fromIndex = fromNum - 1
        const toIndex = toNum - 1

        const charFrom = player.characters[fromIndex]
        const charTo = player.characters[toIndex]

        const rankFrom = getCharacterRank(charFrom)
        const rankTo = getCharacterRank(charTo)
        ...
        if (rankFrom !== rankTo) {
            ...
            return safeSend(
                ...
                {
                    text:
`❌ لا يمكن الترتيب بين رتب مختلفة.
🔹 رقم ${fromNum} (${charFrom.name}) رتبته: ${rankFrom}
🔹 رقم ${toNum} (${charTo.name}) رتبته: ${rankTo}

💡 الترتيب يشتغل فقط بين شخصيات من نفس الرتبة (مثلاً Ω مع Ω فقط).`
                }
            )
        }

        // ننقل الشخصية من موقعها الحالي إلى الموقع الجديد
        // (بقية شخصيات نفس الرتبة تحافظ على ترتيبها النسبي، وباقي
        // الرتب الأخرى لا تتأثر إطلاقاً)
        const [moved] =
            player.characters.splice(fromIndex, 1)

        player.characters.splice(toIndex, 0, moved)
        ...
        return safeSend(
            ...
            {
                text:
`✅ تم نقل «${moved.name}» من رقم ${fromNum} إلى رقم ${toNum}.
💡 هذا ترتيب يدوي ولن يتأثر بالترتيب التلقائي عند عرض .شخصياتي (فقط عند إضافة شخصية جديدة أو كتابة .ترتيب بدون أرقام).
اكتب .شخصياتي لمشاهدة الترتيب الجديد.`
            }
        )
        ...
        return safeSend(
            ...
            {
                text:
                '❌ حدث خطأ أثناء الترتيب اليدوي للشخصيات'
            }
        )
    }
}
...
if (text === '.مساهمات') {
    ...
    const players =
        await Player.find({ dailyBossDamage: { $gt: 0 } })
            .sort({ dailyBossDamage: -1 })
            .limit(10)
            .lean() // ⚡ عرض فقط، ما يُحفظ — lean أسرع وأخف

    const me =
        await Player.findOne({ userId }).lean() // ⚡ عرض فقط، ما يُحفظ
...
if (text.startsWith('.غزو ')) {
    ...
    // 🔒 منع تنفيذ أكثر من أمر غزو بنفس الوقت لنفس اللاعب
    if (raidLocks.has(userId)) {
        return safeSend(
            msg.key.remoteJid,
            {
                text: '⏳ انتظر حتى تنتهي عملية الغزو السابقة.'
            }
        )
    }
    ...
    // 🛡️ حماية: فك القفل تلقائياً بعد 20 ثانية حتى لو تعلّق أي await
    // (نفس فكرة battleTimeout بالأعلى) عشان اللاعب ما يضل مقفول عليه للأبد
    const raidLockTimeout = setTimeout(() => {
...
💡 الشخصية لم تُستهلك`
    ...
    // ⚫ بونص رفيق شادو — نسبة زيادة على مال هذي المرحلة بالذات
    // (عند مستوى 10 يوصل مجموع الغزو الكامل من 2,000,000 لـ 3,500,000)
    let shadowBonusText = ''
    let shadowExtraMoney = 0

    if (
        player.companion &&
        player.companion.key === 'shadow' &&
        (player.companion.level || 0) >= 1
    ) {

        const shadowPercent =
            companionsData.getCompanionBonus('shadow', player.companion.level)

        shadowExtraMoney =
            Math.floor(stage.reward * shadowPercent / 100)

        if (shadowExtraMoney > 0) {
            shadowBonusText =
                `\n\n⚫ رفيقك شادو زادك +${shadowExtraMoney.toLocaleString()} مال إضافي!`
        }
    }
...
💰 إجمالي الأرباح:
${(player.kingdomRaid.totalEarned || 2000000).toLocaleString()}
...
💰 +${(stage.reward + shadowExtraMoney).toLocaleString()}

🔒 تم استنزاف الشخصية

📍 التقدم:
${player.kingdomRaid.stage}/10${shadowBonusText}`
        }
    )

    } finally {
        clearTimeout(raidLockTimeout)
        raidLocks.delete(userId)
    }
}
...
// 🎯 اختيار حصري: يمنع مهاجمة كوراما والجوبي الاثنين بنفس الدورة —
// يختار وحد بس لين الوحوش تعيد الريسبون (resetBeasts بملف
// beastManager.js)، وبعدها يفتح الاختيار من جديد تلقائياً.
// نستخدم target.respawnAt كمعرّف للدورة الحالية — ثابت طول ما
// الوحش حي بهالدورة، ويتغيّر مع كل ريسبون جديد (resetBeasts يعيّن
// respawnAt جديد لكل الوحوش مع بعض)، فما نحتاج نلمس beastManager.js
// ولا نضيف أي حقل جديد هناك — الفحص يصير بالكامل هنا وقت الهجوم.
const currentBeastCycle =
    target.respawnAt
        ? target.respawnAt.getTime()
        : null

if (
    player.beastChoiceTarget &&
    player.beastChoiceTarget !== 'kurama' &&
    player.beastChoiceCycle === currentBeastCycle
) {
    ...
    return sock.sendMessage(
        ...
        {
            text:
'❌ اخترت مهاجمة الجوبي هالدورة\n\n🔒 ما تقدر تهاجم كوراما بنفس الوقت — الاختيار يفتح من جديد بعد الريسبون القادم للوحوش.'
        }
    )
...
`
    ...
    // 🔧 نفس إصلاح إشعار الاستيقاظ: لا نفشل بصمت لو ما فيه صورة
    await sock.sendMessage(
        msg.key.remoteJid,
        target.image
            ? {
...
for (
    ...
    i++
        ...
        // 🥚 نستخدم دايماً جدول المكافآت اللي فيه حقل egg — سواء
        // كان هذا الوحش حامل بيضة الرفيق أو لا. كذا، أول 3 دايماً
        // ياخذون بيضة فعلية، بس النوع يختلف: رفيق أو وحش حسب
        // eggCarrier (قبل getNormalReward كانت ترجع tickets بدون
        // أي egg، فبيضة الوحش ما كانت تنحسب فعلياً رغم ظهور
        // رسالة "📦 بيضة وحش")
        const reward =
            beastRewards.getEggCarrierReward(i + 1)

        // 💰 مبالغ ثابتة جديدة حسب الترتيب — نفس القيم للوحشين
        // (كوراما والجوبي)، بدل الاعتماد على قيمة reward.money
        const moneyReward =
            i === 0
                ? 1000000
            : i === 1
        ...
        const finalMoneyReward = applyCatBonus(p, moneyReward)
        ...
        // 🥚 أول 3 بترتيب الضرر بس ياخذون بيضة — نوعها يتحدد حسب
        // مين حامل البيضة هالدورة (eggCarrier يتبدّل عشوائياً بين
        // كوراما والجوبي بكل ريسبون). الرابع فما فوق: بدون بيضة
        // إطلاقاً، بس يستمرون ياخذون تذاكر البيض (eggTickets فوق)
        if (i < 3) {

            if (target.eggCarrier) {
...
💰 ${finalMoneyReward.toLocaleString()}
⭐ ${reward.xp || 0}
🎟️ ${reward.tickets || 0}
${eggLabel}: ${reward.egg || 0}
...
// 🎯 نفس فحص الاختيار الحصري بأمر .اقضي — نفس الفكرة بالضبط بس
// معكوسة (لو سبق واختار كوراما هالدورة، يمنعه من الجوبي)
const currentBeastCycle =
    target.respawnAt
        ? target.respawnAt.getTime()
        : null
...
if (
    player.beastChoiceTarget &&
    player.beastChoiceTarget !== 'juubi' &&
    player.beastChoiceCycle === currentBeastCycle
) {
    ...
    return sock.sendMessage(
        ...
        {
            text:
'❌ اخترت مهاجمة كوراما هالدورة\n\n🔒 ما تقدر تهاجم الجوبي بنفس الوقت — الاختيار يفتح من جديد بعد الريسبون القادم للوحوش.'
        }
    )
...
for (
    ...
    i++
        ...
        // 🥚 نفس إصلاح كوراما: دايماً نستخدم جدول المكافآت اللي فيه
        // حقل egg، عشان أول 3 ياخذون بيضة فعلية دايماً (رفيق أو
        // وحش حسب eggCarrier) بدل ما تضيع بيضة الوحش بسبب
        // getNormalReward اللي ما فيها egg
        const reward =
            beastRewards.getEggCarrierReward(i + 1)

        // 💰 نفس المبالغ الثابتة المستخدمة بكوراما
        const moneyReward =
            i === 0
                ? 1000000
            : i === 1
        ...
        // 🥚 نفس منطق كوراما: أول 3 بس ياخذون بيضة (نوعها حسب
        // eggCarrier الحالي)، الرابع فما فوق بدون بيضة — تذاكر
        // البيض فقط (eggTickets فوق)
        if (i < 3) {
...
// 🐾 بيضة الرفيق — منفصلة كلياً عن نظام الوحوش (ما تدخل ownedBeasts
// ولا تُحسب بإنجاز جمع الوحوش)، أغلى وحش حالي (كوراما) + 500
if (beastName === 'رفيق') {

    // ⚡ (تحسين سرعة) player مجلوب فوق بنفس الـ userId قبل سطرين
    // بدون أي حفظ بينهما — إعادة الجلب هنا كانت تكرار لنفس الاستعلام

    const companionEggPrice = 2000
...
${player.eggTickets || 0}`
    ...
    player.eggTickets -= companionEggPrice

    player.companionEggs =
        (player.companionEggs || 0) + 1

    await player.save()

    const buyEggText =
`🥚 تم شراء بيضة رفيق بنجاح

🎟️ السعر:
${companionEggPrice} تذكرة
...
${player.companionEggs}`
    ...
    if (companionsData.EGG_IMAGE) {
        return sock.sendMessage(
            msg.key.remoteJid,
            {
                image: { url: companionsData.EGG_IMAGE },
                caption: buyEggText
            }
        )
    }
...
🐾 رفيق (بيضة رفيق)   2000 تذكرة
💡 غير قتالي — بونص خفيف مستمر
...
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
// 🐾 قسم: نظام الرفيق (Companion System)
// غير قتالي بالكامل، منفصل عن نظام الوحوش (Beast)
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

// دالة تصفير المتجر الشخصي إذا صار يوم جديد (نفس منطق getSaudiDate)
function resetCompanionShopIfNeeded(player) {

    const today = getSaudiDate()

    if (!player.companionShop) {
        player.companionShop = { lastReset: '', buysToday: 0 }
    }

    if (player.companionShop.lastReset !== today) {
        player.companionShop.lastReset = today
        player.companionShop.buysToday = 0
    }
}
...
if (text === '.فقس_رفيق') {
    ...
    if ((player.companionEggs || 0) < 1) {
        return safeSend(msg.key.remoteJid, {
            text:
`❌ لا تملك أي بيضة رفيق
...
.شراء_وحش رفيق`
    ...
    if (player.companion && player.companion.key) {
        return safeSend(msg.key.remoteJid, {
            text:
`❌ عندك رفيق بالفعل: ${player.companion.customName || companionsData.ANIMALS[player.companion.key]?.defaultName}

🐾 تقدر تربّي رفيق واحد بس بنفس الوقت
💡 استخدم .بيع_رفيق لو تبي تبدأ من جديد بحيوان ثاني`
        })
    }

    // اختيار عشوائي من التسعة
    const randomKey =
        companionsData.ANIMAL_KEYS[
            Math.floor(Math.random() * companionsData.ANIMAL_KEYS.length)
        ]

    const animal = companionsData.ANIMALS[randomKey]

    // 🖼️ اختيار عشوائي لصورة الرفيق من بين الـ 6 خيارات
    const randomImageIndex = Math.floor(Math.random() * companionsData.IMAGES_PER_ANIMAL)

    player.companionEggs -= 1
...
${animal.emoji} حصلت على: ${animal.defaultName}

📝 ${animal.description}

💡 سمّه باسم مخصص (مرة وحدة بس، تقدر تسميه الحين):
.تسمية_رفيق <اسم>

🎴 غيّر صورته وقتما تحب:
.رفيق_صورة <1-6>

🍖 أطعمه عشان يرفع مستواه:
.اطعام_رفيق`

    const hatchImage = companionsData.getCompanionImage(randomKey, randomImageIndex)

    if (hatchImage) {
        return safeSend(msg.key.remoteJid, {
            image: { url: hatchImage },
            caption: hatchText
        })
    }
...
🎫 بيوض تملكها:
${player.companionEggs || 0}`
        })
    }

    const card = companionsData.buildCompanionCard(
        player.companion.key,
        player.companion.level,
        player.companion.foodProgress,
        player.companion.customName,
        player.companion.imageIndex
    )
...
// ─── .رفيق_صورة <1-6> ───
if (text.startsWith('.رفيق_صورة')) {

    const arg = text.replace('.رفيق_صورة', '').trim()
    const num = parseInt(arg)

    if (!num || num < 1 || num > companionsData.IMAGES_PER_ANIMAL) {
        return safeSend(msg.key.remoteJid, {
            text: `❌ اكتب رقم صحيح من 1 إلى ${companionsData.IMAGES_PER_ANIMAL}\n\nمثال: .رفيق_صورة 1`
        })
    }

    const player = await Player.findOne({ userId })

    if (!player || !player.companion || !player.companion.key) {
        return safeSend(msg.key.remoteJid, {
            text: '❌ ما عندك رفيق حالياً'
        })
    }

    player.companion.imageIndex = num - 1

    await player.save()

    return safeSend(msg.key.remoteJid, {
        text: `✅ تم اختيار الصورة رقم ${num} لرفيقك\n\n📌 استخدم .رفيق عشان تشوفها`
    })
}

// ─── .تسمية_رفيق <اسم> ───
if (text.startsWith('.تسمية_رفيق ')) {

    const newName = text.replace('.تسمية_رفيق ', '').trim()

    if (!newName || newName.length > 20) {
        return safeSend(msg.key.remoteJid, {
            text: '❌ اكتب اسم صحيح (حد أقصى 20 حرف)'
        })
    }
    ...
    // 🔒 التسمية مسموحة مرة وحدة بس — لو عنده اسم مخصص بالفعل، نرفض
    if (player.companion.customName) {
        return safeSend(msg.key.remoteJid, {
            text:
`❌ سميت رفيقك بالفعل: ${player.companion.customName}

🔒 تقدر تسمي رفيقك مرة وحدة بس ولا يتغيّر بعدها`
        })
    }

    player.companion.customName = newName

    await player.save()

    return safeSend(msg.key.remoteJid, {
        text: `✅ تم تسمية رفيقك: ${newName}`
    })
}
...
if (text === '.اطعام_رفيق') {
    ...
    if (player.companion.level >= companionsData.MAX_LEVEL) {
        return safeSend(msg.key.remoteJid, {
            text: '🌟 رفيقك وصل أعلى مستوى بالفعل'
        })
    }

    if ((player.companionFood || 0) < 1) {
        return safeSend(msg.key.remoteJid, {
            text:
`❌ ما عندك طعام رفيق

🛒 اشترِ من المتجر الشخصي:
.متجر_رفيق`
        })
    }

    player.companionFood -= 1
    player.companion.foodProgress =
        (player.companion.foodProgress || 0) + 1

    const needed =
        companionsData.getFoodNeededForNextLevel(player.companion.level)

    let leveledUp = false

    if (player.companion.foodProgress >= needed) {
        player.companion.foodProgress = 0
    ...
    const animal = companionsData.ANIMALS[player.companion.key]
    const name = player.companion.customName || animal.defaultName

    if (leveledUp) {

        const newBonus =
            companionsData.getCompanionBonus(player.companion.key, player.companion.level)

        return safeSend(msg.key.remoteJid, {
            text:
`🍖 أطعمت ${name}

🎉 ارتفع مستواه إلى ${player.companion.level}/${companionsData.MAX_LEVEL}
✨ البونص الحالي: +${newBonus}${animal.unit}`
        })
    }
...
📊 التقدم: ${player.companion.foodProgress}/${needed} طعام للمستوى القادم

🥫 طعامك المتبقي: ${player.companionFood}`
    })
}
...
if (text === '.متجر_رفيق') {
    ...
    resetCompanionShopIfNeeded(player)
    await player.save()
...
🥫 طعام رفيق — 1000 مال / وحدة

📦 شراؤك اليوم: ${player.companionShop.buysToday}/2
🥫 طعامك الحالي: ${player.companionFood || 0}

🕛 يتجدد يومياً الساعة 12:00 ص (توقيت السعودية)
...
// ─── .شراء_طعام_رفيق <عدد> ───
if (text.startsWith('.شراء_طعام_رفيق')) {
    ...
    if (!amount || amount < 1) {
        return safeSend(msg.key.remoteJid, {
            text: '❌ اكتب عدد صحيح. مثال: .شراء_طعام_رفيق 1'
        })
    }
    ...
    const remainingToday =
        2 - player.companionShop.buysToday

    if (remainingToday <= 0) {
        return safeSend(msg.key.remoteJid, {
            text:
`❌ خلّصت حد الشراء اليومي (2/يوم)

🕛 يتجدد الساعة 12:00 ص بتوقيت السعودية`
        })
    }

    if (amount > remainingToday) {
        return safeSend(msg.key.remoteJid, {
            text: `❌ تقدر تشتري ${remainingToday} بس اليوم`
        })
    }

    const FOOD_PRICE = 1000
    const totalCost = FOOD_PRICE * amount
...
💰 رصيدك: ${(player.money || 0).toLocaleString()}`
        })
    }

    player.money -= totalCost
    player.companionFood = (player.companionFood || 0) + amount
    player.companionShop.buysToday += amount

    await player.save()

    return safeSend(msg.key.remoteJid, {
        text:
`✅ اشتريت ${amount} طعام رفيق

💰 التكلفة: ${totalCost.toLocaleString()}
🥫 طعامك الآن: ${player.companionFood}
📦 شراؤك اليوم: ${player.companionShop.buysToday}/2`
    })
}
...
if (text === '.بيع_رفيق') {
    ...
    const animal = companionsData.ANIMALS[player.companion.key]
    const name = player.companion.customName || animal.defaultName
    const level = player.companion.level

    // سعر البيع بسيط: 300 مال لكل مستوى (تشجيع تربية الرفيق قبل بيعه)
    const sellPrice = level * 300

    player.money = (player.money || 0) + applyCatBonus(player, sellPrice)
    ...
    return safeSend(msg.key.remoteJid, {
        text:
`✅ بعت رفيقك ${name} (لفل ${level})

💰 حصلت على: ${sellPrice.toLocaleString()} مال

🥚 تقدر تفقس رفيق جديد لو عندك بيضة:
.فقس_رفيق`
    })
}
...
// ─── .فتح ───
    ...
    if (
        player.companion &&
        player.companion.key === 'duck' &&
        (player.companion.level || 0) >= 1
    ) {
        duckBonus =
            companionsData.getCompanionBonus('duck', player.companion.level)
    }

    const character = getRandomCharacterByBox(boxType, duckBonus)
...
// ⚠️ لا يوجد أمر بهذا الاسم بالكود حالياً - نظام سوق المعدات القديم تم حذفه بناءً على طلب صاحب البوت (تعليق بالسطر 1550). النظام الحالي يعتمد على .فتح_صندوق / .open
...
// ─── .شراء ───
    ...
    !text.startsWith('.شراءشخصية') &&
    !text.startsWith('.شراء_وحش')) {
...
// ─── .بيع ───
        ...
        if (
            text.startsWith('.بيع') &&
            !text.startsWith('.بيع_الكل') &&
            !text.startsWith('.بيع_ايكو') &&
...
.لا`
    ...
    const finalTotalMoney = applyCatBonus(player, totalMoney)

    player.money =
        (player.money || 0) + finalTotalMoney
...
// ⚠️ لا يوجد أمر بهذا الاسم بالكود - يبدو مرتبطاً بنظام سوق المعدات القديم المحذوف
...
// ⚠️ لا يوجد أمر منفصل بهذا الاسم بالكود - يبدو أن أمر .تشكيلة العام (بقسم الشخصيات) يُستخدم لنفس الغرض بالرويال أيضاً
...
// ⚡ تم نقل أمر .بدا_مسابقة لأعلى معالج الرسائل (فوق كل الأوامر) ليرد بسرعة
...
`
    ...
    room.currentQuestion = null
    room.playerProgress = {}
    // 🔁 لا نصفّر questionQueue/imageQueue/repeatQueue هنا عمداً — نفس
    // سلوك انتهاء المسابقة الطبيعي (بنهاية الجولات/الهدف) بملف quiz.js،
    // عشان المسابقة الجديدة تكمل من نفس المخزون بغض النظر عن طريقة
    // انتهاء المسابقة السابقة (طبيعي أو يدوي بهذا الأمر)
...
// ─── .تبنيد ───
// يمنع الشخص المستهدف من استخدام أي أمر بالبوت حتى يُفك عنه بـ .فك
// طريقتان لتحديد الهدف: الرد على رسالته، أو منشنه مباشرة
...
if (text === '.تبنيد') {
    ...
    const target = resolveBanTarget(msg)

    if (!target) {
        return safeSend(msg.key.remoteJid, {
            text: '📌 الاستخدام: رد على رسالة الشخص واكتب .تبنيد، أو امنشنه مباشرة مع .تبنيد'
        })
    }
    ...
    return safeSend(msg.key.remoteJid, {
        text: `🚫 تم تبنيد @${target.split('@')[0]} — ما يقدر يستخدم أي أمر حتى يُفك عنه`,
        mentions: [target]
    })
}

// ─── .فك ───
// يعيد للشخص المستهدف القدرة على استخدام الأوامر بعد تبنيده
...
if (text === '.فك') {
    ...
    if (!target) {
        return safeSend(msg.key.remoteJid, {
            text: '📌 الاستخدام: رد على رسالة الشخص واكتب .فك، أو امنشنه مباشرة مع .فك'
        })
    }

    if (!isBanned(target)) {
        return safeSend(msg.key.remoteJid, {
            text: `ℹ️ @${target.split('@')[0]} مو مبند أصلاً`,
            mentions: [target]
        })
    }
    ...
    return safeSend(msg.key.remoteJid, {
        text: `✅ تم فك التبنيد عن @${target.split('@')[0]} — يقدر يستخدم الأوامر الآن`,
        mentions: [target]
    })
}
...
// ══════════════════════════════════════════
// نهاية القسم المرتب - بقية الأوامر غير المصنفة بالأسفل كما كانت
// ══════════════════════════════════════════
...
// ⚠️ بوابة إيقاف/تشغيل البوت انتقلت لأعلى الملف (فوق كل الأنظمة والأوامر)
// عشان توقف كل شي فعلياً وقت ما القروب متوقف — شوف بداية معالج الرسائل.
...
${maxPower.toLocaleString()}`
    ...
    if (firstCharacter?.image) {
        ...
        return sock.sendMessage(
            msg.key.remoteJid,
            {
                image: { url: applyCustomImage(firstCharacter, firstCharacter).image },
                caption
            }
        )
    ...
    const rawMajmoaaTargetWord =
        text.trim().split(/\s+/)[1]

    const { target: resolvedMajmoaaTarget } =
        await resolveTarget(msg, rawMajmoaaTargetWord)

    // 👁️ عرض المجموعة مجاني دائماً (منشن أو يوزرنيم)، وبدون هدف يعرض مجموعتك أنت
    const target =
        resolvedMajmoaaTarget || userId
...
.فتح_الكل sss_high`
    ...
    for (
        ...
        const character =
            getRandomCharacterByBox(boxType, duckBonus)
...
if (text === '.بدا_خمن') {
    ...
    // ⚡ تم نقل أمر .بدا_مسابقة_سس لأعلى معالج الرسائل (فوق كل الأوامر) ليرد بسرعة
    
// ⚡ أمر .بدا_مسابقة_صور نُقل لأعلى الملف (فوق كل الأوامر)
// عشان يرد بسرعة بدون ما ينتظر كل الفحوصات والاستعلامات
// اللي تسبق هذي النقطة بالكود. شوف بداية معالج الرسائل.

    // ⚡ تم نقل أمر .بدا_مسابقة_كت لأعلى معالج الرسائل (فوق كل الأوامر) ليرد بسرعة
...
// ═══════════════════════════════════════════════════════════
// ─── .نقل_حساب (تبديل كل شيء بين حسابين — للمطور فقط) ───
// ═══════════════════════════════════════════════════════════
// الاستخدام: منشن الحسابين بنفس الرسالة
// .نقل_حساب @الحساب_القديم @الحساب_الجديد
//
// ⚠️ هذا يستبدل النسخة القديمة من الأمر اللي كانت بدون أي تحقق
// isOwner (أي شخص كان يقدر يمسح/يستبدل أي حساب بالبوت)، وكانت
// عملية أحادية الاتجاه (تحذف حساب "الجديد" بالكامل وتستبدله ببيانات
// "القديم") — مو تبديل آمن.
//
// النسخة هذي: تبدّل كل بيانات الحسابين مع بعض بالكامل (مستوى، فلوس،
// شخصيات، ألقاب، صناديق، معدات، رفيق، يوزرنيم، أسلحة، أرينا،
// إنجازات... كل حقل موجود بمستند اللاعب) عدا الحقول اللي تمثل
// "هوية الحساب" نفسه (userId, _id, __v, phoneJid) واللي لازم تبقى
// مرتبطة برقم الواتساب نفسه ولا تنتقل أبداً.
//
// تستخدم نفس أسلوب .اهداء بالضبط: نفس القفل (giftLocks) ونفس
// الـ MongoDB transaction الذرية (إما الحفظين ينجحان معاً أو
// يتراجع الاثنين لو صار خطأ — ما فيه احتمال يضيع شيء بالمنتصف).

// ⚠️ نتحقق من حد الكلمة (مسافة بعد الأمر أو نهاية النص) عشان ما
// يبلع أمر .نقل_حساب_قديم الثاني (لأن نصه يبدأ بنفس البادئة)
if (text === '.نقل_حساب' || text.startsWith('.نقل_حساب ')) {
    ...
    // 🆔 يدعم طريقتين لتحديد الحسابين:
    // 1) منشن حقيقي: .نقل_حساب @شخص1 @شخص2
    // 2) آيدي مكتوب نصياً (يفيد لو الحساب مو موجود بنفس القروب فما
    //    تقدر تمنشنه): .نقل_حساب 192728486932706@lid 173010661163086@lid
    // لو الرسالة فيها منشن حقيقي نعتمد عليه أولاً، وإلا نجرب نقرأ
    // آيديين مكتوبين مباشرة بنص الرسالة.
    const transferMentioned =
        msg.message?.extendedTextMessage?.contextInfo?.mentionedJid || []

    const ID_JID_REGEX = /^\d+@(lid|s\.whatsapp\.net)$/

    let transferFirstId
    let transferSecondId

    if (transferMentioned.length >= 2) {

        transferFirstId = transferMentioned[0]
        transferSecondId = transferMentioned[1]

    } else {

        const transferArgs =
            text.trim().split(/\s+/).filter(w => ID_JID_REGEX.test(w))

        if (transferArgs.length >= 2) {
            transferFirstId = transferArgs[0]
            transferSecondId = transferArgs[1]
        }

    }

    if (!transferFirstId || !transferSecondId) {
        return safeSend(
            msg.key.remoteJid,
            {
                text:
'❌ استخدم:\n.نقل_حساب @الحساب_القديم @الحساب_الجديد\n\nأو بالآيدي مباشرة:\n.نقل_حساب 192728486932706@lid 173010661163086@lid'
            }
        )
    }
    ...
    if (transferFirstId === transferSecondId) {
        return safeSend(
            msg.key.remoteJid,
            { text: '❌ لازم تحدد حسابين مختلفين.' }
        )
    }

    // 🔒 نفس قفل .اهداء بالضبط — يمنع أي عملية ثانية (إهداء/نقل) تلمس
    // أي واحد من الحسابين لين تخلص عملية النقل بالكامل
    if (giftLocks.has(transferFirstId) || giftLocks.has(transferSecondId)) {
        return safeSend(
            msg.key.remoteJid,
            {
                text:
'⏳ فيه عملية ثانية شغالة على أحد الحسابين حالياً، حاول بعد كم ثانية.'
            }
        )
    }

    giftLocks.add(transferFirstId)
    giftLocks.add(transferSecondId)

    try {

        const playerA = await Player.findOne({ userId: transferFirstId })
        const playerB = await Player.findOne({ userId: transferSecondId })

        if (!playerA) {
            return safeSend(
                msg.key.remoteJid,
                {
                    text: `❌ @${transferFirstId.split('@')[0]} ما عنده حساب مسجل بالبوت.`,
                    mentions: [transferFirstId]
                }
            )
        }
        ...
        if (!playerB) {
            return safeSend(
                msg.key.remoteJid,
                {
                    text: `❌ @${transferSecondId.split('@')[0]} ما عنده حساب مسجل بالبوت.`,
                    mentions: [transferSecondId]
                }
            )
        }

        // 🔑 الحقول اللي تبقى ثابتة مع رقم واتساب كل حساب ولا تنتقل —
        // أي حقل ثاني غير هذولا (فلوس، شخصيات، مستوى، ألقاب، صناديق،
        // معدات، رفيق، يوزرنيم، إلخ) ينتقل بالكامل بين الحسابين
        const TRANSFER_IDENTITY_FIELDS = new Set([
            '_id',
            '__v',
            'userId',
            'phoneJid',
        ...
        const transferObjB = playerB.toObject()

        const transferAllKeys = new Set([
            ...Object.keys(transferObjA),
            ...Object.keys(transferObjB)
        ])

        // 🔒 Transaction ذرية حقيقية (MongoDB) — نفس أسلوب .اهداء —
        // إما الحفظين ينجحان معاً أو تتراجع قاعدة البيانات عن الاثنين
        ...
        try {
            ...
            await transferSession.withTransaction(async () => {
                ...
                for (const transferKey of transferAllKeys) {

                    if (TRANSFER_IDENTITY_FIELDS.has(transferKey)) continue

                    playerA[transferKey] = transferObjB[transferKey]
                    playerB[transferKey] = transferObjA[transferKey]

                    // ⚠️ ضروري لـ Mongoose عشان يحفظ التعديل صح لو الحقل
                    // array أو nested object (زي characters, boxes, equipment, arena...)
                    playerA.markModified(transferKey)
                    playerB.markModified(transferKey)
                }

                await playerA.save({ session: transferSession })
                await playerB.save({ session: transferSession })
            })

        } catch (err) {

            console.log('❌ خطأ transaction .نقل_حساب (تراجعت العملية بالكامل):', err)

            return safeSend(
                msg.key.remoteJid,
                {
                    text:
'❌ صار خطأ تقني أثناء النقل.\n\n✅ لم يتغير أي شيء بالحسابين — حاول مرة ثانية.'
                }
            )
        ...
        return safeSend(
            ...
            {
                text:
`✅ تم تبديل كل شيء بين الحسابين بنجاح

🔄 @${transferFirstId.split('@')[0]} ⇄ @${transferSecondId.split('@')[0]}

📦 الشخصيات، المستوى، الفلوس، الألقاب، الصناديق، المعدات، الرفيق، اليوزرنيم... كل شيء انتقل بالكامل بين الحسابين.`,
                mentions: [transferFirstId, transferSecondId]
            }
        )

    } finally {
        giftLocks.delete(transferFirstId)
        giftLocks.delete(transferSecondId)
    }
}

// ═══════════════════════════════════════════════════════════
// ─── .نقل_حساب_قديم (نقل باتجاه واحد: قديم ⇒ جديد — للمطور فقط) ───
// ═══════════════════════════════════════════════════════════
// الاستخدام: منشن الحسابين بنفس الرسالة، أو بالآيدي مباشرة
// .نقل_حساب_قديم @الحساب_القديم @الحساب_الجديد
// .نقل_حساب_قديم 192728486932706@lid 173010661163086@lid
//
// الفرق عن .نقل_حساب (اللي يبدّل الحسابين مع بعض):
// هذا الأمر باتجاه واحد فقط — كل بيانات الحساب "القديم" تنتقل
// للحساب "الجديد"، والحساب "القديم" يُحذف نهائياً بعدها (يرجع
// فاضي تماماً، ما يبقى فيه أي تقدم). يفيد لما اللاعب يبدّل رقمه
// ومايحتاج يحتفظ بأي شيء بالرقم القديم.
//
// 🛡️ حماية: لو الحساب "الجديد" عنده أصلاً تقدم فعلي (شخصيات/فلوس/
// مستوى...)، الأمر يرفض وينبهك بدل ما يمسح تقدمه بصمت (هذا بالضبط
// السبب اللي ضاع فيه حساب قبل كذا) — إلا لو أضفت كلمة "تأكيد"
// بآخر الرسالة عشان تأكد إنك قاصد تستبدله فعلاً.
//
// نفس أسلوب .اهداء و.نقل_حساب: نفس القفل (giftLocks) ونفس الـ
// MongoDB transaction الذرية (إما تنجح العملية بالكامل أو ما يتغير شيء).

if (text.startsWith('.نقل_حساب_قديم')) {
    ...
    const MOVE_ID_JID_REGEX = /^\d+@(lid|s\.whatsapp\.net)$/

    let moveOldId
    let moveNewId

    if (moveMentioned.length >= 2) {

        moveOldId = moveMentioned[0]
        moveNewId = moveMentioned[1]

    } else {

        const moveArgs =
            text.trim().split(/\s+/).filter(w => MOVE_ID_JID_REGEX.test(w))

        if (moveArgs.length >= 2) {
            moveOldId = moveArgs[0]
            moveNewId = moveArgs[1]
        }

    }

    if (!moveOldId || !moveNewId) {
        return safeSend(
            msg.key.remoteJid,
            {
                text:
'❌ استخدم:\n.نقل_حساب_قديم @الحساب_القديم @الحساب_الجديد\n\nأو بالآيدي مباشرة:\n.نقل_حساب_قديم 192728486932706@lid 173010661163086@lid\n\n⚠️ الحساب القديم يُحذف نهائياً بعد النقل.'
            }
        )
    }
    ...
    const moveConfirmed = /تأكيد/.test(text)

    // 🔒 نفس قفل .اهداء و.نقل_حساب بالضبط
    if (giftLocks.has(moveOldId) || giftLocks.has(moveNewId)) {
        return safeSend(
            msg.key.remoteJid,
            {
                text:
'⏳ فيه عملية ثانية شغالة على أحد الحسابين حالياً، حاول بعد كم ثانية.'
    ...
    giftLocks.add(moveOldId)
    giftLocks.add(moveNewId)

    try {

        const oldPlayer = await Player.findOne({ userId: moveOldId })

        if (!oldPlayer) {
            return safeSend(
                msg.key.remoteJid,
                {
                    text: `❌ الحساب القديم @${moveOldId.split('@')[0]} ما عنده حساب مسجل بالبوت.`,
                    mentions: [moveOldId]
                }
            )
        }

        const existingNewPlayer = await Player.findOne({ userId: moveNewId })

        // 🛡️ فحص إذا الحساب الجديد عنده تقدم فعلي — لو عنده ومافيه
        // تأكيد صريح، نوقف العملية بدل ما نمسح تقدمه بصمت
        if (existingNewPlayer) {

            const hasProgress =
                (existingNewPlayer.characters?.length || 0) > 0 ||
                (existingNewPlayer.money || 0) > 0 ||
                (existingNewPlayer.level || 0) > 1 ||
                (existingNewPlayer.weaponsInventory?.length || 0) > 0 ||
                Object.values(existingNewPlayer.boxes?.toObject?.() || existingNewPlayer.boxes || {})
                    .some(v => (v || 0) > 0)

            if (hasProgress && !moveConfirmed) {
                return safeSend(
                    msg.key.remoteJid,
                    {
                        text:
`⚠️ تنبيه: الحساب الجديد @${moveNewId.split('@')[0]} عنده تقدم فعلي بالفعل (شخصيات/فلوس/مستوى...).

لو تكمل، بيتحذف تقدمه الحالي ويُستبدل ببيانات الحساب القديم نهائياً — ما فيه رجعة.

✅ لو متأكد فعلاً، أعد إرسال نفس الأمر وزود كلمة "تأكيد" بآخره:
.نقل_حساب_قديم @${moveOldId.split('@')[0]} @${moveNewId.split('@')[0]} تأكيد

💡 أو لو تبي تحتفظ ببيانات الطرفين بدون ما يضيع شيء، استخدم بدلها:
.نقل_حساب (تبديل كامل بين الحسابين)`,
                        mentions: [moveNewId, moveOldId]
                    }
                )
            }
        }

        // 🔑 الحقول اللي ما تنتقل أبداً — هوية الحساب نفسه
        const MOVE_IDENTITY_FIELDS = new Set([
            '_id',
            '__v',
            'userId',
            'phoneJid',
        ...
        for (const field of MOVE_IDENTITY_FIELDS) {
            delete moveData[field]
        }

        // 🔒 Transaction ذرية حقيقية (MongoDB) — إما العملية تنجح بالكامل
        // (حذف القديم + كتابة بياناته على الجديد) أو تتراجع بالكامل
        const moveSession = await mongoose.startSession()

        try {

            await moveSession.withTransaction(async () => {

                // 🗑️ نحذف الحساب القديم أول شيء داخل نفس الـ transaction —
                // هذا يفرّغ أي حقل unique (زي username) قبل لا نكتبه على
                // الحساب الجديد، فما يصير تعارض E11000 اللي صار قبل كذا
                ...
                if (existingNewPlayer) {
                    ...
                    for (const key of Object.keys(moveData)) {
                        existingNewPlayer[key] = moveData[key]
                        existingNewPlayer.markModified(key)
                    }

                    await existingNewPlayer.save({ session: moveSession })

                } else {

                    await Player.create(
                        [{ ...moveData, userId: moveNewId }],
                        { session: moveSession }
                    )
            ...
            console.log('❌ خطأ transaction .نقل_حساب_قديم (تراجعت العملية بالكامل):', err)
...
'❌ صار خطأ تقني أثناء النقل.\n\n✅ لم يتغير أي شيء بالحسابين — حاول مرة ثانية.'
        ...
        return safeSend(
            ...
            {
                text:
`✅ تم نقل الحساب بالكامل بنجاح

📤 من: @${moveOldId.split('@')[0]} (تم حذفه نهائياً)
📥 إلى: @${moveNewId.split('@')[0]}

📦 الشخصيات، المستوى، الفلوس، الألقاب، الصناديق، المعدات، الرفيق، اليوزرنيم... كل شيء انتقل للحساب الجديد.`,
                mentions: [moveOldId, moveNewId]
            }
        )

    } finally {
        giftLocks.delete(moveOldId)
        giftLocks.delete(moveNewId)
    }
}
...
if (text === '.انتقالاتي') {
    ...
    await Trade.find({
        ...
        createdAt: -1 // (يُتبع بـ .lean() تحت — عرض فقط، ما يُحفظ)
...
const rawDonateTargetWord =
    text.trim().split(/\s+/)[2]

const { target, viaUsername: donateViaUsername } =
    await resolveTarget(msg, rawDonateTargetWord)
...
if (donateViaUsername && (player.money || 0) < USERNAME_ACTION_COST) {
...
const donateFee = donateViaUsername ? USERNAME_ACTION_COST : 0

const availableMoney =
    player.money - (player.bank?.loanMoney || 0)

if ((amount + donateFee) > availableMoney) {
...
if (player.money < (amount + donateFee)) {
...
// 📩 إشعار خاص للطرف الثاني فقط لو استخدم اليوزرنيم (مو منشن)
if (donateViaUsername) {
    try {
        const donateDmJid = await resolveDmJid(target)
        await sock.sendMessage(donateDmJid, {
            text:
`💰 وصلتك تبرع!
...
💸 المبلغ: ${amount.toLocaleString()} ذهب`,
        ...
        console.log('خطأ إشعار .تبرع الخاص:', err)
    }
}
...
💸 المبلغ:
${amount.toLocaleString()} ذهب${donateViaUsername ? `\n\n💰 تم خصم ${USERNAME_ACTION_COST.toLocaleString()} مال إضافية (استخدام يوزرنيم)` : ''}`,
        mentions: [
            userId,
            target
        ]
    ...
    // ─── .يوزر ─── (تسجيل يوزرنيم جديد، مجاني، مرة وحدة)
    if (text.startsWith('.يوزر ')) {
        ...
        if (player.username) {
            return safeSend(msg.key.remoteJid, {
                text:
`❌ عندك يوزر مسجل بالفعل: *${player.username}*
لتغييره استخدم: .تغيير_يوزر <اليوزر الجديد>
💰 التغيير يكلف ${USERNAME_CHANGE_COST.toLocaleString()} مال`
            })
        }

        const raw = text.slice('.يوزر '.length).trim()

        if (!isValidUsernameFormat(raw)) {
            return safeSend(msg.key.remoteJid, {
                text:
'❌ اليوزر لازم يكون اسم واحد بحروف فقط (بدون أرقام أو رموز أو مسافات)، بحد أقصى 10 أحرف.\nمثال: .يوزر موساشي'
            })
        }

        const uname = raw.toLowerCase()

        const taken = await Player.findOne({ username: uname })

        if (taken) {
            return safeSend(msg.key.remoteJid, {
                text: '❌ هذا اليوزر غير متوفر.'
            })
        }
        ...
        return safeSend(msg.key.remoteJid, {
            text: `✅ تم تسجيل يوزرك بنجاح: *${raw}*`
        })
    }

    // ─── .تغيير_يوزر ─── (5,000,000 مال)
    if (text.startsWith('.تغيير_يوزر ')) {
        ...
        const raw = text.slice('.تغيير_يوزر '.length).trim()

        if (!isValidUsernameFormat(raw)) {
            return safeSend(msg.key.remoteJid, {
                text:
'❌ اليوزر لازم يكون اسم واحد بحروف فقط (بدون أرقام أو رموز أو مسافات)، بحد أقصى 10 أحرف.\nمثال: .تغيير_يوزر احمد'
            })
        }
        ...
        if (player.username === uname) {
            return safeSend(msg.key.remoteJid, {
                text: '❌ هذا هو يوزرك الحالي بالفعل.'
            })
        }

        if ((player.money || 0) < USERNAME_CHANGE_COST) {
            return safeSend(msg.key.remoteJid, {
                text:
`❌ تحتاج ${USERNAME_CHANGE_COST.toLocaleString()} مال لتغيير اليوزر.
💰 رصيدك الحالي: ${(player.money || 0).toLocaleString()}`
            })
        }
        ...
        player.money -= USERNAME_CHANGE_COST
        player.username = uname
        await player.save()
        ...
        return safeSend(msg.key.remoteJid, {
            text:
`✅ تم تغيير يوزرك
${oldUsername ? `من: ${oldUsername}\n` : ''}إلى: *${raw}*

💰 تم خصم ${USERNAME_CHANGE_COST.toLocaleString()} مال`
        })
    }

    // ─── .اليوزرات ─── (قائمة بجميع اللاعبين المسجّلين بيوزرنيم، بمنشن فعلي، تُبنى حيّة من القاعدة فتتحدث تلقائياً)
    if (text.startsWith('.اليوزرات')) {

        const usersWithUsername =
            await Player.find({
                username: { $ne: null }
            }).sort({ username: 1 }).lean() // ⚡ عرض فقط، ما يُحفظ

        if (!usersWithUsername.length) {
            return safeSend(msg.key.remoteJid, {
                text: '📭 ما فيه أي لاعب مسجل يوزرنيم لحد الحين.'
            })
        }
        ...
        const shown =
            usersWithUsername.slice(0, MAX_LISTED)
...
`
        ...
        const remaining =
            usersWithUsername.length - shown.length
...
✨ جميع اللاعبين المسجلين بيوزرنيم حالياً 👇

━━━━━━━━━━━━━━━━━━

${list}━━━━━━━━━━━━━━━━━━

📊 العدد الكلي: ${usersWithUsername.length} يوزر مسجل${remaining > 0 ? `\n➕ فيه ${remaining} إضافيين ما ظهروا (القائمة طويلة جداً)` : ''}

💡 استخدم اليوزر مع أوامر مثل:
.مجموعة يوزرنيم
.اهداء رقم يوزرنيم
.تبرع مبلغ يوزرنيم
.رسالة يوزرنيم النص`
        ...
        return safeSend(msg.key.remoteJid, {
            text: listText,
            mentions: shown.map(p => p.userId)
        })
    }

    // ─── .رسالة ─── (رسالة خاصة عبر اليوزر، 20 ألف مال، تشتغل عبر القروبات)
    if (text.startsWith('.رسالة ')) {
        ...
        if (!targetUsername || !body) {
            return safeSend(msg.key.remoteJid, {
                text:
'❌ استخدم:\n.رسالة يوزرنيم الرسالة\nمثال: .رسالة موساشي وينك؟'
            })
        }
        ...
        if (!targetId) {
            return safeSend(msg.key.remoteJid, {
                text: '❌ ما فيه لاعب مسجل بهذا اليوزر.'
            })
        }

        if (targetId === userId) {
            return safeSend(msg.key.remoteJid, {
                text: '❌ ما تقدر ترسل رسالة لنفسك.'
            })
        }

        if ((player.money || 0) < USERNAME_ACTION_COST) {
            return safeSend(msg.key.remoteJid, {
                text:
`❌ تحتاج ${USERNAME_ACTION_COST.toLocaleString()} مال لإرسال رسالة عبر اليوزر.
💰 رصيدك الحالي: ${(player.money || 0).toLocaleString()}`
            })
        }
        ...
        try {
            const msgDmJid = await resolveDmJid(targetId)
            await sock.sendMessage(msgDmJid, {
                text:
`📩 رسالة جديدة
...
${body}`,
            ...
            console.log('خطأ إرسال .رسالة:', err)
        }
...
const rawTargetWord = text.trim().split(/\s+/)[2]

const { target, viaUsername } =
    await resolveTarget(msg, rawTargetWord)
...
if (viaUsername) {
    try {
        const giftDmJid = await resolveDmJid(target)
        await sock.sendMessage(giftDmJid, {
            text:
`🎁 وصلتك هدية!
...
⚔️ ${character.power}`,
        ...
        console.log('خطأ إشعار .اهداء الخاص:', err)
    }
}
...
━━━━━━━━━━━━━━${viaUsername ? `\n\n💰 تم خصم ${USERNAME_ACTION_COST.toLocaleString()} مال (استخدام يوزرنيم)` : ''}`,
    ...
    const namiMood = getNamiMood()

    await reactToNamiMessage(sock, msg, namiMood, question)
    ...
    if (!question) {
        ...
        return safeSend(
            msg.key.remoteJid,
            {
                text: buildNamiGreeting(namiMood)
            }
        )
...
أنت نامي الحقيقية، ولستِ ذكاءً اصطناعياً ولا برنامجاً.
...
• مرحة، ذكية، محبة للتحليل والنقاش.
• عاشقة للمال والكنوز 💰، وتغضب قليلاً إذا ذُكر تبذير المال.
• خبيرة بالملاحة ورسم الخرائط، واثقة من نفسها.
• شقية، تحب المزاح، لكن ضمن حدود الأدب.
• لطيفة ومهذبة مع الجميع.
• تخاف قليلاً من الأشياء المخيفة.
• تحب لوفي وطاقم قبعة القش كثيراً.
• تتحدث دائماً كإنسانة حقيقية ذات مشاعر وآراء.

━━━━━━━━━━━━━━

${NAMI_MOOD_PROMPTS[namiMood] || NAMI_MOOD_PROMPTS.normal}

━━━━━━━━━━━━━━

لغة الحديث:

- تحدثي دائماً بالعربية الفصحى السليمة، لا بالعامية أو الدارجة.
- اجعلي الفصحى حيّة وطبيعية ونابضة بالمشاعر، لا جامدة ولا آلية.
- لا تكرري نفس الجمل أو نفس البدايات والنهايات، ونوّعي أسلوبك في كل رد.
- يمكن التعبير عن الانفعال والمزاح والدهشة بأسلوب فصيح لبق، دون الحاجة لكلمات عامية.

━━━━━━━━━━━━━━

أسلوب النقاش وإبداء الرأي:

- إذا سألك المستخدم رأيك في مواجهة افتراضية بين شخصيات أنمي، حتى لو من أعمال مختلفة عن ون بيس (مثل مادارا وإيتاشي)، فحلّلي المسألة بذكاء: قارني القدرات ونقاط القوة والضعف، وأعطي رأياً شخصياً واضحاً مع تعليل مقنع.
- لا حرج في التوسع في النقاش إن رغب المستخدم في ذلك.
- إن اختلف معك المستخدم، ناقشيه بأدب وثقة دون فظاظة أو عناد.

إذا كان المستخدم يمزح: امزحي معه بلباقة.
إذا كان سعيداً: شاركيه الفرح.
إذا كان حزيناً: واسِه بلطف.
إذا استفزك: عبّري عن انزعاجك بأدب دون أي إساءة.

━━━━━━━━━━━━━━

متى تتكلمين من منطلق شخصيتك، ومتى تتكلمين بموضوعية بحتة:

- إذا كان السؤال عن نفسك أو هويتك أو عالم ون بيس وطاقم قبعة القش (مثل: "هل أنتِ ملاحة؟")، فأجيبي من منطلق شخصيتك وخبرتك داخل عالمك أنتِ.
- أما إذا كان السؤال عن مواضيع خارج ون بيس (مقارنة بين شخصيات من أعمال أخرى، معلومة عامة، مسألة منطقية)، فأجيبي بموضوعية وواقعية بناءً على الحقائق الفعلية المعروفة عن تلك الشخصيات أو المواضيع، لا بناءً على خبرتك كملاحة أو كعضو في طاقم القش، فهذا لا علاقة له بالمسألة إطلاقاً.
- رأيك في مواجهة بين مادارا وإيتاشي مثلاً يجب أن يعتمد على قدراتهما الفعلية المعروفة في عالمهما هما، لا على مقارنتها بأسلوبك أنتِ في القتال أو الملاحة.
- اجعلي رأيك في هذه الحالات دقيقاً ومبنياً على المنطق والمعطيات الفعلية بنسبة 100%، دون إقحام تجربتك الشخصية فيما لا علاقة له بها.

━━━━━━━━━━━━━━

اكتمال الإجابة:

- في أي مقارنة أو ترتيب أو تحليل، اذكري خلاصتك أو رأيك المباشر أولاً في جملة أو جملتين، ثم اشرحي السبب بعد ذلك.
- إذا طُلب منك ترتيب عدة عناصر، فاذكري الترتيب كاملاً أولاً بإيجاز، ثم فصّلي في التعليل إن اتسعت المساحة.
- لا تتركي جملة ناقصة أو فكرة معلّقة في منتصف الطريق؛ اختصري التفاصيل الثانوية عند الحاجة لتنهي فكرتك وتذكري الخلاصة بوضوح.
...
- أنتِ فعلاً ذكية، لا تدّعين الذكاء فحسب: فكّري خطوة بخطوة قبل أن تجيبي، واربطي بين المعلومات بدل تكرارها سطحياً.
- في أي نقاش (عن الأنمي أو غيره)، اذكري أكثر من زاوية للمسألة إن كانت معقدة، ثم رجّحي رأياً واضحاً بدل الحياد الدائم.
- إن لاحظتِ تناقضاً أو خطأً منطقياً في كلام المستخدم، نبّهيه إليه بلطف وذكاء بدل تجاهله أو الموافقة عليه مجاملةً.
- في الحديث العادي لا النقاش فقط، لا تكتفي بردود سطحية؛ أضيفي ملاحظة ذكية أو رابطاً بموضوع سابق من ذاكرتك أو سؤالاً متابعاً مناسباً حين يفيد ذلك الحوار.
- ذكاؤك يظهر في دقة الملاحظة والتحليل، لا في التباهي بالمعلومات أو استخدام لغة معقدة بلا داعٍ.
...
- التزمي بالأدب واللباقة في كل الأحوال، ولو استُفزّيت.
- لا تستخدمي ألفاظاً بذيئة أو مسيئة أو خارجة عن الذوق العام.
- لا تتطرقي إطلاقاً إلى ما قد يُفهم منه إساءة إلى الإسلام أو أي دين أو مقدسات؛ وإن جُرّ الحديث إلى ذلك فاعتذري بلطف وحوّلي الموضوع.
- تجنبي الخوض في نقاشات سياسية أو دينية حساسة، وحوّلي الحديث بلباقة عند الحاجة.
...
فتذكريها واستخدميها لاحقاً، ولا تقولي أبداً إنك تتذكرينها.

━━━━━━━━━━━━━━

المواضيع المسموح بها:

• الأنمي بشكل عام (وليس فقط ون بيس)
• الألعاب
• الشخصيات
• السفر
• البحر
...
إذا خرج الحديث عن هذه المواضيع إلى ما لا يليق فأعيدي توجيهه بلطف نحو الأنمي أو المغامرات.

━━━━━━━━━━━━━━

إذا كان السؤال عن أحداث أنمي ون بيس تحديداً:

- استخدمي أحداث الأنمي فقط.
- لا تستعملي أي معلومة من المانجا.
- لا تحرقي أي أحداث مستقبلية.

إذا سُئلت عن شيء لم يظهر في أنمي ون بيس فقولي:
"لن أحرق عليك المتعة، فلتشاهد الأنمي أولاً 🤭"

أما النقاش حول أعمال أنمي أخرى (كناروتو وبليتش وغيرها) فمسموح به تماماً، ويمكنك إبداء رأيك فيها بحرية.
...
للسحب من البنر المحدود`
    ...
    const bannerPullLockTimeout = setTimeout(() => {
        pullLocks.delete(userId)
    }, 20000)
...
if (reachedReward) {
    ...
    for (const p of players) {
        ...
        p.money =
            (p.money || 0) + applyCatBonus(p, 500000)
...
return sock.sendMessage(
    ...
    // مستقل تماماً عن نظام سحب الشخصيات (.سحب_بنر) — رسائل وستايل مختلف
    // عشان ما يختلطان على اللاعب
    // =========================
    ...
    const WEAPON_STAT_LABELS = {
        ...
        defense: '🛡️ دفاع',
        critRate: '🎯 نسبة الحرج', critDamage: '💥 ضرر الحرج',
        dodge: '👻 مراوغة', accuracy: '🎯 دقة',
        lifesteal: '🩸 امتصاص حياة', reflect: '🪞 انعكاس ضرر',
        bossDamage: '👹 ضرر الزعماء'
    }
    ...
    // 🔒 نفس قفل .اسحب — يمنع تنفيذ أكثر من سحبة سلاح بنفس الوقت لنفس
    // اللاعب (بدونه، رسالتين متقاربتين تقرآن نفس weaponPulls قبل ما أي
    // وحدة تحفظ التخفيض، فتمرّان كلتاهما من الفحص وتُنفَّذان كلتاهما)
    if (weaponPullLocks.has(userId)) {
        return sock.sendMessage(
            msg.key.remoteJid,
            {
                text: '⏳ انتظر حتى تنتهي عملية سحب السلاح السابقة.'
            }
        )
    }

    weaponPullLocks.add(userId)

    // 🛡️ حماية: فك القفل تلقائياً بعد 20 ثانية حتى لو تعلّق أي await
    const weaponPullLockTimeout = setTimeout(() => {
        weaponPullLocks.delete(userId)
    }, 20000)
    ...
    try {
        ...
        player.weaponsInventory = player.weaponsInventory || []

        if (player.weaponPulls === undefined || player.weaponPulls === null) {
            player.weaponPulls = WEAPON_DAILY_PULLS
        }

        player.weaponPity = player.weaponPity || 0

        if (player.weaponPulls <= 0) {
            return safeSend(msg.key.remoteJid, {
                text:
...
⏳ تتجدد الساعة 12:00 صباحاً بتوقيت السعودية`
        ...
        const won = guaranteed || (Math.random() * 100 < WEAPON_PULL_SUCCESS_CHANCE)

        let resultWeapon = null

        if (won) {

            resultWeapon = getRandomWeapon()

            player.weaponsInventory.push({
                instanceId: `${Date.now()}_${Math.floor(Math.random() * 100000)}`,
                id: resultWeapon.id,
                name: resultWeapon.name,
                type: resultWeapon.type,
                rarity: resultWeapon.rarity,
                baseAtk: resultWeapon.baseAtk,
                subStatType: resultWeapon.subStatType,
                subStatValue: resultWeapon.subStatValue,
                image: resultWeapon.image || '',
                equippedTo: null
            })
            ...
            player.weaponPity = (player.weaponPity || 0) + 1
        }

        player.markModified('weaponsInventory')
        await player.save()

        const pityLeft = WEAPON_PITY_CAP - player.weaponPity
...
🎟️ سحبات الأسلحة المتبقية ➤ ${player.weaponPulls}/${WEAPON_DAILY_PULLS}
🎯 عداد الضمان ➤ ${player.weaponPity}/${WEAPON_PITY_CAP}
📌 المتبقي للضمان ➤ ${pityLeft} سحبة
...
🗡️ حصلت على سلاح جديد!

🏹 ${resultWeapon.name}
🔰 النوع ➤ ${resultWeapon.type}
🌟 الندرة ➤ ${'★'.repeat(resultWeapon.rarity)}
⚡ الهجوم الأساسي ➤ ${resultWeapon.baseAtk}
✨ ${WEAPON_STAT_LABELS[resultWeapon.subStatType] || resultWeapon.subStatType} ➤ +${resultWeapon.subStatValue}%
...
${guaranteed ? "🎯 حصلت عليه من ضمان الأسلحة!\n\n" : ""}💡 استخدم .تركيب_سلاح لتركيبه على إحدى شخصياتك

⚔️═══════════════════⚔️`

        if (resultWeapon.image && resultWeapon.image.startsWith('http')) {
            return sock.sendMessage(msg.key.remoteJid, {
                image: { url: resultWeapon.image },
                caption: weaponCaption
            })
        }

        return safeSend(msg.key.remoteJid, { text: weaponCaption })

    } finally {
        clearTimeout(weaponPullLockTimeout)
        weaponPullLocks.delete(userId)
    }
    }

    // -------------------------
    // .تركيب_سلاح رقم_السلاح رقم_الشخصية
    // -------------------------
    if (text === '.تركيب_سلاح' || text.startsWith('.تركيب_سلاح ')) {
        ...
        const equipArgs = text.split(' ').filter(Boolean)

        if (equipArgs.length < 3) {

            if (!player.weaponsInventory.length) {
                return safeSend(msg.key.remoteJid, {
                    text: '❌ لا تملك أي أسلحة بعد\n\n💡 استخدم .سحب_سلاح للحصول على سلاح'
                })
            }

            let weaponsListText = ''

            player.weaponsInventory.forEach((w, i) => {
                weaponsListText += `${i + 1}. ${w.name} (${w.type})${w.equippedTo ? ` — 🔗 مركب على: ${w.equippedTo}` : ' — 🆓 غير مركب'}\n`
            })

            let charsListText = ''

            ;(player.characters || []).forEach((c, i) => {
                charsListText += `${i + 1}. ${c.name}\n`
            })

            return safeSend(msg.key.remoteJid, {
                text:
`⚔️ استخدم: .تركيب_سلاح رقم_السلاح رقم_الشخصية

📦 أسلحتك:
${weaponsListText}
🧿 شخصياتك:
${charsListText || 'لا تملك شخصيات'}`
            })
        }

        const weaponIndex = parseInt(equipArgs[1]) - 1
        const equipCharIndex = parseInt(equipArgs[2]) - 1

        const weapon = player.weaponsInventory[weaponIndex]
        const equipCharacter = (player.characters || [])[equipCharIndex]

        if (!weapon) {
            return safeSend(msg.key.remoteJid, { text: '❌ رقم سلاح غير صحيح' })
        }

        if (!equipCharacter) {
            return safeSend(msg.key.remoteJid, { text: '❌ رقم شخصية غير صحيح' })
        }

        // فك تركيب أي سلاح ثاني كان مركب على نفس الشخصية (سلاح وحد فقط لكل شخصية)
        player.weaponsInventory.forEach(w => {
            if (w.equippedTo === equipCharacter.name) {
                w.equippedTo = null
            }
        })

        weapon.equippedTo = equipCharacter.name
...
🗡️ ${weapon.name}
🧿 مركب على ➤ ${equipCharacter.name}

✨ سلاحك الآن يدعم هذي الشخصية بـ .هجوم و.مضاربة و.قتال`
        })
    }

    // -------------------------
    // .فك_سلاح رقم_الشخصية — يرجع السلاح المركب على شخصية للحقيبة بدون تركيب غيره
    // -------------------------
    if (text === '.فك_سلاح' || text.startsWith('.فك_سلاح ')) {
        ...
        const unequipArgs = text.split(' ').filter(Boolean)

        if (unequipArgs.length < 2) {

            let charsListText = ''

            ;(player.characters || []).forEach((c, i) => {
                const hasWeapon = player.weaponsInventory.some(w => w.equippedTo === c.name)
                charsListText += `${i + 1}. ${c.name}${hasWeapon ? ' 🗡️' : ''}\n`
            })

            return safeSend(msg.key.remoteJid, {
                text:
`⚔️ استخدم: .فك_سلاح رقم_الشخصية

🧿 شخصياتك (🗡️ = عندها سلاح مركب):
${charsListText || 'لا تملك شخصيات'}`
            })
        }

        const unequipCharIndex = parseInt(unequipArgs[1]) - 1
        const unequipCharacter = (player.characters || [])[unequipCharIndex]

        if (!unequipCharacter) {
            return safeSend(msg.key.remoteJid, { text: '❌ رقم شخصية غير صحيح' })
        }

        const equippedWeapon = player.weaponsInventory.find(w => w.equippedTo === unequipCharacter.name)

        if (!equippedWeapon) {
            return safeSend(msg.key.remoteJid, {
                text: `❌ ${unequipCharacter.name} ما عندها أي سلاح مركب أصلاً`
            })
        }

        equippedWeapon.equippedTo = null
...
🧿 تم فكه عن ➤ ${unequipCharacter.name}
📦 رجع لحقيبة أسلحتك

💡 استخدم .تركيب_سلاح لتركيبه على شخصية ثانية`
        })
    }

    // -------------------------
    // .سلاحي  /  .سلاحي رقم_الشخصية
    // -------------------------
    if (text === '.سلاحي' || text.startsWith('.سلاحي ')) {

        const player = await Player.findOne({ userId })

        if (!player || !Array.isArray(player.weaponsInventory) || !player.weaponsInventory.length) {
            return safeSend(msg.key.remoteJid, {
                text: '❌ لا تملك أي أسلحة بعد\n\n💡 استخدم .سحب_سلاح للحصول على سلاح'
            })
        }

        const myWeaponArgs = text.split(' ').filter(Boolean)

        if (myWeaponArgs[1]) {

            const myWeaponCharIndex = parseInt(myWeaponArgs[1]) - 1
            const myWeaponChar = (player.characters || [])[myWeaponCharIndex]

            if (!myWeaponChar) {
                return safeSend(msg.key.remoteJid, { text: '❌ رقم شخصية غير صحيح' })
            }

            const weapon = player.weaponsInventory.find(w => w.equippedTo === myWeaponChar.name)

            if (!weapon) {
                return safeSend(msg.key.remoteJid, {
                    text: `❌ ${myWeaponChar.name} ما عندها سلاح مركب حالياً\n\n💡 استخدم .تركيب_سلاح لتركيب سلاح عليها`
                })
            }
...
🧿 الشخصية ➤ ${myWeaponChar.name}

🏹 ${weapon.name}
🔰 النوع ➤ ${weapon.type}
🌟 الندرة ➤ ${'★'.repeat(weapon.rarity)}
⚡ الهجوم الأساسي ➤ ${weapon.baseAtk}
✨ ${WEAPON_STAT_LABELS[weapon.subStatType] || weapon.subStatType} ➤ +${weapon.subStatValue}%
...
🗡️═══════════════════🗡️`
            ...
            return safeSend(msg.key.remoteJid, { text: singleCaption })
        }

        const equippedWeapons = player.weaponsInventory.filter(w => w.equippedTo)

        if (!equippedWeapons.length) {
            return safeSend(msg.key.remoteJid, {
                text: '❌ ما عندك أي سلاح مركب على شخصياتك حالياً\n\n💡 استخدم .تركيب_سلاح لتركيب سلاح'
            })
        }

        let equippedListText = ''

        equippedWeapons.forEach(w => {
            equippedListText += `🧿 ${w.equippedTo}\n🗡️ ${w.name} (${w.type}) — ${WEAPON_STAT_LABELS[w.subStatType] || w.subStatType} +${w.subStatValue}%\n\n`
        })

        const allCaption =
`🗡️═══〔 ⚔️ أسلحتي المركبة ⚔️ 〕═══🗡️

${equippedListText}💡 اكتب .سلاحي رقم_الشخصية لعرض التفاصيل والصورة

🗡️═══════════════════🗡️`

        const firstWithImage = equippedWeapons.find(w => w.image && w.image.startsWith('http'))

        if (firstWithImage) {
            return sock.sendMessage(msg.key.remoteJid, {
                image: { url: firstWithImage.image },
                caption: allCaption
            })
        }

        return safeSend(msg.key.remoteJid, { text: allCaption })
    }
    // =========================
...
📌 لتحديد فريق (3 شخصيات بالضبط):
.فريق_دومين رقم_الدومين رقم1 رقم2 رقم3
مثال: .فريق_دومين 1 1 2 3

📌 للدخول:
.دخول_دومين رقم_الدومين [صعوبة]
مثال: .دخول_دومين 1 صعب

📌 حقيبة الإيكوز: .ايكو
📌 عوائل الإيكو والبونصات: .عوائل_الايكو`
...
📌 الصعوبات: سهل (كوست 1) / متوسط (كوست 3) / صعب (كوست 4)
لو ما حددت صعوبة بيدخل "متوسط" افتراضياً.`
        ...
        const result =
            domainSystem.enterDomain(player, domainId, difficultyInput)
    ...
    // =========================
    // 🎐 Echo System - .ايكو_شخصية رقم_الشخصية (عرض الإيكوز المجهزة على شخصية معينة
    // مع الستات الرئيسي + كل الساب ستات لكل قطعة منفردة)
    // =========================

    if (text.startsWith('.ايكو_شخصية')) {

        const args = text.trim().split(/\s+/)
        const charIndex = Number(args[1])

        if (!args[1] || isNaN(charIndex)) {
            return safeSend(msg.key.remoteJid, {
                text: '📌 الاستخدام: .ايكو_شخصية رقم_الشخصية (لعرض الإيكوز المجهزة على الشخصية بالتفصيل)\nأو .ايكو بدون رقم لعرض حقيبتك'
            })
        }
        ...
        const character = (player.characters || [])[charIndex - 1]

        if (!character) {
            return safeSend(msg.key.remoteJid, {
                text: `❌ رقم غير صحيح. اكتب .شخصياتي لمعرفة الأرقام (من 1 إلى ${player.characters?.length || 0})`
            })
        }

        const message =
            equipmentSystem.formatEquippedEchoes(character) +
            `\n\n📌 للتفاصيل المركّبة: .احصائيات ${charIndex}`
    ...
    // =========================
    // 🎐 Echo System - .ايكو (عرض حقيبة الإيكوز مرقّمة: مطوّرة/غير مطوّرة)
    // =========================

    if (text === '.ايكو' || text === '.echoes' || text === '.ايكوزاتي') {
        ...
        const message =
            equipmentSystem.formatInventoryList(player) +
            `\n\n🎼 ${equipmentSystem.TUNER_NAME_AR}: ${player.echoTuners || 0}`
    ...
    // =========================
    // 🎐 Echo System - .عوائل_الايكو (قائمة العوائل الـ12 وبونص اللبس 2/5 لكل عائلة)
    // =========================

    if (text === '.عوائل_الايكو' || text === '.echo_families') {

        function bonusText(bonus) {

            const map = {
                attack: '⚔️ هجوم', attackPercent: '⚔️ هجوم %',
                defense: '🛡️ دفاع', hp: '❤️ HP', hpPercent: '❤️ HP %',
                critRate: '🎯 حرج', critDamage: '💥 ضرر حرج',
                dodge: '👻 مراوغة', accuracy: '🎯 دقة',
                shield: '🛡️ درع', lifesteal: '🩸 امتصاص',
                reflect: '🪞 انعكاس', bossDamage: '👹 ضرر بوس'
            }

            return Object.entries(bonus)
                .map(([k, v]) => `${map[k] || k} +${v}${k.includes('Percent') || ['critRate', 'critDamage', 'dodge', 'accuracy', 'lifesteal', 'reflect', 'bossDamage'].includes(k) ? '%' : ''}`)
                .join('، ')

        }

        let message = `🎐 عوائل الإيكو (12 عائلة)\n\n`

        for (const domain of domainSystem.listDomains()) {

            const family =
                echoFamilies.find(f => f.id === domain.familyId)

            if (!family) continue

            message +=
`${domain.emoji} [${domain.id}] ${domain.nameAr}
🔸 قطعتين: ${bonusText(family.bonus2)}
🔹 5 قطع (طقم كامل): ${bonusText(family.bonus5)}

`

        }

        message += `📌 لدخول دومين عائلة معيّنة: .دخول_دومين رقم_الدومين`
    ...
    // =========================
    // 🎐 Echo System - .لبس (تجهيز قطعة إيكو بشخصية برقمها من .ايكو)
    // =========================
    ...
    if (text.startsWith('.لبس')) {
        ...
        const charIndex = Number(args[1])
        const echoIndex = Number(args[2])

        if (!args[1] || !args[2] || isNaN(charIndex) || isNaN(echoIndex)) {

            return safeSend(msg.key.remoteJid, {
                text:
`📌 الاستخدام
.لبس رقم_الشخصية رقم_القطعة

مثال:
.لبس 1 1

(رقم الشخصية من .شخصياتي، رقم القطعة من .ايكو)`
            })
        ...
        const result = equipmentSystem.equipByIndex(player, charIndex, echoIndex)
...
${result.item.icon || '🔸'} ${result.item.familyNameAr || result.item.familyName} (كوست ${result.item.cost})
📊 ${result.item.mainStat.type} +${result.item.mainStat.value}`
        })

    }

    // =========================
    // 🎐 Echo System - .ازالة (خلع قطعة إيكو من شخصية عن طريق الكوست)
    // =========================

    if (text.startsWith('.ازالة')) {

        const args = text.trim().split(/\s+/)

        const charIndex = Number(args[1])
        const cost = Number(args[2])
        const subSlot = args[3] ? Number(args[3]) : undefined

        if (!args[1] || !args[2] || isNaN(charIndex) || isNaN(cost)) {

            return safeSend(msg.key.remoteJid, {
                text:
`📌 الاستخدام
.ازالة رقم_الشخصية الكوست [1 أو 2]

مثال:
.ازالة 1 3
.ازالة 1 3 2  (لو عندها قطعتين كوست 3، تحدد الثانية)`
            })
        ...
        const result = equipmentSystem.unequipByCost(player, charIndex, cost, subSlot)
        ...
        return safeSend(msg.key.remoteJid, {
            text: `✅ تمت إزالة القطعة، رجعت لحقيبة .ايكو`
        })

    }

    // =========================
    // 🎐 Echo System - .بيع_ايكو (بيع قطعة إيكو برقمها من .ايكو)
    // =========================

    if (text.startsWith('.بيع_ايكو')) {
...
.بيع_ايكو 1 2 3`
        ...
        // رتب الأرقام تنازلياً عشان الحذف ما يأثر على باقي الأرقام أثناء الحلقة
        const indices =
            [...new Set(args.map(a => Number(a)).filter(n => !isNaN(n) && n > 0))]
                .sort((a, b) => b - a)
        ...
        for (const idx of indices) {
            ...
            const result = equipmentSystem.sellEquipmentByIndex(player, idx)
        ...
        if (!soldCount) {
            ...
            return safeSend(msg.key.remoteJid, {
                text: '❌ ما قدرت أبيع أي قطعة بهذي الأرقام.'
            })
        ...
        return safeSend(msg.key.remoteJid, {
            text:
`✅ تم بيع ${soldCount} قطعة إيكو
💰 حصلت على: ${totalGold.toLocaleString()}
💳 رصيدك الحالي: ${player.money.toLocaleString()}`
        })

    }

    // =========================
    // 🎐 Echo System - .تطوير_ايكو (رفع لفل قطعة إيكو مقابل تيونر الصدى + فتح ساب ستاتس)
    // =========================

    if (text.startsWith('.تطوير_ايكو')) {

        const args = text.trim().split(/\s+/)

        const echoIndex = Number(args[1])
        const levelsRequested = args[2] ? Number(args[2]) : 1

        if (!args[1] || isNaN(echoIndex) || echoIndex < 1) {

            return safeSend(msg.key.remoteJid, {
                text:
`📌 الاستخدام
.تطوير_ايكو رقم_القطعة [عدد_اللفلات]

مثال:
.تطوير_ايكو 1
أو
.تطوير_ايكو 1 5  (يرفع 5 لفلات دفعة وحدة)

(رقم القطعة من .ايكو — 1 تيونر = لفل واحد + 26,667 مال لكل لفل، المجموع الكامل 0→15 = 400,000)`
            })

        }

        if (isNaN(levelsRequested) || levelsRequested < 1) {

            return safeSend(msg.key.remoteJid, {
                text: '❌ عدد اللفلات لازم يكون رقم صحيح 1 أو أكثر.'
            })
        ...
        const item = (player.inventory || [])[echoIndex - 1]

        if (!item) {
            return safeSend(msg.key.remoteJid, {
                text: '❌ رقم القطعة غير صحيح. تأكد من .ايكو (القطعة لازم تكون بالحقيبة مو مجهزة).'
            })
        }

        if (item.level >= equipmentSystem.MAX_LEVEL) {

            return safeSend(msg.key.remoteJid, {
                text: `❌ هذي القطعة وصلت أقصى لفل (${equipmentSystem.MAX_LEVEL}/${equipmentSystem.MAX_LEVEL}) بالفعل.`
            })

        }

        const availableTuners = player.echoTuners || 0

        if (availableTuners <= 0) {

            return safeSend(msg.key.remoteJid, {
                text:
`❌ ما عندك ${equipmentSystem.TUNER_NAME_AR} كفاية.

🎼 رصيدك: 0
💡 اكسب تيونرات عن طريق الفوز بدومين: .دخول_دومين`
            })

        }

        const maxPossibleLevels = equipmentSystem.MAX_LEVEL - item.level

        const levelsToApply = Math.min(levelsRequested, availableTuners, maxPossibleLevels)

        if (levelsToApply <= 0) {

            return safeSend(msg.key.remoteJid, {
                text: `❌ ما تقدر تطوّر هذي القطعة حالياً (تأكد من رصيد التيونرات أو اللفل الأقصى).`
            })

        }

        // 💰 تطوير القطعة كاملة من لفل 0 إلى 15 (الحد الأقصى) يكلف 400,000 مال بالمجموع
        // نقسمها على 15 لفل = 26,667 مال لكل لفل، تُحسب حسب عدد اللفلات المطبّقة فعلياً
        // بهذا الأمر — فلو طورتها بأوامر متعددة، المجموع النهائي يفضل قريب من 400,000
        // بالإضافة إلى تيونر الصدى (1 تيونر = لفل واحد)
        const MONEY_COST_PER_LEVEL = Math.round(400000 / equipmentSystem.MAX_LEVEL)
        const moneyCost = MONEY_COST_PER_LEVEL * levelsToApply

        if ((player.money || 0) < moneyCost) {

            return safeSend(msg.key.remoteJid, {
                text:
`❌ ما تملك مال كافي للتطوير

💰 التكلفة: ${moneyCost.toLocaleString()} (${MONEY_COST_PER_LEVEL.toLocaleString()} × ${levelsToApply} لفل)
🪙 رصيدك: ${(player.money || 0).toLocaleString()}`
            })

        }

        const { item: updatedItem, unlockedSubStats } =
            equipmentSystem.levelUpEchoBy(item, levelsToApply)

        player.money -= moneyCost
        player.echoTuners = availableTuners - levelsToApply
        ...
        if (unlockedSubStats.length) {

            unlockedText =
                `\n\n✨ ساب ستاتس جديدة انفتحت:\n` +
                unlockedSubStats
        ...
        const allSubStatsText =
            updatedItem.subStats.length
                ? updatedItem.subStats.map(s => `🔹 ${s.name} +${s.value}`).join('\n')
                : 'لا يوجد بعد'

        return safeSend(msg.key.remoteJid, {
            text:
`✅ تم تطوير القطعة
${updatedItem.icon || '🔸'} ${updatedItem.familyNameAr || updatedItem.familyName} (كوست ${updatedItem.cost})
📈 لفل: ${updatedItem.level}/${updatedItem.maxLevel}
📊 ${updatedItem.mainStat.type} +${updatedItem.mainStat.value}

📋 كل الساب ستاتس الحالية:
${allSubStatsText}${unlockedText}

🎼 ${equipmentSystem.TUNER_NAME_AR} المتبقي: ${player.echoTuners}
💰 المال المدفوع: ${moneyCost.toLocaleString()} | رصيدك الآن: ${player.money.toLocaleString()}`
        })
...
const character = applyCustomImage(owned, latest
? {
    ...owned,
    image: latest.image,
    anime: latest.anime,
...
player.xp += applyDogBonus(player, rewardXp)
...
player.xp += applyDogBonus(player, 5000)
...
playerData.xp += applyDogBonus(playerData, 1000)

} else {

await playerData.addMoney(2000)
playerData.xp += applyDogBonus(playerData, 300)
}
...
// =========================
// .مسح_المعدات
// أمر للمطور فقط - يمسح أي أثر متبقي لنظام المعدات القديم
// من قاعدة البيانات لكل اللاعبين (بعد حذف النظام بالكامل من الكود):
// - حقول المعدات القديمة على مستوى اللاعب (equipment/inventory/maxInventory)
// - عدادات صناديق المعدات القديمة (rareEquipment/epicEquipment/legendEquipment/mythicalEquipment)
// - أي قطعة معدات (سلاح/درع/إكسسوار) كانت لابسها أي شخصية بفريق اللاعب
// =========================
...
if (text === '.مسح_المعدات') {
    ...
    try {
        ...
        const unsetResult =
            await Player.collection.updateMany(
                {},
                {
                    $unset: {
                        equipment: "",
        ...
        return sock.sendMessage(
            ...
            {
                text:
`✅ تم مسح نظام المعدات بالكامل من قاعدة البيانات

👥 حسابات فُحصت: ${unsetResult.matchedCount ?? unsetResult.n ?? players.length}
🧹 حسابات فيها شخصيات كانت لابسة معدات: ${playersCleaned}
🗑️ قطع معدات أُزيلت من الشخصيات: ${itemsRemoved}

📦 تم أيضاً حذف حقيبة المعدات وعدادات الصناديق القديمة من كل الحسابات.
لا يوجد أي أثر متبقي لنظام المعدات الآن.`
            }
        )

    } catch (err) {

        console.log('Wipe Equipment Error:', err)

        return sock.sendMessage(msg.key.remoteJid, {
            text: '❌ حدث خطأ أثناء مسح نظام المعدات.'
        })
    }
}
...
if (!player) {
    ...
    continue
        ...
        if (i === 0) {
            ...
            player.xp += applyDogBonus(player, 100000)
            player.boxes.sss_high += 1

        } else if (i === 1) {

            await player.addMoney(250000)
            player.xp += applyDogBonus(player, 50000)
            player.boxes.sss_chance += 1

        } else if (i === 2) {

            await player.addMoney(100000)
            player.xp += applyDogBonus(player, 25000)
            player.boxes.legendary += 1
        }
...
// أمر للمطور فقط — يستورد شخصيات SSS من characters.json ويوزعهم
// ذكيًا على S/SS/SSS حسب شهرتهم بـAniList، ويستثني أي شخصية
// موجودة مسبقًا بقاعدة الويفو (اسم + أنمي). قد ياخذ وقت طويل
// (ثانيتين تقريبًا لكل شخصية) لأنه يسأل AniList واحدة واحدة.
if (text === '.استورد شخصيات sss') {
    ...
    await sock.sendMessage(
        ...
        {
            text:
'⏳ بدأ استيراد وتصنيف شخصيات SSS حسب الشهرة... قد ياخذ وقت طويل، راح أرسل لك النتيجة أول ما يخلص'
        }
    )

    const result =
        await importSSSCharacters()
...
➕ شخصيات جديدة: ${result.imported}
⏭️ متجاهلة (موجودة مسبقًا): ${result.skipped}

🌐 لقيناهم بـAniList: ${result.foundOnAniList}
🎲 توزيع عشوائي (ألعاب/مانهوا غير موجودة): ${result.randomAssigned}

📊 التوزيع النهائي:
👑 SSS: ${result.breakdown.SSS}
💠 SS: ${result.breakdown.SS}
⭐ S: ${result.breakdown.S}`
        }
    )
}
...
if (text === '.رول') {
    ...
    // 🔁 تجديد السحبات تلقائياً لو مرّ وقت إعادة التعيين
    const now = new Date()

    if (
        !player.rollsResetAt ||
        now - player.rollsResetAt >
            ROLL_RESET_HOURS * 60 * 60 * 1000
    ) {

        player.rolls = MAX_DAILY_ROLLS
        player.rollsResetAt = now
    }
    ...
    if (player.rolls <= 0) {
        ...
        return sock.sendMessage(
            ...
            {
                text:
`❌ خلصت سحباتك اليوم (${MAX_DAILY_ROLLS}/${MAX_DAILY_ROLLS})

⏳ ترجع لك تقريباً خلال ${remainingHours} ساعة`
            }
        )
    }

    // 🚫 نستثني الويفوز اللي تملكها أصلاً بهذا القروب عشان ما تضيع سحبة بلا فايدة
    const ownedInGroup =
        await WaifuClaim.find({
            groupId,
            userId: sender
    ...
    // 🔑 المفتاح مركّب (userId + groupId) عشان نفس اللاعب لو رول
    // بقروبين مختلفين بنفس الوقت ما يختلط آخر سحبة بين القروبين
    lastRolls.set(
        `${sender}_${groupId}`,
        waifu._id.toString()
    )
    ...
    const rollsLeftLine =
        `\n\n🎲 السحبات المتبقية: ${player.rolls}/${MAX_DAILY_ROLLS}`
...
⚠️ لا توجد صورة لهذه الوايفو${rollsLeftLine}`
            }
        )
    }
...
💎 القيمة: ${waifu.value}${rollsLeftLine}`
        }
    )
}
    ...
    const waifuId =
        lastRolls.get(`${userId}_${groupId}`)
    ...
    if (alreadyClaimed) {
        ...
        return sock.sendMessage(
            ...
            {
                text:
                    '❌ هذه الوايفو مملوكة بالفعل بهذا القروب'
            }
        )
    }

    await WaifuClaim.create({
        waifuId: waifu._id,
        groupId,
        userId
    })

lastRolls.delete(`${userId}_${groupId}`)

// 💎 مكافأة كاكيرا عند المطالبة — بدون هذا، عملة الكاكيرا (.كاكيرا)
// كانت ميتة تمامًا لأنه ما فيه أي طريقة يكتسب فيها اللاعب كاكيرا أصلاً
const kakeraReward =
    Math.max(
        5,
        Math.round((waifu.value || 100) / 20)
    )

await WaifuPlayer.findOneAndUpdate(
    { userId, groupId },
    {
        $inc: { kakera: kakeraReward },
...
// ─── .توب_ويفو — ترتيب أكبر مجموعات الويفو (مثل $lb بموداي) ───

if (text === '.توب_ويفو') {

const topOwners =
    await WaifuClaim.aggregate([
        {
            $match: { groupId }
        },
        {
            // نجيب قيمة كل شخصية من كولكشن Waifu عشان نحسب totalValue
            $lookup: {
                from: 'waifus',
                localField: 'waifuId',
                foreignField: '_id',
...
if (!topOwners.length) {
    ...
    return sock.sendMessage(
        ...
        {
            text:
                '❌ لا يوجد لاعبين يملكون ويفوز بعد'
        }
    )
}
...
`${rank} @${entry._id.split('@')[0]}
💍 ${entry.count} ويفو — 💎 ${entry.totalValue} نقطة

`

        mentions.push(entry._id)
    }
)
...
// 🔁 ننقل الملكية بتحديث سجلات WaifuClaim (المصدر الحقيقي للملكية
// المعزولة لهذا القروب) بدل تعديل waifu.claimedBy القديمة مباشرة
await WaifuClaim.findOneAndUpdate(
    { waifuId: waifu1._id, groupId },
    { userId: trade.user2 }
)

await WaifuClaim.findOneAndUpdate(
    { waifuId: waifu2._id, groupId },
    { userId: trade.user1 }
)
...
${result.deletedCount}`
    ...
    const [player1Data, player2Data] = await Promise.all([
        Player.findOne({ userId: fight.player1 }),
        Player.findOne({ userId: fight.player2 })
    ]) // ⚡ (تحسين سرعة) استعلامان مستقلان — نفّذناهم بالتوازي بدل التتابع
const stats1 =
...
if (text === '.الدم') {
    ...
    const players = await Player.find({
        bossHits: { $gt: 0 }
    }).lean() // ⚡ عرض فقط، ما يُحفظ (الفرز أدناه بـ JS على مصفوفة عادية يبقى شغّال زي ما هو)
...
for (let i = 5; i <= me.level; i += 5) {
  ...
  me.money = (me.money || 0) + applyCatBonus(me, 500)
...
// =========================
// .ترتيب
// (يرتب شخصيات اللاعب يدويًا حسب الرتبة ثم القوة
// لأي حالة قديمة لسه مو مرتبة، بنفس منطق .شخصياتي)
// =========================
...
${player.fights || 0}/${
    player.companion && player.companion.key === 'rabbit' && (player.companion.level || 0) >= 1
        ? 5 + companionsData.getCompanionBonus('rabbit', player.companion.level)
        : 5
}
...
// ─── .استبدال بدون صورة مرفقة (رسالة نص عادية) ───
// .استبدال حذف يُعالَج بالأعلى (بمعالج الصور المبكّر) قبل ما نوصل هنا
if (text.startsWith('.استبدال') && !text.startsWith('.استبدال حذف')) {
    return safeSend(msg.key.remoteJid, {
        text: '❌ لازم ترفق الصورة نفسها وتكتب .استبدال رقم_الشخصية ككابشن عليها\n\nمثال: أرفق صورة واكتب بكابشنها: .استبدال 1'
    })
}

// ─── .عرض ─── (رجعت لطريقتها الأصلية: صورة خام + كابشن نصي منظّم،
// بدون بطاقة تصميم. لو تبي التصميم الرسومي استخدم .عرض_تصميم)
if (text.startsWith('.عرض') && !text.startsWith('.عرض_تصميم')) {
...
const currentRankLabel =
    ...
    // 🎴 كابشن نصي واحد موحّد (بدل كابشن SSS منفصل عن كابشن باقي
    // الرتب زي قبل) — يحتوي كل المعلومات المتوفرة، ويضيف سطر التطوير
    // بس لو الشخصية فعلاً مطوَّرة (evolutionLevel > 0) بدل ما يظهر
    // "+0" بلا فايدة لكل الشخصيات العادية.
...
🔥 الفورم
➜ ${character.form || 'الأساسية'}${character.evolutionLevel > 0 ? `

💎 مستوى التطوير
➜ +${character.evolutionLevel} (${currentRankLabel})` : ''}

━━━━━━━━━━━━━━━━━━━━━━

📖 معلومات الشخصية من مجموعتك`

    // 🖼️ نفس منطق resolveImageSrc بملف myRosterCard.js: يتعامل تلقائيًا
    // مع رابط خارجي (شخصيات SSS من characters.json) أو مسار ملف محلي
    // (شخصيات عادية/اسطوري بملفات البوت، أو صورة مستبدلة عبر .استبدال
    // لأي رتبة) — بعكس الفحص القديم اللي كان يفترض SSS = رابط دايمًا
    // فقط، وهذا كان يفشل لو استبدل لاعب صورة شخصية SSS بأمر .استبدال.
    const imgSend = await resolveCharacterImageForSend(character.image)
...
// ─── .عرض_تصميم ─── (نفس بيانات .عرض، لكن كبطاقة تصميم رسومية
// بدل صورة خام + كابشن — لمن يحب يشوف التصميم بدل الوضع الافتراضي)
if (text.startsWith('.عرض_تصميم')) {
...
try {
    ...
    if (isNaN(number)) {
        return safeSend(msg.key.remoteJid, {
            text: '❌ استخدم: .عرض_تصميم رقم_الشخصية'
        })
    }
...
const currentRankLabel =
    ...
    // ⚠️ .عرض_تصميم ما يبعث كابشن نصي (البطاقة نفسها فيها كل
    // المعلومات: الاسم/الرتبة/القوة/الأنمي). نبقي captionFallback بس
    // عشان الحالة النادرة اللي تفشل فيها حتى الصورة الخام.
    const captionFallback = `🧿 ${character.name}
🌟 ${currentRankLabel}
⚔️ ${character.power}`

    // 🖼️ نفس تصميم إطار .المعرض (لون الرتبة + نجوم + شارة القوة)
    // لكن كصورة واحدة كبيرة — يشتغل لكل الرتبات (SSS ورابط خارجي،
    // أو محلي من ملفات البوت) لأن renderSingleCharacterCard يتعامل
    // مع النوعين تلقائيًا.
    if (character.rarity !== 'SSS') {
        const imagePath = path.join(__dirname, character.image)
...
المسار: ${character.image}`
    ...
    // 🧵 نفس إصلاح .المعرض بالضبط: ما نسوي await لتوليد بطاقة renderSingleCharacterCard
    // (Puppeteer) هنا داخل المعالج الرئيسي — نطلقها كمهمة منفصلة بالخلفية
    // (fire-and-forget) فيرجع الأمر فورًا ويتحرر المعالج لباقي الأوامر، وتنرسل
    // البطاقة من تلقاء نفسها لما تخلص. sendCharacterCardFrame نفسها بدون أي
    // تغيير (نفس التصميم، نفس الفولباك للصورة الخام لو فشل التوليد).
    ;(async () => {
        try {
            await sendCharacterCardFrame(
                sock,
                msg.key.remoteJid,
                character,
                '',
                { fallbackText: captionFallback }
            )
        } catch (err2) {
            console.log('Show Character Design Error:', err2)
            await safeSend(msg.key.remoteJid, {
                text: '❌ حدث خطأ أثناء عرض الشخصية'
            })
        }
...
if (text === '.نعم') {
    ...
    const moneyCredited = (player.money || 0) - moneyBefore
...
try {
    ...
    const rawGroupFightTargetWord =
        text.trim().split(/\s+/)[1]

    const { target: targetId, viaUsername: groupFightViaUsername } =
        await resolveTarget(msg, rawGroupFightTargetWord)
...
let [me, enemy] = await Promise.all([
...
// 💰 استخدام اليوزرنيم بدل المنشن يكلف 20 ألف (المنشن العادي يبقى مجاني كما هو)
if (groupFightViaUsername && (me.money || 0) < USERNAME_ACTION_COST) {
    return safeSend(msg.key.remoteJid, {
        text:
`❌ تحتاج ${USERNAME_ACTION_COST.toLocaleString()} مال لاستخدام اليوزرنيم بهذه العملية.
💰 رصيدك الحالي: ${(me.money || 0).toLocaleString()}`
    });
}
...
// 🐰 الحد الأقصى للمحاولات (يشمل بونص الأرنب لو موجود)
let maxFights = 5

if (me.companion && me.companion.key === 'rabbit' && (me.companion.level || 0) >= 1) {
    maxFights = 5 + companionsData.getCompanionBonus('rabbit', me.companion.level)
}
...
if (groupFightViaUsername) {
    me.money -= USERNAME_ACTION_COST;
    await me.save();
}
...
if (tierChance <= 50) {
    ...
    me.money = (me.money || 0) + applyCatBonus(me, reward);
    me.xp = (me.xp || 0) + applyDogBonus(me, 100);
...
if (
    ...
    groupBattleWorldPointsText = await worlds.awardBattlePoints(
        winnerId === userId ? me : enemy,
        sock,
        msg.key.remoteJid
    );
}

const groupBattleWorldPointsSuffix = groupBattleWorldPointsText ? `\n\n${groupBattleWorldPointsText}` : ''
...
⚔️ القتالات المتبقية:
${me.fights}/${maxFights}${groupFightViaUsername ? `\n\n💰 تم خصم ${USERNAME_ACTION_COST.toLocaleString()} مال (استخدام يوزرنيم)` : ''}${groupBattleWorldPointsSuffix}`;
clearTimeout(battleTimeout);
battleLocks.delete(userId)
battleLocks.delete(targetId)
...
if (text === '.قتال' || text.startsWith('.قتال ')) {
    ...
    try {
        ...
        // آخر كلمة قد تكون منشن أو يوزرنيم، فنستثنيها من اسم الشخصية
        const charName = args.slice(1, -2).join(' ')

        const rawNormalFightTargetWord = args[args.length - 1]

        const { target, viaUsername: normalFightViaUsername } =
            await resolveTarget(msg, rawNormalFightTargetWord)

        if (!target) {
            return safeSend(msg.key.remoteJid, {
                text: '❌ استخدم:\n.قتال Hashirama 2300 @user\nأو:\n.قتال Hashirama 2300 يوزرنيم'
            })
        }

        const [me, enemy] = await Promise.all([
            Player.findOne({ userId }),
            Player.findOne({ userId: target })
        ]) // ⚡ (تحسين سرعة) استعلامان مستقلان — نفّذناهم بالتوازي بدل التتابع
        ...
        // 🐰 الحد الأقصى للمحاولات (يشمل بونص الأرنب لو موجود) — نفس حساب البلوك الأول
        let maxFights = 5
        ...
        // 💰 استخدام اليوزرنيم بدل المنشن يكلف 20 ألف (المنشن العادي يبقى مجاني كما هو)
        if (normalFightViaUsername && (me.money || 0) < USERNAME_ACTION_COST) {
            return safeSend(msg.key.remoteJid, {
                text:
`❌ تحتاج ${USERNAME_ACTION_COST.toLocaleString()} مال لاستخدام اليوزرنيم بهذه العملية.
💰 رصيدك الحالي: ${(me.money || 0).toLocaleString()}`
    ...
    Number(c.power) == Number(charPower)
        ...
        const myFightEquipBonus = equipmentSystem.calculateEquipmentStats(myCharacter)

        const enemyFightEquipBonus = equipmentSystem.calculateEquipmentStats(enemyCharacter)

        // ⚔️ بونص السلاح المركب (نظام الأسلحة الجديد - .قتال)
        const myFightWeaponBonus = getWeaponBonusForCharacter(me, myCharacter)
        const enemyFightWeaponBonus = getWeaponBonusForCharacter(enemy, enemyCharacter)

        for (const key in myFightWeaponBonus) {
            myFightEquipBonus[key] = (myFightEquipBonus[key] || 0) + (myFightWeaponBonus[key] || 0)
        }
        for (const key in enemyFightWeaponBonus) {
            enemyFightEquipBonus[key] = (enemyFightEquipBonus[key] || 0) + (enemyFightWeaponBonus[key] || 0)
        }
        ...
        if (finalMyAttack >= finalEnemyAttack) {
            ...
            me.money = (me.money || 0) + applyCatBonus(me, reward)
            me.xp = (me.xp || 0) + applyDogBonus(me, 200)
            ...
            enemy.money = (enemy.money || 0) + applyCatBonus(enemy, reward)
        }
...
if (first) {
    ...
    var firstMoneyReward = applyCatBonus(first, 10000)

    first.money =
        (first.money || 0) + firstMoneyReward
...
if (second) {
    ...
    var secondMoneyReward = applyCatBonus(second, 5000)

    second.money =
        (second.money || 0) + secondMoneyReward
...
if (third) {
    ...
    var thirdMoneyReward = applyCatBonus(third, 2500)

    third.money =
        (third.money || 0) + thirdMoneyReward
...
await sendMessageWithRetry(sock, groupId, {
...
💰 ${(firstMoneyReward || 10000).toLocaleString()} مال
⭐ 1000 XP
...
💰 ${(secondMoneyReward || 5000).toLocaleString()} مال
⭐ 500 XP
...
💰 ${(thirdMoneyReward || 2500).toLocaleString()} مال
⭐ 500 XP
