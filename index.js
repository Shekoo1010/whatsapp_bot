...
.then(async () => {
    ...
    // ═══════════════════════════════════════════════
    try {
        // 🩹 إصلاح: الـ index القديم على username كان غير sparse (تم
        // إنشاؤه قبل ما نضيف sparse:true بالأسفل). createIndex ما يقدر
        // يعدّل index موجود بنفس الاسم بخيارات مختلفة — يفشل بصمت
        // ويضل القديم غير sparse، وهذا سبب أخطاء E11000 المتكررة على
        // username: null لكل لاعب ما سجّل يوزر بعد. نحذفه هنا أول مرة
        // (لو موجود وغير sparse) قبل إعادة إنشائه صح تحت.
        ...
        if (usernameIdx && !usernameIdx.sparse) {
            ...
            console.log('🗑️ حذفنا username_1 index القديم (غير sparse) عشان نعيد بناءه صح')
        }
        ...
        await Promise.all([
            ...
            // unique + sparse مطابقين لتعريف username بالـ schema
            // (models/Player.js) — نفس سبب مشكلة userId بالضبط: الفهرس
            // التلقائي اللي ينشئه mongoose من unique:true/sparse:true
            // بالـ schema يختلف عن هذا الاستدعاء الصريح بدونهم
            Player.collection.createIndex({ username: 1 }, { unique: true, sparse: true }),
