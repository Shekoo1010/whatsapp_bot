...
// =========================
// 🖼️ استعادة صور .استبدال المفقودة بعد ريستارت البوت
// =========================
// كثير من الاستضافات تصفّر مجلد المشروع على القرص المحلي عند كل
// ريستارت/إعادة نشر، لكن قاعدة البيانات (Mongo) تفضل ثابتة. من هنا،
// كل صورة يحطها لاعب بأمر .استبدال تُخزَّن أيضًا كنسخة احتياطية
// (base64) جوا مستند اللاعب نفسه، وهذي الدالة تشتغل مرة وحدة عند
// إقلاع البوت: تدور على كل اللاعبين، ولو لقت customImage مسجّلة
// بالداتابيس لكن ملفها الفعلي مو موجود بالقرص، تعيد كتابته من النسخة
// الاحتياطية تلقائيًا — بدون أي تدخل من اللاعب. الصورة تفضل موجودة
// إلى أن يحذفها اللاعب نفسه بأمر .استبدال حذف.
async function restoreMissingCustomImages() {

    try {

        const playersWithImages = await Player.find({
            'characters.customImageData': { $exists: true }
        })

        if (!playersWithImages.length) return

        const customImagesDir = path.join(__dirname, 'custom_images')
        ...
        for (const player of playersWithImages) {
            ...
            if (!player.characters?.length) continue

            for (const char of player.characters) {

                if (!char.customImage || !char.customImageData) continue

                const abs = path.isAbsolute(char.customImage)
                    ? char.customImage
                    : path.join(__dirname, char.customImage)
                ...
                try {
                    ...
                    fs.writeFileSync(abs, Buffer.from(char.customImageData, 'base64'))
                    restoredCount++
                } catch (err) {
                    console.log('❌ فشل استرجاع صورة مستبدلة:', char.customImage, err)
                }
            }
        }

        if (restoredCount > 0) {
            console.log(`✅ تم استرجاع ${restoredCount} صورة مستبدلة كانت مفقودة بعد الريستارت`)
        }

    } catch (err) {
        console.log('❌ خطأ أثناء استرجاع الصور المستبدلة:', err)
    }
}
...
// هذا القروب شغال طول الوقت، فقط القروب القديم أعلاه هو اللي يفتح وينقفل)
...
.then(async () => {
    ...
    // 🖼️ نرجّع أي صورة .استبدال انمسحت من القرص المحلي بعد الريستارت
    await restoreMissingCustomImages()
...
📸 عدد الصور المكتشفة: ${fresh.length}/4`
    ...
    if (
        ...
        try {
            ...
            const customImagesDir = path.join(__dirname, 'custom_images')
            if (!(await fileExists(customImagesDir))) {
                await fs.promises.mkdir(customImagesDir, { recursive: true })
            }

            // اسم ملف فريد لكل (لاعب + رقم شخصية) — لو استُخدم .استبدال
            // على نفس الرقم مرة ثانية ينكتب فوق نفس الملف بدون تراكم
            const safeUserId = replaceUserId.replace(/[^a-zA-Z0-9]/g, '_')
            const fileName = `${safeUserId}_${idx}.jpg`
            const relativePath = `custom_images/${fileName}`

            await fs.promises.writeFile(path.join(customImagesDir, fileName), buffer)

            // 🖼️ نحفظ نسخة احتياطية من بايتات الصورة (base64) جوا قاعدة
            // البيانات نفسها مع مسار الملف — لو انمسح الملف من القرص
            // المحلي بعد ريستارت البوت (شائع بكثير من الاستضافات)، يتم
            // ترجيعه تلقائيًا عند إقلاع البوت (شوف restoreMissingCustomImages)
            await replacePlayer.setCharacterCustomImage(idx, relativePath, buffer.toString('base64'))
    ...
    if (
        ...
        if (deleteArgs.length < 3) {
            const removedCount = await deletePlayer.removeAllCustomImages()
...
for (const key in bossWeaponBonus) {
    bossEquipBonus[key] = (bossEquipBonus[key] || 0) + (bossWeaponBonus[key] || 0)
}

damage += (bossEquipBonus.attack || 0)

// ⚔️ هجوم % من الستات الرئيسي الجديد (نسبة من قوة الضربة)
if ((bossEquipBonus.attackPercent || 0) > 0) {
    damage = Math.floor(damage * (1 + bossEquipBonus.attackPercent / 100))
}
...
if (
    Math.random() * 100 <
    ((bossEquipBonus.critRate || 0) + tigerCritBonus)
) {
...
if (me.companion && me.companion.key === 'bear' && (me.companion.level || 0) >= 1) {
    const bearBonus = companionsData.getCompanionBonus('bear', me.companion.level)
    bearMaxHp = Math.floor(bearMaxHp * (1 + bearBonus / 100))
}
