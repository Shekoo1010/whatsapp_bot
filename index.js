...
// 🟢 فتح القروب عند رأس كل ساعة (00 دقيقة)
if (
minutes === 0 &&
!global.bossAttackWindowOpen
) {
...
// 🗄️ تخزين جلسة الواتساب داخل MongoDB بدل ملفات محلية —
// الملفات المحلية (./auth) تُمسح مع أي Restart/Redeploy على Render
// بدون Persistent Disk، فالجلسة تنمسح وتحتاج QR جديد كل مرة.
// MongoDB دائم ومستقل عن السيرفر، فالجلسة تبقى محفوظة دائماً.
const { MongoClient } = require('mongodb')
const { proto, initAuthCreds, BufferJSON } = require('@whiskeysockets/baileys')

// 🛠️ بديل مخصص لدالة useMongoDBAuthState من مكتبة mongo-baileys —
// المكتبة الأصلية تسوي updateOne بتمرير بيانات ثنائية (BinData) مباشرة
// كمستند تحديث بدون تغليفها بـ $set، وهذا غير متوافق مع mongodb driver
// v6.x الحديث ويسبب خطأ "Modifiers operate on fields but we found
// type binData instead". هذي الدالة تسوي نفس الوظيفة بالضبط لكن بصيغة
// $set صحيحة، فتحل المشكلة نهائياً بدون تغيير باقي كود البوت.
async function useMongoDBAuthStateFixed(collection) {
...
let authMongoClient = null
let authCollection = null

async function getAuthCollection() {
    if (authCollection) return authCollection

    authMongoClient = new MongoClient(process.env.MONGO_URI)
    await authMongoClient.connect()

    // نفس قاعدة البيانات المستخدمة أصلاً (المحددة داخل رابط MONGO_URI)،
    // بس بكولكشن مخصص لجلسة الواتساب باسم authState
    authCollection = authMongoClient.db().collection('authState')

    console.log('✅ Auth MongoDB Collection Connected')
...
.then(async () => {
    ...
    try {
        await Promise.all([
            Player.collection.createIndex({ userId: 1 }),
            ...
            Player.collection.createIndex({ username: 1 }),
...
console.log('🚀 START BOT', Date.now())
    ...
    const { state, saveCreds } =
        await useMongoDBAuthStateFixed(collection)
    // =========================
// Event System
// =========================
...
if (!text) return;
    ...
    if (
        ...
        text !== '.انهاء_مسابقة' &&
        text !== '.النقاط' &&
        !isOwner(msg) // 👑 نفس نمط بقية البوابات (تبنيد/إيقاف/وقت العمل) — المطور ما ينحظر حتى لو المسابقة عالقة
    ) {
