...
// =========================
// 🐺 لعبة المستذئبين — تحديثات تصويت الـ Poll (محكمة الإعدام/المسامحة)
// تصل هذي بحدث منفصل تماماً عن الرسائل العادية (messages.upsert)
// =========================
sock.ev.on('messages.update', async (updates) => {
    for (const u of updates) {
        if (u.key?.remoteJid?.endsWith('@g.us')) {
            await werewolfGame.handlePollUpdate(u.key.remoteJid, u).catch(err => {
                console.log('Werewolf poll update handler error:', err)
            })
        }
    }
})
