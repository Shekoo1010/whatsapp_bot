...
if (!text) return;
    ...
    // =========================
    // 🧠 QUIZ SYSTEM — لازم تكون أول شي بعد بوابة إيقاف/تشغيل البوت،
    // قبل أي نظام أو لعبة ثانية (برا السالفة/المستذئبين/كود نيمز/
    // بطولة المعرض/السفن/العوالم). كل وحدة من هذي فيها await منفصل،
    // فلو المسابقة نشطة وهذي البوابة متأخرة، كل رسالة إجابة كانت
    // تنتظر كل تلك الأنظمة الثانية قبل ما توصل لحساب الوقت والرد،
    // وهذا يخرب دقة ⏱️ الوقت المعروض ويبطئ رد المسابقة عموماً.
    // هنا: وقت المسابقة نشطة، البوت ما يهتم بأي شي ثاني إطلاقاً —
    // فقط .انهاء_مسابقة و.النقاط مسموحين، وأي شي ثاني يروح مباشرة
    // لـ checkAnswer بدون المرور على أي نظام آخر.
    // =========================
    const quizGateRoom = quizData.getQuizRoom(msg.key.remoteJid)

    if (
        quizGateRoom.quizActive &&
        text !== '.انهاء_مسابقة' &&
        text !== '.النقاط'
    ) {
