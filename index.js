...
// ⚠️ راوت مؤقت لهجرة صور .استبدال من base64 إلى Cloudinary — احذفه بعد الاستخدام
app.get('/run-migration-once-xk29', async (req, res) => {
    try {
        ...
        for (const player of players) {
            ...
            for (let i = 0; i < (player.characters?.length || 0); i++) {
                ...
                try {
                    ...
                    changed = true
                    migratedImages++
                    log.push(`✅ ${player.userId} #${i} -> ${url}`)
                } catch (err) {
                    failedImages++
                    log.push(`❌ فشل ${player.userId} #${i}: ${err.message}`)
                }
            }
        ...
        res.status(500).send('خطأ عام: ' + err.message)
    }
})
