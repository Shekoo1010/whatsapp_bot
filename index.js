...
// تُوزَّع تلقائياً كل يوم الساعة 12 صباحاً بتوقيت السعودية
// =========================
...
`
    ...
    console.log('📊 Daily contribution rewards distributed & reset')
}
...
if (text === ".تصفير_الانجازات") {
    ...
    return safeSend(msg.key.remoteJid, {
        text:
`✅ تم تصفير سجل الإنجازات المُستلمة لجميع اللاعبين.

📊 كل شخص سيبدأ عدّاد الرتب المستلمة من 0.
⚠️ ملاحظة: إحصائيات التقدم نفسها (الفوز، ضرر الزعيم، السحوبات...) لم تتغير — فقط سجل "ما تم استلامه". أول تفاعل بعد التصفير سيمنح مرة واحدة أي رتبة تجاوزها اللاعب فعلياً ولم يستلمها بعد.`
    })
}
...
مثال: .دخول_دومين 1`
    ...
    if (text.startsWith('.فريق_دومين') || text.startsWith('.domain_team')) {
        ...
        if (!args[1] || !digits || isNaN(domainId)) {
...
(الأرقام تعني الشخصيات 1 و2 و3 من .شخصياتي)`
...
💡 الشخصية لم تُستهلك`
    ...
    await checkAndGrantAchievement(player, 'kingdom', player.kingdomRaid.stage, sock, msg.key.remoteJid)
...
📌 استخدم .تركيب_جوبي لتجهيزه`
    ...
    // القيم الحالية لكل تصنيف (نفس المصادر المستخدمة بالربط)
    const currentValues = {
        pvp: player.wins || 0,
        boss: player.totalBossDamage || 0,
        beasts: player.beastCollection || 0,
        brawl: player.brawlWins || 0,
        tower: player.towerFloor || 0,
        kingdom: (player.kingdomRaid && player.kingdomRaid.stage) || 0,
...
👑 حصلت على الجوبي!`
    ...
    await checkAndGrantAchievement(player, 'beasts', player.beastCollection, sock, msg.key.remoteJid)
...
if (text === '.مساهمات') {
    ...
    // ⏳ حساب الوقت المتبقي للتصفير (12 صباحاً بتوقيت السعودية)
    const riyadhNow =
        new Date(
            new Date().toLocaleString(
                'en-US',
...
💥 ${(me?.totalBossDamage || 0).toLocaleString()} ضرر إجمالي
