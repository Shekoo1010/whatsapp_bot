...
.then(async () => {
    ...
    // ==========================================================
    // 🔍 بحث لمرة واحدة: إيجاد ايدي لاعب عن طريق شخصياته
    // (احذف هذا البلوك كامل بعد ما تاخذ النتيجة من الـ Logs)
    // ==========================================================
    try {
        ...
        console.log(
            `🔍 [بحث الشخصيات] عدد اللاعبين المطابقين جزئيًا: ${matchingPlayers.length}`
        )

        const searchResults = matchingPlayers.map(p => {

            const ownedNames = new Set(
                (p.characters || []).map(c => c.name)
            )
        ...
        searchResults.forEach(r => {
            ...
            console.log(
                `🔍 [بحث الشخصيات] الاسم: ${r.name} | الايدي: ${r.userId} | تطابق: ${r.matchedCount}/${TARGET_CHARACTERS.length} | الشخصيات: ${r.matchedNames.join('، ')}`
            )
        ...
        if (
            ...
            searchResults[0].matchedCount === TARGET_CHARACTERS.length
        ) {

            console.log(
                `🎯 [بحث الشخصيات] تطابق كامل! الايدي: ${searchResults[0].userId}`
            )

        } else if (!searchResults.length) {

            console.log(
                '🔍 [بحث الشخصيات] ما فيه أي لاعب عنده أي شخصية من القائمة'
            )
