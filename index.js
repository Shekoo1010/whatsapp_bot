...
const commandExplanations = {
    ...
    'الشخصيات': {
        ...
        '.المعرض اضف': 'يضيف شخصية لمعرضك برقمها من .شخصياتي. مثال: .المعرض اضف 5',
        '.المعرض حذف': 'يحذف شخصية من معرضك برقم مكانها *داخل المعرض*. مثال: .المعرض حذف 3',
...
if (text.startsWith('.المعرض')) {
    ...
    if (sub === 'اضف') {
        ...
        if (isNaN(idx)) {
            return safeSend(
                msg.key.remoteJid,
                { text: '❌ الاستخدام: .المعرض اضف رقم (نفس رقم .شخصياتي)' }
            )
        }
        ...
        if (!result.ok) {
            return safeSend(msg.key.remoteJid, { text: `❌ ${result.reason}` })
        }

        await player.save()

        const addedTier = result.rarity ? resolveTierKey(result.rarity, result.evolutionLevel) : ''

        return safeSend(
            msg.key.remoteJid,
            {
                text: `✅ ${result.name}${addedTier ? ` (${addedTier})` : ''} انضافت للمعرض (${result.count}/${MAX_GALLERY})`
            }
        )
    }
    ...
    if (sub === 'حذف') {
        ...
        if (isNaN(idx)) {
            return safeSend(
                msg.key.remoteJid,
                { text: '❌ الاستخدام: .المعرض حذف رقم (رقم مكانها بالمعرض — اكتب .المعرض قائمة عشان تشوف الأرقام)' }
            )
        }
        ...
        return safeSend(
            msg.key.remoteJid,
            { text: `🗑️ ${result.name} انحذفت من المعرض (${result.count}/${MAX_GALLERY})` }
        )
    }
