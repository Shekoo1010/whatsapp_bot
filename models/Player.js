...
// 🖼️ base64Data (اختياري): نسخة احتياطية من بايتات الصورة نفسها تُحفظ
// جوا مستند اللاعب بقاعدة البيانات (وليس على القرص المحلي فقط). هذا
// يضمن رجوع الصورة تلقائيًا حتى لو انمسح ملف custom_images/ بعد
// ريستارت/إعادة نشر البوت (استضافات كثيرة تصفّر القرص المحلي، لكن
// قاعدة البيانات نفسها ثابتة) — شوف restoreMissingCustomImageFiles
// بالأسفل اللي يستخدم هالنسخة عشان يعيد كتابة الملف المفقود تلقائيًا.
PlayerSchema.methods.setCharacterCustomImage = async function(characterIndex, imagePath, base64Data) {
    const char = this.characters?.[characterIndex]
    if (!char) return false

    char.customImage = imagePath
    if (base64Data) {
        char.customImageData = base64Data
    }
