...
const BIAS_WINDOW_SIZE = 20       // آخر كم عينة نحتفظ فيها لكل قروب
const BIAS_MIN_SAMPLES = 5        // أقل عدد عينات قبل ما نثق بالـ baseline
const BIAS_BASELINE_PERCENT = 0.1 // نأخذ متوسط أدنى 10% من العينات (أكثر ثباتاً من أدنى عينة وحدة)
...
function recordBiasSample(room, biasMs) {
    ...
    if (biasMs < 0 || biasMs > 10000) return // عينة شاذة (غالباً خطأ قياس) — لا تفسد الـ baseline
...
function computeElapsedSeconds(room, winner) {
    ...
    // بدون طابع واتساب على رسالة الإجابة ما نقدر نحسب bias الاستلام —
    // نرجع الرقم الخام بدون تصحيح (بدون ما نفشل الجولة)
    if (!winner.msgWaTs) {
        return result
    }
    ...
    const baseline = getBaselineBias(room)

    result.baselineBias = baseline

    if (baseline !== null) {

        const extraLag = currentBias - baseline
    ...
    // 📊 نسجل هذي العينة عشان الـ baseline يتحدث للجولات الجاية
            ...
            console.log(
                ...
                (timing.invalid
                    ...
                    : `elapsed(بعد التصحيح)=${timing.correctedElapsedMs}ms` +
                      (timing.corrected ? ` (طرحنا ${timing.extraLagMs}ms، baseline=${timing.baselineBias?.toFixed(0)}ms, current=${timing.currentBias}ms)` : ' (بدون تصحيح)') + `\n`
                ) +
