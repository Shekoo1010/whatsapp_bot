...
async function resetBeasts() {
    ...
    // 🔄 تناوب حقيقي: نلاقي مين كان حامل البيضة بالجولة
    // السابقة ونعطيها هالمرة للوحش الثاني، بدل الاختيار
    // العشوائي اللي كان ممكن يعطي نفس الوحش البيضة أكثر
    // من مرة متتالية
    const previousCarrierIndex =
        beasts.findIndex(b => b.eggCarrier)

    const eggIndex =
        previousCarrierIndex === -1
            ? Math.floor(Math.random() * beasts.length)
            : (previousCarrierIndex + 1) % beasts.length
