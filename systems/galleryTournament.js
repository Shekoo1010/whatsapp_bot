...
//                          (يُغلق تلقائيًا بعد دقيقتين لو ما سجّل أي أحد)
...
// .انهاء_معرض          → المطور فقط: ينهي البطولة/التسجيل فورًا بأي وقت
// =====================================================================
...
// ⏱️ لو ما سجّل أي أحد خلال هالمدة من فتح باب التسجيل، يُغلق تلقائيًا
const REGISTRATION_TIMEOUT_MS = 2 * 60 * 1000  // دقيقتين
...
function getState(groupId) {
    if (!states.has(groupId)) {
        states.set(groupId, {
            ...
            // ⏱️ تايمر القفل التلقائي لباب التسجيل (يُلغى فور اكتمال شرط
            // البدء أو عند إنهاء البطولة يدويًا)
...
function resetState(groupId) {
    ...
    if (t?.registrationTimer) clearTimeout(t.registrationTimer)
...
async function openRegistration(sock, groupId) {
    ...
    // ⏱️ لو ما سجّل أي أحد خلال دقيقتين، يُغلق باب التسجيل تلقائيًا
    if (t.registrationTimer) clearTimeout(t.registrationTimer)
    t.registrationTimer = setTimeout(() => {
        autoCloseRegistration(sock, groupId, t).catch(err =>
            console.log('gallery tournament autoCloseRegistration error:', err))
...
⏰ يُغلق التسجيل تلقائيًا لو ما سجّل أي أحد خلال دقيقتين`
    })
}

// 🔒 يُستدعى تلقائيًا بعد دقيقتين من فتح التسجيل — يقفل الباب فقط لو
// ما انضم أي أحد لحد هاللحظة (لو فيه أي مسجَّل، يستمر التسجيل عادي)
async function autoCloseRegistration(sock, groupId, t) {
    // ✅ تأكد إن هذي نفس حالة التسجيل اللي بدأ معها التايمر (مو حالة
    // جديدة اتفتحت بعدها) وإن التسجيل لسا بمرحلة "registration"
    const current = states.get(groupId)
    if (current !== t) return
    if (t.phase !== 'registration') return
    if (t.registered.length > 0) return

    resetState(groupId)

    await sock.sendMessage(groupId, {
        text:
`⏰ ═══〔 أُغلق باب التسجيل تلقائيًا 〕═══ ⏰

📭 ما سجّل أي أحد خلال أول دقيقتين من فتح البطولة
🔁 استخدم .مسابقه_معرض لإعادة فتح باب التسجيل من جديد`
    })
}
...
async function beginTournament(sock, groupId) {
    ...
    // ⏱️ البطولة بدأت فعليًا، ما فيه داعي لتايمر القفل التلقائي بعد الآن
    if (t.registrationTimer) {
        clearTimeout(t.registrationTimer)
        t.registrationTimer = null
    }
...
// =====================================================================
// إنهاء يدوي (المطور) — يشتغل بأي مرحلة: تسجيل أو بطولة شغالة فعليًا
// =====================================================================

async function forceEndTournament(sock, groupId) {
    const t = states.get(groupId)

    if (!t || !t.phase) {
        return sock.sendMessage(groupId, {
            text: '❌ ما فيه تسجيل أو بطولة شغالة حاليًا بهذا القروب'
        })
    }

    const wasActive = t.phase === 'active'

    // 🧹 يمسح الحالة بالكامل ويلغي أي تايمرات شغالة (تصويت أو قفل تسجيل)
    resetState(groupId)
...
⚔️ أنهى المطور البطولة الجارية يدويًا
🔁 استخدم .مسابقه_معرض لبدء بطولة جديدة`
            :
`🛑 تم إغلاق باب التسجيل يدويًا من قبل المطور`
    })
}
...
async function handleMessage(sock, groupId, userId, text, pushName, isOwnerFlag) {
    ...
    if (text === '.انهاء_معرض') {
        ...
        await forceEndTournament(sock, groupId)
        return true
    }
