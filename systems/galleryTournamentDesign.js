...
// ⚡ تحسين أداء: المتصفح مشترك ومحمي من التشغيل المتزامن (utils/sharedBrowser.js)
const { getBrowser } = require('../utils/sharedBrowser')

// ---------------------------------------------------------------
// متصفح مشترك (نفس فكرة myRosterCard.js) — يعاد استخدامه للسرعة
// ---------------------------------------------------------------
// ⚡ تحسين أداء: المتصفح صار مشترك مع myRosterCard.js (Chrome واحد بدل
// اثنين) ومحمي من التشغيل المتزامن — شوف utils/sharedBrowser.js.
...
// 🚫 خانة "شل" (Shell) — للأعمدة الخارجية غير المستخدمة فعليًا بهذي
// البطولة (مثلاً عمود دور الـ16 لبطولة بدأت بـ8، أو عمودي دور الـ16
// والثمانية لبطولة بدأت بـ4). تُعرض باهتة بحدود متقطعة، بدون رقم
// مباراة حقيقي ولا seed، وكل خانة مكتوب عليها "غير مسجل" مع شطب —
// هذا هو اللي يخلي صورة البطولة "قالب كامل" ثابت الشكل دائمًا (7
// أعمدة) بغض النظر عن حجم البطولة الفعلي (16/8/4)
function shellMatchCellHTML() {
    return `
        <div class="match shell"><span class="mnum">—</span>
            <div class="slot"><span class="pname empty">غير مسجل</span></div>
...
function bracketBaseStyles() {
    ...
    /* ✅ صف العناوين يتبع بالضبط نفس هيكلة flex اللي عند .bracket تحته
       (side flex:3 يحتوي 3 تسميات، final-label flex:1.3 بالمنتصف)
       فبكذا كل badge يطلع بالضبط فوق نفس عمود مبارياته */
    .round-labels{display:flex; align-items:stretch; margin-top:56px;}
    .round-labels .side{display:flex; flex:3; min-width:0;}
    .round-labels .label{flex:1; min-width:0; text-align:center; display:flex; flex-direction:column; align-items:center; justify-content:center; gap:8px; padding:0 22px;}
    .round-labels .final-label{flex:1.3; min-width:0; display:flex; flex-direction:column; align-items:center; justify-content:center; gap:8px;}
    .round-labels .badge{
        width:40px; height:40px; border-radius:50%; display:flex; align-items:center; justify-content:center;
        background:rgba(240,192,74,.08); border:1.5px solid var(--gold-dim); color:var(--gold); font-weight:800; font-size:15px; flex:none;
    }
    .round-labels .txt{font-size:14px; font-weight:800; color:var(--text-dim); white-space:nowrap;}
    /* عمود "شل" (غير مستخدم بهذي البطولة، مثلاً دور الـ16 لبطولة بدأت
       بـ8) → تسميته الباهتة "غير مسجل" */
    .round-labels .label.shell .badge{opacity:.35; border-color:rgba(240,192,74,.15); color:var(--text-dim);}
    .round-labels .label.shell .txt{opacity:.4;}
    ...
    /* ⚠️ display:block هنا مهم — بدونه، تعريف .match الآخر بـ
       baseStyles() (للصورة البسيطة العادية) يسرّب display:flex لهذا
       العنصر، فتنكمش خانات الاسم بصف أفقي ضيق واحد بدل ما تترصّ فوق
       بعض (اسم/VS/اسم) بشكل مريح للأسماء الطويلة */
    .match{
        display:block; position:relative; background:linear-gradient(180deg, var(--card2), var(--card));
    ...
    /* ✅ إصلاح الأسماء المقصوصة: بدل قصّها بسطر وحد بـ "..."، نسمح
       بسطرين (line-clamp) مع كسر الكلمة عند الحاجة — الاسم يظهر كامل
       بمعظم الحالات، ولو طويل جدًا يوقف بأمان بسطرين بدون ما يكسر
       تصميم الكرت أو يطلع بره حدوده */
    .pname{
        flex:1; font-size:16px; font-weight:800; color:var(--text); min-width:0;
        display:-webkit-box; -webkit-line-clamp:2; -webkit-box-orient:vertical;
        overflow:hidden; word-break:break-word; line-height:1.22;
    }
    ...
    /* ⚪ خانة "شل" (عمود غير مستخدم بهذي البطولة حسب حجمها 8/4) →
       "غير مسجل" مشطوبة وباهتة، والكرت نفسه بحدود متقطعة خافتة */
    .pname.empty{
        color:var(--text-dim);
        text-decoration:line-through;
        text-decoration-color:rgba(255,59,92,.6);
        text-decoration-thickness:2.5px;
        opacity:.5; font-weight:700; -webkit-line-clamp:1;
    }
    .match.shell{opacity:.42; border-style:dashed; border-color:rgba(240,192,74,.12); box-shadow:none;}
    .match.shell .mnum{background:rgba(136,145,168,.35); color:#0c0f1a; box-shadow:none; border-color:#0c0f1a;}
...
// ⚠️ نفس القالب الكامل الكبير (7 أعمدة) اللي يُستخدم لـ16 — عمود
// دور الـ16 الخارجي غير مستخدم بهذي البطولة فيُرسم "شل" (غير مسجل
// ومشطوب)، والأسماء الحقيقية تبدأ من عمود دور الثمانية مباشرة
// (يمين ويسار)، مو من دور الـ16
...
// ⚠️ نفس القالب الكامل الكبير (7 أعمدة) اللي يُستخدم لـ16 — عمودي
// دور الـ16 ودور الثمانية الخارجيين غير مستخدمين بهذي البطولة
// فيُرسمان "شل" (غير مسجل ومشطوب)، والأسماء الحقيقية تبدأ من عمود
// نصف النهائي مباشرة (يمين ويسار)
function treeBodySize4(matches) {
    ...
    <div class="bracket">
        <div class="side-left" style="display:flex; flex:3;">
            ...
            <div class="round">
                <div class="pair">${matchCellHTML(g(1))}</div>
            </div>
        </div>
        ...
        <div class="side-right" style="display:flex; flex:3;">
            <div class="round">
                <div class="pair">${matchCellHTML(g(2))}</div>
            </div>
            <div class="round">
                <div class="pair">
                    ${shellMatchCellHTML()}
...
// 🏷️ صف عناوين الأدوار — دائمًا 3 تسميات بكل جهة (يمين ويسار) +
// النهائي بالمنتصف، بنفس هيكلة flex اللي عند .bracket تحته بالضبط
// (side flex:3, final-label flex:1.3) حتى يوقف كل badge فوق عموده.
// الأعمدة غير المستخدمة فعليًا حسب حجم البطولة (8 أو 4) تُعرض باهتة
// ومكتوب عليها "غير مسجل" (class="shell") بدل رقم الدور الحقيقي
function roundLabelsHTML(size) {
    const tiers =
        size === 16 ? [
            { n: '16', t: 'دور الـ16', shell: false },
            { n: '8', t: 'دور الثمانية', shell: false },
            { n: '4', t: 'نصف النهائي', shell: false },
        ] :
        size === 8 ? [
            { n: '16', t: 'غير مسجل', shell: true },
            { n: '8', t: 'دور الثمانية', shell: false },
            { n: '4', t: 'نصف النهائي', shell: false },
        ] : [
            { n: '16', t: 'غير مسجل', shell: true },
            { n: '8', t: 'غير مسجل', shell: true },
            { n: '4', t: 'نصف النهائي', shell: false },
        ]

    const label = ({ n, t, shell }) =>
        `<div class="label${shell ? ' shell' : ''}"><span class="badge">${n}</span><span class="txt">${t}</span></div>`

    return `
    <div class="round-labels">
        <div class="side">${tiers.map(label).join('')}</div>
        <div class="final-label"><span class="badge">🏆</span><span class="txt">النهائي</span></div>
        <div class="side">${[...tiers].reverse().map(label).join('')}</div>
    </div>`
}

// 🖼️ من الآن الشجرة دائمًا نفس القالب الكامل الكبير (7 أعمدة) بغض
// النظر عن حجم البطولة الفعلي (16/8/4) — الأعمدة غير المستخدمة
// تُعرض "شل" باهتة بدل ما تُحذف، فما عاد نحتاج نكبّر الخط/الكروت
// حسب الحجم متل قبل؛ نفس المقاس بالضبط لكل الأحجام
function sizeScaleStyles(size) {
    return `.wrap{max-width:2000px;}`
}
...
async function renderBracketTreeImage({ size, matches, roundBadge }) {
    ...
    // نفس مقاس الصورة لكل الأحجام (16/8/4) — الشجرة دائمًا نفس القالب
    // الكامل الكبير بـ7 أعمدة، فرق الحجم يظهر فقط بأي الأعمدة "حية"
    // وأيها "شل" غير مسجل
    const viewport = { width: 2000, height: 1250 }
