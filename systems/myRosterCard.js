...
<style>
  ...
  *{box-sizing:border-box;margin:0;padding:0;}
  body{
    ...
    padding:77px 62px 93px;
  }
  .frame{max-width:3130px; margin:0 auto; position:relative;}
  ...
  .eyebrow{text-align:center; font-family:'Oswald',sans-serif; letter-spacing:.45em; font-size:17px; color:var(--gold-dim); text-transform:uppercase; margin-bottom:15px;}
  h1{text-align:center; font-size:80px; font-weight:900; background:linear-gradient(180deg,#fff6d8,var(--gold) 55%,#a9791f); -webkit-background-clip:text; background-clip:text; color:transparent;}
  .sub{text-align:center; font-family:'Oswald',sans-serif; font-size:20px; letter-spacing:.35em; color:var(--text-dim); margin:9px 0 62px; text-transform:uppercase;}

  /* 🔍 تكبير عرض المعرض: MAX_PER_ROW صار 5 بدل 6، وكل قيم الكرت
     مكبّرة بنفس النسبة (×1.55 تقريبًا) عشان الصورة تبان أوضح وأكبر
     بدون أي تغيير بمنطق التوزيع أو الألوان أو الرتب. */
  .roster{display:flex; flex-wrap:wrap; justify-content:center; gap:43px; margin-bottom:40px;}
  .card{
    position:relative; width:580px; flex:0 0 580px; border-radius:23px; overflow:hidden; background:var(--bg2);
    border:4px solid var(--tier); display:flex; flex-direction:column;
    box-shadow:0 20px 50px rgba(0,0,0,.45), 0 0 38px color-mix(in srgb, var(--tier) 35%, transparent);
  }
  .card.omega{border-image:linear-gradient(135deg,#ff3860,#f0c04a,#3ea8ff,#c04aff,#ff3860) 1;}
  .tier-tag{display:flex; align-items:center; justify-content:space-between; padding:20px 28px 11px;}
  .tier-name{font-size:31px; font-weight:700; color:var(--tier);}
  .tier-name.en{font-family:'Oswald',sans-serif; letter-spacing:.14em; text-transform:uppercase; font-size:28px; direction:ltr;}
  .tier-name.ar{font-family:'Cairo',sans-serif; font-weight:800;}
  .pwr-badge{font-family:'Oswald',sans-serif; font-size:23px; font-weight:600; color:#0a0d16; background:var(--tier); padding:6px 19px; border-radius:31px; direction:ltr;}
  .stars{padding:0 28px 20px; font-size:31px; letter-spacing:2.3px; color:var(--tier); text-shadow:0 0 8px color-mix(in srgb, var(--tier) 60%, transparent); direction:ltr; text-align:right; line-height:1.3;}
  .art{position:relative; flex:1; min-height:680px; background-size:cover; background-position:center top; background-color:#151a28;}
  .art .fade{position:absolute; inset:0; background:linear-gradient(180deg, transparent 55%, rgba(10,13,22,.92) 100%);}
  .plate{padding:28px 28px 31px; text-align:center; background:linear-gradient(180deg, transparent, rgba(0,0,0,.5));}
  .name-en{font-family:'Oswald',sans-serif; font-weight:600; font-size:37px; letter-spacing:.03em; color:#fff; direction:ltr;}
  .anime-chip{margin-top:19px; display:inline-block; font-size:23px; font-family:'Oswald',sans-serif; letter-spacing:.08em; color:var(--tier); border:1px solid color-mix(in srgb, var(--tier) 50%, transparent); border-radius:31px; padding:6px 22px; text-transform:uppercase; background:color-mix(in srgb, var(--tier) 10%, transparent); direction:ltr;}
</style>
</head>
<body>
...
async function renderRosterImage(characters, opts = {}) {
    ...
    const page = await browser.newPage()
    try {
        // 🔍 أبعاد الـ viewport محدّثة لتتماشى مع الكرت الأكبر (580px)
        // وحد الصف الجديد (5 بدل 6): العرض لازم يستوعب صف كامل (5 كروت
        // + الفجوات + هامش الإطار)، والارتفاع لكل صف زاد بنفس نسبة تكبير
        // الكرت (~×1.55) عشان محتوى الصف ما ينقص عن مساحته الحقيقية.
        const rowCount = Math.ceil(characters.length / MAX_PER_ROW)
        await page.setViewport({
            width: 3260,
            height: 464 + rowCount * 990,
            deviceScaleFactor: 2
        })
        await page.setContent(html, { waitUntil: 'networkidle0' })
        await applySmartArtPositioning(page)
