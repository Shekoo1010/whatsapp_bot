...
function singleCardGlow(tierKey, color) {
    if (tierKey === 'Ω OMEGA') {
        ...
        return [
            ...
            `0 0 140px 26px ${hexToRgba(c4, .28)}`,
            `0 0 240px 44px rgba(160,90,220,.14)`
        ].join(', ')
    }
    ...
    return [
        ...
        `0 0 140px 26px ${hexToRgba(color, .22)}`,
        `0 0 230px 42px ${hexToRgba(color, .1)}`
    ].join(', ')
}

// خلفية المسرح خلف الكرت — تدرّج شفاف خفيف بنفس لون الرتبة يذوب بهدوء
// لأسود قريب من الحواف، عشان ما يبقى أي فراغ أسود صامت بدون توهج.
function singleCardStageBg(color) {
    return `radial-gradient(circle at center, ${hexToRgba(color, .13)}, #06070c 68%)`
}
...
<style>
  *{box-sizing:border-box;margin:0;padding:0;}
  ...
  .stage{
    ...
    background:${stageBg}; padding:100px 140px;
  }
  .card{
    position:relative; width:616px; height:880px; border-radius:22px; overflow:hidden; background:#0f1422;
  ...
  .tier-name{font-size:22px; font-weight:700; color:${t.color};}
  .tier-name.en{font-family:'Oswald',sans-serif; letter-spacing:.16em; text-transform:uppercase; font-size:19px; direction:ltr;}
  .tier-name.ar{font-family:'Cairo',sans-serif; font-weight:900;}
  .pwr-badge{font-family:'Oswald',sans-serif; font-size:15px; font-weight:600; color:#0a0d16; background:${t.color}; padding:4px 14px; border-radius:30px; direction:ltr;}
  .stars{padding:0 24px 13px; font-size:22px; letter-spacing:2.6px; color:${t.color}; text-shadow:0 0 12px color-mix(in srgb, ${t.color} 60%, transparent); direction:ltr; text-align:right;}
  ...
  .plate{padding:16px 18px 22px; text-align:center; background:linear-gradient(180deg, transparent, rgba(0,0,0,.55));}
  .name-en{font-family:'Oswald',sans-serif; font-weight:600; font-size:31px; letter-spacing:.03em; color:#fff; direction:ltr;}
  .anime-chip{margin-top:11px; display:inline-block; font-size:15px; font-family:'Oswald',sans-serif; letter-spacing:.08em; color:${t.color}; border:2px solid color-mix(in srgb, ${t.color} 50%, transparent); border-radius:30px; padding:5px 17px; text-transform:uppercase; background:color-mix(in srgb, ${t.color} 10%, transparent); direction:ltr;}
</style>
</head>
<body>
