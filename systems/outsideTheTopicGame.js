...
// =========================
// نفس منطق resolveDmJid الموجود بـ index.js: واتساب يرسل هوية مخفية
// (@lid) داخل القروبات بدل رقم الهاتف الحقيقي، وإرسال خاص مباشر لـ
// @lid يفشل بصمت لو ما فيه رقم حقيقي محفوظ له (phoneJid بجدول اللاعب).
async function resolveDmJid(jid) {
    ...
    if (jid.endsWith('@lid')) {
        try {
            ...
            console.log('outsideGame resolveDmJid error:', err)
        }
    }
...
async function beginRound(sock, groupId) {
    ...
    // ⚠️ نحاول نحوّل أي @lid لرقم الهاتف الحقيقي (phoneJid) قبل الإرسال —
    // إرسال خاص مباشر لـ @lid يفشل بصمت (error 463) لو ما فيه جلسة/رقم محفوظ.
    const failedPlayers = []
    let isFirst = true
    for (const player of state.players) {
        ...
        try {
            const dmJid = await resolveDmJid(player)
    ...
    let startMsg = `🎬 بدأت الجولة!\n\n📂 الفئة: ${state.category}\n👥 اللاعبين: ${state.players.length}\n🚫 عدد اللي برا السالفة: ${outsiderCount}\n\n📩 تم إرسال دوركم بالخاص، تأكدوا من فتح الخاص مع البوت.\n\nجاوبوا بـ "نعم" أو "لا" فقط عند دوركم، عندكم دقيقتين لكل دور ⏱️`

    if (failedPlayers.length) {
        startMsg += `\n\n⚠️ ما وصلت الرسالة الخاصة لـ: ${failedPlayers.map(mentionTag).join('، ')}\nراسلوا البوت بالخاص أي رسالة (مثلاً "هلا") عشان يحفظ رقمكم، وبعدها اطلبوا من مسؤول الجولة يبدأ جولة جديدة.`
    }

    await sock.sendMessage(groupId, {
        text: startMsg,
        mentions: failedPlayers.length ? [...state.players, ...failedPlayers] : state.players
    })
