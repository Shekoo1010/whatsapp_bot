...
async function startNight(sock, groupId, state) {
    ...
    if (wolves.length) {
        ...
        let isFirstWolf = true
        for (const w of wolves) {
            if (!isFirstWolf) await sleep(4000)
            isFirstWolf = false

            // 🐾 تذكير برفيق الذئب — أول ليلة بس (state.round === 1)، ما
            // يتكرر بالليالي اللي بعدها
            let packExtra = ''
            let packMentions = []
            if (state.round === 1) {
                const packMates = wolves.filter(p => p !== w)
                if (packMates.length) {
                    packExtra = `\n🐾 رفيقك الذئب: ${packMates.map(p => displayName(state, p)).join('، ')}`
                    packMentions = packMates
                }
            }

            await dm(sock, w, `🐺 اختر ضحية الليلة:\n\n${list}\nاكتب: .اختيار <الرقم>${introExtra}${packExtra}`, [...targets, ...packMentions])
        }
    }
...
async function handleNightAction(sock, senderId, text, groupId, state) {
    ...
    if (role === 'seer' && senderId === n.seer) {
        ...
        // 🔧 إصلاح: العراف كان يشوف بس "ذئب" أو "قروي" — أي دور ثاني (طبيب/
        // عراف/صياد) كان يظهر "قروي" تلقائياً لأنه مو ذئب. الآن يظهر الدور
        // الحقيقي لغير الذئاب، وتبقى خديعة الألفا شغالة (يظهر "قروي" عمداً)
        let result
        if (targetRole === 'alpha') {
            result = 'قروي 👤'
        } else if (isWolfRole(targetRole)) {
            result = 'ذئب 🐺'
        } else {
            result = ROLE_INFO[targetRole]?.name || 'قروي 👤'
        }
