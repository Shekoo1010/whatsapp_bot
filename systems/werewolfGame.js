...
// - بعدها يُسكت المتهم أيضاً، ويرسل البوت Poll: "إعدام" أو "مسامحة".
// - أعلى صوت بالـ Poll يحسم: إعدام = يموت، مسامحة = يرجع بينهم بدون ما يموت.
// - ⚠️ نتائج الـ Poll تصل عبر حدث Baileys المنفصل "messages.update"، فلازم
//   index.js يستدعي werewolfGame.handlePollUpdate(groupId, update) لكل
//   عنصر بذاك الحدث عشان تُحتسب الأصوات صح (راجع نهاية الملف).
//
...
async function joinLobby(sock, groupId, userId, userName) {
    ...
    return sock.sendMessage(groupId, {
        text: `✅ ${displayName(state, userId)} انضم للعبة! (${state.players.length}/${MAX_PLAYERS})`,
        mentions: [userId]
    })
}
...
async function beginGame(sock, groupId, { forced }) {
    ...
    if (failed.length) {
        startText += `\n\n⚠️ ما وصلت الرسالة الخاصة لـ: ${failed.map(p => displayName(state, p)).join('، ')}\nلازم تفتحون الخاص مع البوت عشان تلعبون بشكل صحيح!`
    }
...
async function triggerHunterRevenge(sock, groupId, state, hunterId, onDone) {
    ...
    await sock.sendMessage(groupId, {
        text: `🏹 ${displayName(state, hunterId)} كان الصياد! عنده فرصة أخيرة يأخذ ثأره...`,
        mentions: [hunterId]
    })
...
async function finishHunterRevenge(sock, groupId, state, target, onDone) {
    ...
    if (target) {
        ...
        await sock.sendMessage(groupId, {
            text: `🏹 أخذ الصياد ثأره من ${displayName(state, target)}! (كان دوره: ${ROLE_INFO[role].name})`,
            mentions: [target]
        })
    } else {
...
async function startVoting(sock, groupId, state) {
    ...
    const alivePlayers = aliveList(state)
    let list = ''
    alivePlayers.forEach((p, i) => { list += `${i + 1}- ${displayName(state, p)}\n` })
...
async function handleVoteAction(sock, senderId, text, groupId, state) {
    ...
    if (state.votes.has(senderId)) {
        await sock.sendMessage(groupId, {
            text: `❌ ${displayName(state, senderId)} صوّتّ من قبل، التصويت الأول بس هو المحسوب وما تقدر تغيّره.`,
            mentions: [senderId]
        })
        return true
    }
    ...
    await sock.sendMessage(groupId, {
        text: `✅ تم تسجيل صوت ${displayName(state, senderId)} (${state.votes.size}/${state.alive.size})`,
        mentions: [senderId]
    })
...
async function resolveVoting(sock, groupId, state) {
    ...
    for (const [target, count] of tally.entries()) {
        resultText += `${displayName(state, target)} → ${count} صوت\n`
    }
    ...
    resultText += `\n👉 المتهم بالأغلبية: ${displayName(state, eliminated)}\nبينتقل الآن للمحكمة يدافع عن نفسه...`
...
// ---------------------------------------------------------
// 11.5) المحكمة: المتهم يدافع عن نفسه، ثم Poll إعدام/مسامحة يحسم مصيره
// ---------------------------------------------------------
async function startTrial(sock, groupId, state, accusedId) {
    if (!games.has(groupId)) return
    state.phase = 'trial'
    state.trial = { accusedId, timer: null, pollMsg: null, pollUpdates: [] }
    ...
    await sock.sendMessage(groupId, {
        text: `⚖️ ═══〔 المحكمة 〕═══ ⚖️\n\n${displayName(state, accusedId)} عندك دقيقتين تدافع عن نفسك وتوضح ليش ما لازم نصوّت عليك.\n\n⏱️ إذا خلصت كلامك بدري، اكتب: .خلصت`,
        mentions: [accusedId]
    })
...
async function finishDefense(sock, groupId, state) {
    ...
    // 🔇 يُسكت المتهم أيضاً، الحين وقت التصويت بالـ Poll بس
    await demotePlayers(sock, groupId, [accusedId], state)

    let pollMsg = null
    try {
        pollMsg = await sock.sendMessage(groupId, {
            poll: {
                // ⚠️ عنوان الـ Poll نص عادي، واتساب ما يدعم فيه منشن حقيقي (ما فيه
                // "mentions" خاص بالـ poll name)، فلو حطينا رقم اللاعب الخام بيظهر
                // كأرقام مب اسم. نستخدم الاسم المسجل بدل الرقم عشان يبين مفهوم.
                name: `⚖️ مصير ${displayName(state, accusedId)}: إعدام أم مسامحة؟`,
                values: ['⚖️ إعدام', '🤝 مسامحة'],
                selectableCount: 1
            },
            mentions: [accusedId]
        })
    } catch (err) {
        console.log('فشل إرسال Poll المحكمة:', err)
    }

    state.trial.pollMsg = pollMsg

    if (!pollMsg) {
        await sock.sendMessage(groupId, { text: '⚠️ صار خطأ بإرسال تصويت المحكمة، اعتُبر عفو تلقائي.' })
        return resolveTrialPoll(sock, groupId, state)
    }

    state.trial.timer = setTimeout(() => {
        resolveTrialPoll(sock, groupId, state).catch(console.log)
    }, TRIAL_POLL_MS)
}

// تُستدعى من index.js عند وصول حدث "messages.update" (فيه تحديثات تصويت الـ Poll)
async function handlePollUpdate(groupId, update) {
    const state = games.get(groupId)
    if (!state || !state.trial || !state.trial.pollMsg) return false
    const pollMsgId = state.trial.pollMsg.key && state.trial.pollMsg.key.id
    if (!pollMsgId || !update || !update.key || update.key.id !== pollMsgId) return false
    const pollUpdates = update.update && update.update.pollUpdates
    if (!pollUpdates || !pollUpdates.length) return false
    // ⚠️ كل حدث "messages.update" يوصل بس بتحديث الصوت الجديد (مو كل الأصوات
    // من البداية)، فلازم نراكم/نجمع كل التحديثات بمصفوفة وحدة بدل ما نستبدلها
    // بآخر تحديث بس. لو استبدلناها (كان الكود القديم كذا) نضيع كل الأصوات
    // السابقة ونحسب النتيجة غلط (مثلاً 0-0 بدل 3-2 الفعلية).
    if (!state.trial.pollUpdates) state.trial.pollUpdates = []
    state.trial.pollUpdates.push(...pollUpdates)
    return true
}

async function resolveTrialPoll(sock, groupId, state) {
    if (!games.has(groupId)) return
    if (!state.trial) return
    if (state.trial.timer) clearTimeout(state.trial.timer)
    ...
    if (trial.pollMsg && trial.pollMsg.message) {
        try {
            const aggregate = await getAggregateVotesInPollMessage({
                message: trial.pollMsg.message,
                pollUpdates: trial.pollUpdates || []
            })
            // ⚠️ تصويت الـ Poll تفاعل مستقل عن صلاحية الكتابة بالقروب، فحتى لاعب
            // ميت (اتنزّل من الإشراف) يقدر يفتح الـ Poll ويصوّت. لازم نستثني
            // أي صوت من شخص مو موجود بـ state.alive وقت الحسم، عشان بس
            // الأحياء يأثرون بمصير المتهم.
            for (const opt of aggregate) {
                const voters = (opt.voters || []).filter(v => state.alive.has(v))
                if (opt.name && opt.name.includes('إعدام')) executeVotes = voters.length
                if (opt.name && opt.name.includes('مسامحة')) pardonVotes = voters.length
            }
        } catch (err) {
            console.log('فشل تجميع أصوات Poll المحكمة:', err)
        }
    }
    ...
    if (!execute) {
        await sock.sendMessage(groupId, {
            text: `🤝 ═══〔 نتيجة المحكمة 〕═══ 🤝\n\n${displayName(state, accusedId)} نجا! (إعدام: ${executeVotes} — مسامحة: ${pardonVotes})\nيرجع بينهم بدون ما يموت.`,
            mentions: [accusedId]
        })
        return afterVoteResolved(sock, groupId, state)
    }
    ...
    await sock.sendMessage(groupId, {
        text: `⚖️ ═══〔 نتيجة المحكمة 〕═══ ⚖️\n\nتم إعدام ${displayName(state, accusedId)}! (إعدام: ${executeVotes} — مسامحة: ${pardonVotes})\n(كان دوره: ${ROLE_INFO[role].name})`,
        mentions: [accusedId]
    })
...
async function handleWinIfAny(sock, groupId, state) {
    ...
    state.players.forEach(p => {
        ...
        reveal += `${displayName(state, p)} — ${ROLE_INFO[role].name}\n`
    })
...
// ⚠️ لازم تُربط هذي بحدث Baileys "messages.update" من index.js عشان تُحسب
// أصوات Poll المحكمة صح (راجع تعليق أعلى الملف). مثال بسيط بـ index.js:
//
//   sock.ev.on('messages.update', async (updates) => {
//       for (const u of updates) {
//           if (u.key?.remoteJid?.endsWith('@g.us')) {
//               await werewolfGame.handlePollUpdate(u.key.remoteJid, u).catch(console.log)
//           }
//       }
//   })
module.exports = {
