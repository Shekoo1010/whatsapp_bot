...
async function finishDefense(sock, groupId, state) {
    ...
    try {
        pollMsg = await sock.sendMessage(groupId, {
            poll: {
                name: `⚖️ مصير ${mentionTag(accusedId)}: إعدام أم مسامحة؟`,
...
async function handlePollUpdate(groupId, update) {
    ...
    if (!pollUpdates) return false
    state.trial.pollUpdates = pollUpdates
    return true
}
    ...
    if (trial.pollMsg && trial.pollMsg.message) {
        try {
            ...
            for (const opt of aggregate) {
                if (opt.name && opt.name.includes('إعدام')) executeVotes = (opt.voters || []).length
                if (opt.name && opt.name.includes('مسامحة')) pardonVotes = (opt.voters || []).length
            }
        } catch (err) {
